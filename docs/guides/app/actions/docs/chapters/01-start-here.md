...
That flow gives all of your code a clear home: define routes in `routes.ts`, connect middleware, routes, and controllers in `router.ts`, handle route-specific behavior in `actions/`, and put shared views and layouts in `ui/`. Browser-reachable source lives in `public/` directories beside its narrowest owner, making the server/browser boundary explicit in the file structure.
...
└── app/
    ...
    └── assets.ts              # browser module server configuration and entry hrefs
```
...
```sh
mkdir -p app/actions/albums/edit/public
...
```tsx filename=app/actions/albums/edit/public/album-edit-form.tsx lines=[6]
// ...
