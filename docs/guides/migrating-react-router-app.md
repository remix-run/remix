...
If you're using React 17 or a non streaming API (eg. `renderToString`), we recommend using the [`ClientOnly` component](https://github.com/sergiodxa/remix-utils#clientonly) from the [`remix-utils`](https://npm.im/remix-utils) community package. An example of its usage can be found in the [`examples` repo](https://github.com/remix-run/examples/blob/main/client-only-components).
