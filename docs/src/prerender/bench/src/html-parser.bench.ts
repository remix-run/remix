...
import { parse as customParse } from '../../html-parser.ts'
...
for (let { name, html } of fixtures) {
  let customResult = extractLinksAndAssets(customParse(html).elements)
...
describe('Full Crawl Scenario - Landing Page', () => {
  bench('custom parser + extraction', () => {
    let elements = customParse(landingPage).elements
...
describe('Full Crawl Scenario - Blog Post', () => {
  bench('custom parser + extraction', () => {
    let elements = customParse(blogPost).elements
    extractLinksAndAssets(elements)
  })
...
describe('Full Crawl Scenario - Docs Page', () => {
  bench('custom parser + extraction', () => {
    let elements = customParse(docsPage).elements
    extractLinksAndAssets(elements)
  })
