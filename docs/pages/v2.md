...
The v2 dev server runs _alongside_ your app server, managing rebuilds, live reloads, and HMR/HDR all for you.
That means you will visit your app servers normal URL in your browser during development.

### `remix dev`

Checklist:

- [ ] Ensure you have `@remix-run/serve` installed as a dev dependency
- [ ] Remove redundant `NODE_ENV=development`
- [ ] Remove usages of `remix dev`'s `--port` flag
- [ ] Uninstall any unused dev dependencies like `npm-run-all` and `cross-env`

#### Example

You should already have `@remix-run/serve` installed as a dev dependency, but never hurts to make sure:

```sh
npm install -D @remix-run/serve
```

Let's say your `package.json` looks like:

```json
{
  "scripts": {
    "dev": "cross-env NODE_ENV=development remix dev --port 8001"
  }
}
```

After going through the checklist your `package.json` should look like:
...
Previously, you could use `remix watch` to rebuild your app whenever source files changed.
You probably also used a task runner like `npm-run-all` or `concurrently` to run your app server alongside `remix watch`.

The v2 dev server will manage your app server for you, so you no longer need `remix watch` nor a task runner.

You will need to call `broadcastDevReady` from within your app server to alert the dev server that your app is running and has up-to-date server code.

Checklist:

- [ ] Remove redundant `NODE_ENV=development`
- [ ] Replace `remix watch` with `remix dev`
- [ ] Use the `-c`/`--command` flag to specify the command to run your app server
- [ ] Call `broadcastDevReady` in your app server **after** it is up and running
- [ ] Uninstall any unused dev dependencies like `npm-run-all` and `cross-env`

If you are managing server changes yourself (e.g. `require` cache purging), you'll need to do one more thing:

- [ ] Use the `--no-restart` flag so that `remix dev` doesn't restart your app server when rebuildsucceed

#### Example

For example, let's say you are using an Express server and your `package.json` looks like:
...
```json
{
  "scripts": {
      ...
      "dev:server": "cross-env NODE_ENV=development nodemon --watch build/ ./server.js"
    }
}
```
...
And your `server.js` file should include a call to `broadcastDevReady`:
