...
You want to learn Remix? You're in the right place. Let's build [Remix Jokes][remix-jokes-1]!
...
You can follow along with this tutorial on [CodeSandbox][codesandbox-1] (a fantastic in-browser editor) or locally on your own computer. If you use the CodeSandbox approach then all you need is a good internet connection and a modern browser. If you run things locally then you're going to need some things installed:

- [Node.js][nodejs-1] 14 or greater
- [npm][www.npmjs-1] 7 or greater
- A code editor ([VSCode][code.visualstudio-1] is a nice one)

If you'd like to follow along with the deploy step at the end, you'll also want an account on [Fly.io][fly-1].
...
- [JavaScript to know for React][kentcdodds-1]
- [The Beginner's Guide to React][kcd-1]

And having a good understanding of [the HTTP API][developer.mozilla-2] is also helpful, but not totally required.
...
If you're planning on using CodeSandbox, you can use [the Basic example][codesandbox-2] to get started.
...
Remix can be deployed in a large and growing list of JavaScript environments. The "Remix App Server" is a full-featured [Node.js][nodejs-1] server based on [Express][expressjs-1]. It's the simplest option and it satisfies most people's needs, so that's what we're going with for this tutorial. Feel free to experiment in the future!
...
- `app/entry.client.tsx` - This is the first bit of your JavaScript that will run when the app loads in the browser. We use this file to [hydrate][reactjs-1] our React components.
...
Open [http://localhost:3000][localhost:3000-1] and the app should greet the world:

![Bare bones hello world app][1]
...
You can programmatically create routes via the [`remix.config.js`][2], but the more common way to create the routes is through the file system. This is called "file-based routing."

Each file we put in the `app/routes` directory is called a ["Route Module"][3] and by following [the route filename convention][4], we can create the routing URL structure we're looking for. Remix uses [React Router][reactrouter-1] under the hood to handle this routing.
...
![A greeting from the index route][5]
...
You should be presented with that component when you go to [`/jokes`][localhost:3000-2]. Now, in that `<Outlet />` we want to render out some random jokes in the "index route".
...
Now if you refresh [`/jokes`][localhost:3000-2], you'll get the content in the `app/routes/jokes.tsx` as well as the `app/routes/jokes/index.tsx`. Here's what mine looks like:

![A random joke on the jokes page: "I was wondering why the frisbee was getting bigger, then it hit me"][6]
...
Great, so now going to [`/jokes/new`][localhost:3000-3] should display your form:
...
Here the parameter `$jokeId` can be anything, and we can lookup that part of the URL up in the database to display the right joke. To make a parameterized route, we use the `$` character in the filename. ([Read more about the convention here][4]).
...
Great, so now going to [`/jokes/anything-you-want`][localhost:3000-4] should display what you just created (in addition to the parent routes):
...
You do this by exporting a [`links`][9] function in your route module. Let's get the homepage styled. You can put your CSS files anywhere you like within the `app` directory. We'll put ours in `app/styles/`.
...
💿 Now update `app/routes/index.tsx` to import that css file. Then add a `links` export (as described in [the documentation][9]) to add that link to the page.
...
Now if you go to [`/`][localhost:3000-1] you may be a bit disappointed. Our beautiful styles aren't applied! Well, you may recall that in the `app/root.tsx` we're the ones rendering _everything_ about our app. From the `<html>` to the `</html>`. That means if something doesn't show up in there, it's not going to show up at all!

So we need some way to get the `link` exports from all active routes and add `<link />` tags for all of them. Luckily, Remix makes this easy for us by providing a convenience [`<Links />`][10] component.
...
Great, now check [`/`][localhost:3000-5] again and it should be nice and styled for you:

![The homepage with a purple gradient background and white text with the words "Hello Index Route"][11]

Hooray! But I want to call out something important and exciting. You know how the CSS we wrote styles the `body` element? What would you expect to happen on the [`/jokes`][localhost:3000-2] route? Go ahead and check it out.

![The jokes page with no background gradient][12]
...
:root {
  ...
  --font-body: -apple-system, "Segoe UI", Helvetica Neue, Helvetica, Roboto, Arial,
...
<docs-info>Did you know that `<link />` tags can use media queries? [Check out the MDN page for `<link />`][developer.mozilla-3].</docs-info>
...
One quick note about CSS. A lot of you folks may be used to using runtime libraries for CSS (like [Styled-Components][www.styled-components-1]). While you can use those with Remix, we'd like to encourage you to look into more traditional approaches to CSS. Many of the problems that led to the creation of these styling solutions aren't really problems in Remix, so you can often go with a simpler styling approach.

That said, many Remix users are very happy with [Tailwind][tailwindcss-1] and we recommend this approach. Basically, if it can give you a URL (or a CSS file which you can import to get a URL), then it's a generally a good approach because Remix can then leverage the browser platform for caching and loading/unloading.
...
You can use any persistence solution you like with Remix; [Firebase][firebase.google-1], [Supabase][supabase-1], [Airtable][www.airtable-1], [Hasura][hasura-1], [Google Spreadsheets][www.google-1], [Cloudflare Workers KV][www.cloudflare-1], [Fauna][fauna-1], a custom [PostgreSQL][www.postgresql-1], or even your backend team's REST/GraphQL APIs. Seriously. Whatever you want.

### Set up Prisma

<docs-info>The prisma team has built [a VSCode extension][marketplace.visualstudio-1] you might find quite helpful when working on the prisma schema.</docs-info>

In this tutorial we're going to use our own [SQLite][sqlite-1] database. Essentially, it's a database that lives in a file on your computer, is surprisingly capable, and best of all it's supported by [Prisma][www.prisma-2], our favorite database ORM! It's a great place to start if you're not sure what database to use.
...
Now that we've got prisma initialized, we can start modeling our app data. Because this isn't a prisma tutorial, I'll just hand you that and you can read more about the prisma schema from [their docs][www.prisma-3]:
...
To _load_ data in a Remix route module, you use a [`loader`][13]. This is simply an `async` function you export that returns a response, and is accessed on the component through the [`useLoaderData`][14] hook. Here's a quick example:
...
So the only way to really be 100% positive that your data is correct, you should use [assertion functions][www.typescriptlang-1] on the `data` you get back from `useLoaderData`. That's outside the scope of this tutorial, but we're fans of [zod][npm-1] which can aid in this.
...
With that you should be able to go to [`/jokes`][localhost:3000-2] and click on a link to get the joke:

![Jokes page showing a unique joke][16]
...
With that your [`/jokes`][localhost:3000-2] route should display a list of links to jokes as well as a random joke:

![Jokes page showing a random joke][17]
...
Not much there. Just a form. What if I told you that you could make that form work with a single export to the route module? Well you can! It's the [`action`][18] function export! Read up on that a bit.
...
The `redirect` utility is a simple utility in Remix for creating a [`Response`][developer.mozilla-4] object that has the right headers/status codes to redirect the user.

</docs-info>

![Create new joke form filled out][19]

![Newly created joke displayed][20]
...
But if there's an error, you can return an object with the error messages and then the component can get those values from [`useActionData`][21] and display them to the user.
...
![New joke form with validation errors][22]
...
The `badRequest` helper function is important because it gives us typechecking that ensures our return value is of type `ActionData`, while still returning the accurate HTTP status, [`400 Bad Request`][developer.mozilla-5], to the client. If we just return the `ActionData` value, that would result in a `200 OK` response, which isn't suitable since the form submission had errors.
...
One thing that would be good to understand for this section is how [HTTP cookies][developer.mozilla-6] work on the web.
...
So our authentication will be of the traditional username/password variety. We'll be using [`bcryptjs`][npm-2] to hash our passwords so nobody will be able to reasonably brute-force their way into an account.
...
![Excalidraw Authentication diagram][23]
...
![A login form with a login/register radio button and username/password fields and a submit button][24]
...
Wahoo! We got the user! Now we need to put that user's ID into the session. We're going to do this in `app/utils/session.server.ts`. Remix has a built-in abstraction to help us with managing several types of storage mechanisms for sessions ([here are the docs][26]). We'll be using [`createCookieSessionStorage`][27] as it's the simplest and scales quite well.
...
Note: If you need a hand, there's a small example of how the whole basic flow goes in [the session docs][26]. Once you have that, you'll want to use it in `app/routes/login.tsx` to set the session and redirect to the `/jokes` route.
...
With that, pop open your [Network tab][developer.chrome-1], go to [`/login`][localhost:3000-6] and enter `kody` and `twixrox` and check the response headers in the network tab. Should look something like this:

![DevTools Network tab showing a "Set-Cookie" header on the POST response][28]

And if you check the cookies section of the [Application tab][developer.chrome-2] then you should have the cookie set in there as well.

![DevTools Application tab showing ][29]

And now every request the browser makes to our server will include that cookie (we don't have to do anything on the client, [this is how cookies work][developer.mozilla-6]):

![Request headers showing the Cookie][30]
...
<docs-info>Remember to check [the docs][26] to learn how to get the session from the request</docs-info>
...
In my example, I created a `requireUserId` which will throw a `redirect`. Remember `redirect` is a utility function that returns a [`Response`][developer.mozilla-4] object. Remix will catch that thrown response and send it back to the client. It's a great way to "exit early" in abstractions like this so users of our `requireUserId` function can just assume that the return will always give us the `userId` and don't need to worry about what happens if there isn't a `userId` because the response is thrown which stops their code execution!
...
💿 Update `app/utils/session.server.ts` to add a `getUser` function that gets the user from prisma and a `logout` function that uses [`destroySession`][31] to log the user out.
...
First, the new `logout` route is just there to make it easy for us to logout. The reason that we're using an action (rather than a loader) is because we want to avoid [CSRF][developer.mozilla-7] problems by using a POST request rather than a GET request. This is why the logout button is a form and not a link. Additionally, Remix will only re-call our loaders when we perform an `action`, so if we used a `loader` then the cache would not get invalidated. The `loader` is just there in case someone somehow lands on that page, we'll just redirect them back home.
...
![Jokes page nice and designed][32]

![New Joke form designed][33]
...
Luckily, error handling in Remix is stellar. You may have used React's [Error Boundary feature][reactjs-2]. With Remix, your route modules can export an [`ErrorBoundary` component][34] and it will be used. But it's even cooler because it works on the server too! Not only that, but it'll handle errors in `loader`s and `action`s too! Wowza! So let's get to it!
...
![Joke Index Page Error][37]

![New Joke Page Error][38]
...
It might help to think of the unexpected errors as 500-level errors ([server errors][developer.mozilla-8]) and the expected errors as 400-level errors ([client errors][developer.mozilla-9]).

For client error responses, Remix offers something similar to Error Boundaries. It's called [`Catch Boundaries`][39] and it works almost exactly the same. In this case, when your server code detects a problem, it'll throw a [`Response`][developer.mozilla-4] object. Remix then catches that thrown response and renders your `CatchBoundary`. Just like the `useLoaderData` hook to get data from the `loader` and the `useActionData` hook to get data from the `action`, the `CatchBoundary` gets its data from the `useCatch` hook. This will return the `Response` that was thrown.
...
![App 400 Bad Request][40]

![A 404 on the joke page][41]

![A 404 on the random joke page][42]

![A 401 on the new joke page][43]
...
This is why Remix has the [`meta`][44] export. Why don't you go through and add a useful few meta tags to the following routes:

- `app/routes/index.tsx`
- `app/routes/login.tsx`
- `app/routes/jokes/$jokeId.tsx` - (this one you can reference the joke's name in the title which is fun)

But before you get started, remember that we're in charge of rendering everything from the `<html>` to the `</html>` which means we need to make sure these `meta` tags are rendered in the `<head>` of the `<html>`. This is why Remix gives us a [`<Meta />` component][45].
...
![Network tab indicating no JavaScript is loaded][49]
...
💿 Use Remix's [`<Scripts />` component][45] component to load all the JavaScript files in `app/root.tsx`.
...
![Network tab showing JavaScript loaded][50]
...
![Browser console showing the log of a server-side error][51]

### Forms

Remix has its own [`<Form />`][52] component. When JavaScript is not yet loaded, it works the same way as a regular form, but when JavaScript is enabled, it's "progressively enhanced" to make a `fetch` request instead so we don't do a full-page reload.
...
We have a pretty in depth [guide on Optimistic UI][53], so go give that a read
...
Your app is now live at `https://<your-app-name>.fly.dev`! You can find that URL in your fly account online as well: [fly.io/apps][fly-5].

Any time you make a change, simply run `fly deploy` again to redeploy.

## Conclusion

Phew! And there we have it. If you made it through this whole thing then I'm really impressed ([tweet your success][twitter-1])! There's a lot to Remix and we've only gotten you started. Good luck on the rest of your Remix journey!

[remix-jokes-1]: https://remix-jokes.lol
[developer.mozilla-1]: https://developer.mozilla.org/en-US/
[www.prisma-1]: https://www.prisma.io/
[codesandbox-1]: https://codesandbox.com/
[nodejs-1]: https://nodejs.org
[www.npmjs-1]: https://www.npmjs.com
[code.visualstudio-1]: https://code.visualstudio.com/
[fly-1]: https://fly.io
[kentcdodds-1]: https://kentcdodds.com/blog/javascript-to-know-for-react
[kcd-1]: https://kcd.im/beginner-react
[developer.mozilla-2]: https://developer.mozilla.org/en-US/docs/Web/HTTP
[codesandbox-2]: https://codesandbox.io/s/github/remix-run/remix/tree/main/examples/basic
[expressjs-1]: https://expressjs.com/
[reactjs-1]: https://reactjs.org/docs/react-dom.html#hydrate
[localhost:3000-1]: http://localhost:3000
[1]: /jokes-tutorial/img/bare-bones.png
[2]: ../api/conventions#remixconfigjs
[3]: ../api/conventions#route-module-api
[4]: ../api/conventions#route-filenames
[reactrouter-1]: https://reactrouter.com/
[5]: /jokes-tutorial/img/index-route-greeting.png
[localhost:3000-2]: http://localhost:3000/jokes
[6]: /jokes-tutorial/img/random-joke.png
[localhost:3000-3]: http://localhost:3000/jokes/new
[7]: /jokes-tutorial/img/new-joke.png
[localhost:3000-4]: http://localhost:3000/jokes/hippos
[8]: /jokes-tutorial/img/param-route.png
[9]: ../api/conventions#links
[10]: ../api/remix#link
[localhost:3000-5]: http://localhost:3000/
[11]: /jokes-tutorial/img/homepage-styles.png
[12]: /jokes-tutorial/img/jokes-no-styles.png
[developer.mozilla-3]: https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link
[www.styled-components-1]: https://www.styled-components.com/
[tailwindcss-1]: https://tailwindcss.com/
[firebase.google-1]: https://firebase.google.com/
[supabase-1]: https://supabase.com/
[www.airtable-1]: https://www.airtable.com/
[hasura-1]: https://hasura.io/
[www.google-1]: https://www.google.com/sheets/about/
[www.cloudflare-1]: https://www.cloudflare.com/products/workers-kv/
[fauna-1]: https://fauna.com/features
[www.postgresql-1]: https://www.postgresql.org/
[marketplace.visualstudio-1]: https://marketplace.visualstudio.com/items?itemName=Prisma.prisma
[sqlite-1]: https://sqlite.org/index.html
[www.prisma-2]: https://www.prisma.io
[www.prisma-3]: https://www.prisma.io/docs/reference/api-reference/prisma-schema-reference
[13]: ../api/conventions#loader
[14]: ../api/remix#useloaderdata
[15]: /jokes-tutorial/img/jokes-loaded.png
[bundlephobia-1]: https://bundlephobia.com/package/graphql@16.0.1
[www.typescriptlang-1]: https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-7.html#assertion-functions
[npm-1]: https://npm.im/zod
[16]: /jokes-tutorial/img/joke-page.png
[17]: /jokes-tutorial/img/random-joke-loaded.png
[18]: ../api/conventions#action
[developer.mozilla-4]: https://developer.mozilla.org/en-US/docs/Web/API/Response
[19]: /jokes-tutorial/img/creating-new-joke.png
[20]: /jokes-tutorial/img/new-joke-created.png
[21]: ../api/remix#useactiondata
[22]: /jokes-tutorial/img/new-joke-form-with-errors.png
[developer.mozilla-5]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/400
[developer.mozilla-6]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies
[npm-2]: https://npm.im/bcryptjs
[23]: /jokes-tutorial/img/auth-flow.png
[24]: /jokes-tutorial/img/login-route.png
[25]: /jokes-tutorial/img/login-form-with-errors.png
[26]: ../api/remix#sessions
[27]: ../api/remix#createcookiesessionstorage
[developer.chrome-1]: https://developer.chrome.com/docs/devtools/network/reference/
[localhost:3000-6]: http://localhost:3000/login
[28]: /jokes-tutorial/img/network-tab-set-cookie.png
[developer.chrome-2]: https://developer.chrome.com/docs/devtools/storage/cookies/
[29]: /jokes-tutorial/img/application-tab-cookies.png
[30]: /jokes-tutorial/img/cookie-header-on-request.png
[31]: ../api/remix#using-sessions
[developer.mozilla-7]: https://developer.mozilla.org/en-US/docs/Glossary/CSRF
[32]: /jokes-tutorial/img/random-joke-designed.png
[33]: /jokes-tutorial/img/new-joke-designed.png
[reactjs-2]: https://reactjs.org/docs/error-boundaries.html#gatsby-focus-wrapper
[34]: ../api/conventions#errorboundary
[35]: /jokes-tutorial/img/app-level-error.png
[36]: /jokes-tutorial/img/joke-id-error.png
[37]: /jokes-tutorial/img/jokes-index-error.png
[38]: /jokes-tutorial/img/new-joke-error.png
[developer.mozilla-8]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#server_error_responses
[developer.mozilla-9]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#client_error_responses
[39]: ../api/conventions#catchboundary
[40]: /jokes-tutorial/img/app-400.png
[41]: /jokes-tutorial/img/joke-404.png
[42]: /jokes-tutorial/img/jokes-404.png
[43]: /jokes-tutorial/img/new-joke-401.png
[44]: ../api/conventions#meta
[45]: ../api/remix#meta-links-scripts
[46]: ../guides/resource-routes
[47]: ../api/conventions#escaping-special-characters
[48]: /jokes-tutorial/img/jokes-rss-feed.png
[49]: /jokes-tutorial/img/no-javascript.png
[50]: /jokes-tutorial/img/yes-javascript.png
[51]: /jokes-tutorial/img/server-side-error-in-browser.png
[52]: ../api/remix#form
[53]: ../guides/optimistic-ui
[fly-2]: https://fly.io/docs/hands-on/installing/
[fly-3]: https://fly.io/docs/hands-on/sign-up/
[fly-4]: https://fly.io/blog/free-postgres/#a-note-about-credit-cards
[fly-5]: https://fly.io/apps
[twitter-1]: https://twitter.com/intent/tweet?text=I%20went%20through%20the%20whole%20remix.run%20jokes%20tutorial!%20%F0%9F%92%BF%20And%20now%20I%20love%20@remix_run!&url=https://remix.run/tutorials/jokes
