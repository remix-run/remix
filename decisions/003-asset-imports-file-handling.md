...
There's no compile-time error if a requested asset doesn't exist. The string argument to `context.assets.resolve()` is opaque — no type checker or language server cross-references it to the source file.

Rather than returning null for missing or misconfigured assets, `resolve()` throws a typed `AssetError` (with subclasses `AssetNotFoundError`, `AssetVariantRequiredError`, `AssetVariantNotFoundError`, `AssetVariantUnexpectedError`). Missing assets surface immediately as loud errors rather than silent failures. The dev resolver validates against the filesystem using sync `fs.existsSync` checks, so dev and production behavior are consistent — a missing asset throws in both environments. Template code can wrap `resolve()` calls in a single `try/catch (e) { if (e instanceof AssetError) ... }` at the request boundary rather than null-checking at every call site.

The deeper issue remains: the string path passed to `resolve()` has no compile-time link to the source file. If an asset is renamed or deleted, nothing flags the stale `resolve()` call site until runtime.

> **Agent impact:** When an LLM renames or deletes an asset file, it has no static signal that `context.assets.resolve('app/images/old-name.png')` call sites elsewhere in the codebase also need updating. The string argument is opaque — no type checker or language server cross-references it to the source file. With static imports, TypeScript immediately flags every stale reference at the import site — giving the agent precise, actionable feedback before any code runs.
...
1. **"Same source, three variants (dynamic resolve)"** — BBQ book cover (`bbq-2.png`, `bbq-1.png`) resolved via `assets.resolve()` at request time. Throws `AssetError` if any asset or variant is missing.
...
| `assets-middleware`     | `assets()` middleware now throws `AssetError` subclasses (via `createAssetResolver`) instead of returning `null` for missing or misconfigured assets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
