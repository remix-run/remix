...
test.describe("Mark all as completed", () => {
  ...
  test("complete all checkbox should update state when items are completed / cleared", async ({
    ...
    let toggleAll = page.locator(".toggle-all");
    await toggleAll.check();
    await expect(toggleAll).toBeChecked();
    await checkNumberOfCompletedTodosInLocalStorage(page, 3);

    // Uncheck first todo.
    let firstTodo = page.locator(".todo-list li").nth(0);
...
test.describe("Item", () => {
  test("should allow me to mark items as complete", async ({ page }) => {
    // Create two items.
    for (let item of TODO_ITEMS.slice(0, 2)) {
    ...
    // Check second item.
    let secondTodo = page.locator(".todo-list li").nth(1);
  ...
  test("should allow me to edit an item", async ({ page }) => {
    ...
    let secondTodo = todoItems.nth(1);
...
test.describe("Editing", () => {
  ...
  test("should hide other controls when editing", async ({ page }) => {
    let todoItem = page.locator(".todo-list li").nth(1);
