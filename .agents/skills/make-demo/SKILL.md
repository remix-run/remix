...
- For demos that need development HMR, keep `server.ts` as the real app server and put `remix/node-hmr` supervision in `dev.ts`. Use a stable public proxy with `createHmrReadyFetch()` when browser requests can race child server restarts.
- For demos that combine HMR with `remix/ui`, use `--import remix/ui-hmr/node` for the child server and `uiHmr()` in the asset server's browser module hooks. Keep all HMR wiring development-only.
...
- `demos/<name>/dev.ts` when the demo has a separate development HMR runner
...
- Smoke-test HMR demos through their development script when the change depends on live server or browser updates.
