...
- The `optimizeBarrelFileImports: true` option rewrites named imports through eligible barrel files to their resolved implementation modules, avoiding intermediary requests and removing side-effect-free dependency branches that are no longer reachable. Every removed module must be declared side-effect-free by its nearest `package.json`. The optimization works with generated import maps or rewritten import URLs.
