...
const { generateDtsBundle } = require('dts-bundle-generator') as typeof DtsBundleGenerator
...
ts.sys.readFile = (filePath, encoding) => {
  let contents = originalReadFile(filePath, encoding)
  if (contents != null && filePath.endsWith('.d.ts') && contents.includes('/src/')) {
...
const remixPkg = JSON.parse(fs.readFileSync(path.join(remixDir, 'package.json'), 'utf8')) as {
  name: string
...
const preferredConfigPath = path.join(projectRoot, 'scripts/tsconfig.bundle.json')
...
function prepareModuleBody(bundle: string, refs: Set<string> = references): string {
  let bodyLines: string[] = []
  for (let line of bundle.split('\n')) {
...
function readPackageMeta(pkg: string): PackageMeta | undefined {
  ...
  try {
    ...
    try {
      ...
      while (true) {
        let candidate = path.join(dir, 'package.json')
        if (fs.existsSync(candidate)) {
          let name = JSON.parse(fs.readFileSync(candidate, 'utf8'))?.name
          if (name === pkg) {
  ...
  let meta = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8')) as PackageMeta
...
function splitSpecifier(specifier: string): { pkg: string; subpath: string } | undefined {
...
function resolveTypesEntry(specifier: string): string | undefined {
  let split = splitSpecifier(specifier)
  if (!split) return undefined
  let { pkg, subpath } = split

  let meta = readPackageMeta(pkg)
  ...
  let resolved = path.resolve(meta.dir, types)
...
function declaredSpecifiers(pkg: string): string[] {
  ...
  for (let [sub, value] of Object.entries(meta.exports ?? {})) {
...
function remixRunSpecifiers(text: string): string[] {
  ...
  for (let m of text.matchAll(/\bfrom\s+['"](@remix-run\/[^'"]+)['"]/g)) {
    found.add(m[1])
  }
  for (let m of text.matchAll(/\bimport\s+['"](@remix-run\/[^'"]+)['"]/g)) {
    found.add(m[1])
  }
  for (let m of text.matchAll(/\bimport\(\s*['"](@remix-run\/[^'"]+)['"]\s*\)/g)) found.add(m[1])
  return [...found]
}
...
function externalSpecifiers(text: string): string[] {
  ...
  for (let pattern of patterns) {
    for (let m of text.matchAll(pattern)) {
      let spec = m[1]
...
for (let [subpath, value] of Object.entries(remixPkg.exports)) {
  if (subpath === './package.json') continue
  let types = typeof value === 'object' ? value.types : undefined
  if (!types) continue

  let filePath = path.resolve(remixDir, types)
  ...
  let specifier =
    subpath === '.' ? remixPkg.name : `${remixPkg.name}/${subpath.replace(/^\.\//, '')}`
  let targets = remixRunSpecifiers(source)
  let reexport = targets[0]
  let isUI = targets.some((t) => t === '@remix-run/ui' || t.startsWith('@remix-run/ui/'))
...
function bundlePackage(pkg: string): string | undefined {
  let meta = readPackageMeta(pkg)
  if (!meta) return undefined
  let specifiers = declaredSpecifiers(pkg).filter((s) => resolveRemixRunTypes(s))
  if (specifiers.length === 0) return undefined

  let entryFile = path.join(meta.dir, `__remix_canonical_entry_${Date.now()}.d.ts`)
  fs.writeFileSync(entryFile, specifiers.map((s) => `export * from '${s}';`).join('\n') + '\n')
  try {
...
const seedSpecifiers = [...new Set(nonUiEntries.flatMap((e) => remixRunSpecifiers(e.source)))]
const packageQueue: string[] = []
const packageSeen = new Set<string>()
for (let spec of seedSpecifiers) {
  if (isUiSpecifier(spec)) continue
  referencedSpecifiers.add(spec)
  let pkg = packageOf(spec)
...
console.log(`bundling @remix-run/* dependency graph (seed: ${packageQueue.length} packages)...`)

while (packageQueue.length > 0) {
  let pkg = packageQueue.shift()!
  ...
  for (let spec of declaredSpecifiers(pkg)) referencedSpecifiers.add(spec)
  console.log(`  ${pkg} (${Date.now() - startedAt}ms)`)

  for (let next of remixRunSpecifiers(bundle)) {
    if (isUiSpecifier(next)) continue
    referencedSpecifiers.add(next)
    let nextPkg = packageOf(next)
...
for (let bundle of canonicalByPackage.values()) {
  for (let spec of externalSpecifiers(bundle)) externalSeed.add(spec)
}
for (let entry of nonUiEntries) {
  for (let spec of externalSpecifiers(entry.source)) externalSeed.add(spec)
}
...
for (let specifier of [...externalSeed].sort()) {
  let typesEntry = resolveTypesEntry(specifier)
  if (!typesEntry) {
  ...
  let pkg = splitSpecifier(specifier)!.pkg
  let startedAt = Date.now()
  try {
    let [bundle] = generateDtsBundle(
      [
...
for (let filePath of uniqueUiFiles) {
  let label = path.relative(remixDir, filePath)
  let startedAt = Date.now()
  try {
    let [bundle] = generateDtsBundle(
      [
...
function buildWrapperModule(entry: ExportEntry): string | undefined {
  ...
  let body = prepareModuleBody(entry.source)
...
for (let [pkg, bundle] of canonicalByPackage) {
  moduleBlocks.push(`declare module "${pkg}" {\n${prepareModuleBody(bundle)}\n}`)
}
...
function buildStandaloneModule(subpath: string): string {
  let specifier = `${remixPkg.name}/${subpath.replace(/^\.\//, '')}`
  let exp = remixPkg.exports[subpath]
  let types = typeof exp === 'object' ? exp.types : undefined
  if (!types) {
    console.warn(`missing export types for ${specifier}`)
    return ''
  }
  let filePath = path.resolve(remixDir, types)
  let bundle = uiBundleByFile.get(filePath)
  if (bundle == null) {
    console.warn(`missing bundle for ${specifier}`)
    return ''
  }

  let localRefs = new Set<string>()
  let body = prepareModuleBody(bundle, localRefs)
...
;(function collectNodeTypes(dir: string) {
  for (let entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      let rel = path.relative(nodeTypesDir, full).split(path.sep).join('/')
...
const output =
  ...
  `export const nodeTypes: ReadonlyArray<readonly [string, string]> = ${JSON.stringify(
    nodeTypeFiles,
  )};\n`
...
if (failures.length > 0) {
  ...
  for (let { filePath, error } of failures) {
