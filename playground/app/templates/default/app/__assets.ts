type ProcessWithDevServer = NodeJS.Process & {
  __playgroundDevServer: (url: URL) => Promise<{
    body?: string
    statusCode: number
...
export const assetServer = {
  ...
  async fetch(request: Request) {
    try {
      ...
      let res = await (process as ProcessWithDevServer).__playgroundDevServer(url)
  ...
  async getHref(filePath: string) {
    let previewPort = process.env.PREVIEW_PORT || '44100'
