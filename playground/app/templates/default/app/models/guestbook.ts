...
export async function getGuestbookEntries({
  ...
  let entries = await db.findMany(schema.guestBook, {
...
export async function createGuestbookEntry(input: CreateGuestbookInput): Promise<GuestbookEntry> {
  let result = await db.create(schema.guestBook, {
    name: input.name,
  })
  ...
  let entry = await db.findOne(schema.guestBook, {
    where: {
      id: result.insertId,
    },
  })
