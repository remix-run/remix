...
function buildTemplateFiles(raw: Record<string, string>): Record<string, TemplateFile> {
  let files: Record<string, TemplateFile> = {}
  let implementations: Record<string, string> = {}

  for (let [globPath, contents] of Object.entries(raw)) {
    let relativePath = globPath.replace(/^\.\/default\//, '')
    let { dir, name } = splitPath(relativePath)
    ...
    let readonly = name.startsWith('_')
    let realName = readonly ? name.slice(1) : name
    files[joinPath(dir, realName)] = { readonly, contents }
  }

  for (let [path, implementation] of Object.entries(implementations)) {
    let file = files[path]
    if (file) {
