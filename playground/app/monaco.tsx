...
export const monacoPromise = import('monaco-editor').then((monaco) => {
  ...
  for (let [relPath, contents] of nodeTypes) {
    monaco.typescript.typescriptDefaults.addExtraLib(
      contents,
...
>((handle) => {
  ...
  function applyModel(next: monacoTypes.editor.ITextModel | null) {
    ...
    if (next) {
      let state = viewStates.get(next)
      if (state) editor.restoreViewState(state)
    }
  }

  let resize = new ResizeObserver(() => editor?.layout())
  ...
  handle.addEventListener('insert', async (event) => {
    ...
    let codeEditorService = (editor as unknown as { _codeEditorService?: CodeEditorService })
      ._codeEditorService
    if (codeEditorService) {
      let openBase = codeEditorService.openCodeEditor.bind(codeEditorService)
      codeEditorService.openCodeEditor = async (input, source, sideBySide) => {
        let result = await openBase(input, source, sideBySide)
        // The base service handled it (same model / in-editor jump).
        if (result || !editor) return result

        let target = openModel?.(input.resource)
