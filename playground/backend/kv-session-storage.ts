...
export function kvSessionStorage<valueData extends Data = Data, flashData extends Data = Data>(
  ...
  return {
    async read(cookie) {
      ...
      if (id) {
        try {
          let cached = await namespace.get(id)
          let data: SessionData<valueData, flashData> | undefined =
            cached && typeof cached === 'string' ? JSON.parse(cached) : undefined
          return createSession<valueData, flashData>(id, data)
        } catch (error) {
          console.error('Failed to read session data', error)
        }
      }
      return createSession<valueData, flashData>(useUnknownIds ? id : undefined)
    },
    async save(session) {
      if (session.deleteId) {
      ...
      if (session.dirty) {
        await namespace.put(session.id, JSON.stringify(session.data)).catch((error) => {
