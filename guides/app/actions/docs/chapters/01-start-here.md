...
Start by creating a browser module for the form:

```sh
touch app/assets/album-edit-form.tsx
```

The template's asset server is configured to serve browser-owned modules under `app/assets/`, so when a server-rendered component becomes a client entry, Remix can turn its source file into a browser module URL. The template already loads `app/assets/entry.ts`, which starts the Remix UI client runtime in the browser. The [Files and Assets](/files-and-assets/) chapter goes deeper into our bundlerless approach and how `remix/assets` works.
...
export function AlbumEditForm(
  handle: Handle<{ action: string; album: Album }>,
) {
  return () => {
    let { album } = handle.props;
...
export const AlbumEditForm = clientEntry(
  import.meta.url,
  function AlbumEditForm(handle: Handle<{ action: string; album: Album }>) {
    // ...
  },
);
```
