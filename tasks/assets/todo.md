...
### Remove unnecessary `__hmr_request_remount` wrapper

**Problem:**

The HMR runtime currently imports `requestRemount` from `@remix-run/component` and re-exports it as `__hmr_request_remount`:
...
export function __hmr_request_remount(handle: any) {
  requestRemount(handle) // Trivial wrapper
}
```

The HMR transform then imports this wrapper and uses it when a component's setup hash changes:

```typescript
// Generated by hmr-transform.ts
import { __hmr_request_remount /* ... */ } from '/__@remix/hmr-runtime.ts'

if (__hmr_setup(handle, __s, hash, setupFn)) {
  __hmr_request_remount(handle) // Could just call requestRemount directly
  return () => null
}
```

This is an unnecessary middleman. The transform could import `requestRemount` directly from `@remix-run/component`.

**Benefits of removing the wrapper:**

1. **Clearer separation of concerns**: `requestRemount` is part of the component API, not HMR infrastructure
2. **Less code**: No trivial re-export wrapper
3. **More explicit imports**: Transform clearly shows it depends on both HMR runtime and component package
4. **Simpler runtime**: HMR runtime doesn't need to know about `requestRemount` at all

**Solution:**

1. Remove `requestRemount` import from `hmr-runtime.ts`
2. Remove `__hmr_request_remount` function and export from `hmr-runtime.ts`
3. Update `hmr-transform.ts`:
   - Remove `'__hmr_request_remount'` from `HMR_IMPORTS` array
   - Add separate import for `requestRemount` from `@remix-run/component` (needs import rewriting to workspace URL)
   - Update generated code to call `requestRemount(handle)` directly instead of `__hmr_request_remount(handle)`

**Implementation notes:**

The transform will need to generate two import statements:

```typescript
// HMR infrastructure
import { __hmr_state, __hmr_setup /* ... */ } from '/__@remix/hmr-runtime.ts'

// Component API
import { requestRemount } from '@remix-run/component' // Will be rewritten to /__@workspace/...
```

**Acceptance Criteria:**

- [ ] `requestRemount` import removed from `hmr-runtime.ts`
- [ ] `__hmr_request_remount` function and export removed from `hmr-runtime.ts`
- [ ] Transform generates separate import for `requestRemount` from `@remix-run/component`
- [ ] Transform calls `requestRemount(handle)` directly instead of `__hmr_request_remount(handle)`
- [ ] All E2E tests pass (HMR still works)
- [ ] All unit tests pass
- [ ] Generated code has correct imports (verify in browser dev tools)
