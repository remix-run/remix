...
test.describe("root splat routes when no index route is present", () => {
  test("matches routes correctly (v1)", async ({ page }) => {
    ...
    let app = new PlaywrightFixture(appFixture, page);
    await app.goto("/");
    expect(await app.getHtml()).toMatch("Hello Splat");

    await app.goto("/splat");
    expect(await app.getHtml()).toMatch("Hello Splat");

    await app.goto("/splat/deep/path");
    expect(await app.getHtml()).toMatch("Hello Splat");
...
test.describe("root splat routes when an index route is present", () => {
  test("matches routes correctly (v1)", async ({ page }) => {
    fixture = await createFixture({
      future: { v2_routeConvention: false },
      files: {
    ...
    let app = new PlaywrightFixture(appFixture, page);
    await app.goto("/");
    expect(await app.getHtml()).toMatch("Hello Index");
