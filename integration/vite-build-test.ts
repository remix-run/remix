...
test.describe("Vite build", () => {
  ...
  <h3>Loading...</h3>
    ...
    test("hydrates", async ({ page }) => {
      ...
      expect(
        await page.locator("#content h3[data-mounted]").textContent()
      ).toBe("Mounted");
    });
  ...
  test.describe("supports additional build.rollupOptions.input", () => {
    let fixture: Fixture;
    let appFixture: AppFixture;
    test.beforeAll(async () => {
      fixture = await createFixture({
        ...
        files: {
          ...baseFiles,
          "app/additional-input.ts": js`
            export const additionalInput = "additionalInput";
          `,
          "vite.config.ts": js`
            import { defineConfig } from "vite";
            import { vitePlugin as remix } from "@remix-run/dev";
            ...
            export default defineConfig(({ isSsrBuild }) => ({
              build: {
                ...
                rollupOptions: {
                  input: isSsrBuild ? {
                    additional: "./app/additional-input.ts",
                  } : ["./app/additional-input.ts"]
                },
              },
              plugins: [
                mdx(),
    ...
    test("should emit additional entry for server build", async () => {
      let clientBuildDir = path.join(fixture.projectDir, "build/client/assets");
      let serverBuildDir = path.join(fixture.projectDir, "build/server");

      let serverEntries = glob
        .sync("*.@(js)", {
          cwd: serverBuildDir,
        })
        .sort();
      expect(serverEntries).toEqual(["additional.js", "index.js"]);

      let clientEntries = glob
        .sync("*.@(js)", {
          cwd: clientBuildDir,
        })
        .sort();
      expect(clientEntries.some((v) => v.startsWith("additional-input-"))).toBe(
        true
      );
    });
  });
