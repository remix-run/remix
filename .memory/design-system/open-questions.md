...
- Add shared interactive row/item mixins beyond the current first pass.
  - useful for tabs, menus, combobox options, command items, navigation rows
- Add a few more structural mixins for list/menu/select-style items.
- Expand the animation mixin family beyond `ui.animation.spin`.
  ...
  - worth exploring a few calm, utilitarian motion mixins rather than one-off animation classes
- Add a first-party `Spinner` component.
  ...
  - should help prove the relationship between glyphs, icon sizing, and animation mixins
- Add an `ActionButton` component with loading-state ergonomics.
  ...
  - good candidate for proving thin component ergonomics on top of `ui.button.*`, glyphs, and motion mixins
...
- Validate the abstraction with an alternate scoped theme.
  - use a second theme or scoped theme variant to confirm mixins actually adapt cleanly
...
- contextual composition mixins and blocks
  - surface text, field text, menu text, item rows, controls, surfaces, statuses, and layout primitives

## Guardrails

- Do not confuse demo-specific mixins or blocks with final design-system taxonomy.
