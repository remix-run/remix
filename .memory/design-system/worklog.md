...
- demo composes heavily from semantic `ui` mixins instead of bespoke styles
...
- The `System Mixins` section needs to behave like a reference artifact, not like a set of normal content cards.
- `Content Areas` benefit when intro text and list rhythm are handled as separate spacing boundaries.
- The card mixin layer is a good fit for this codebase because it captures spacing structure without forcing wrapper components.
...
- Add a few more reusable motion mixins and demos.
...
- Add component-level follow-ups for loading affordances.
  - `Spinner` should wrap the spinner glyph + shared motion mixin
