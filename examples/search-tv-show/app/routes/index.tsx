...
export let loader = async ({request}) => {
  ...
  return {
    status: 'resultsFound',
    searchTerm,
    items: data.filter(item => item.show.image).map(item => ({
      id: item.id,
      name: item.show.name,
      ...
      <Form method="get" className="search-form">
        ...
        {transition.state === 'submitting' ? (
          <div className="results">
            {[...Array(8).keys()].map(() => <div className="placeholder" />)}
          </div>
        ) : (
          <>
            {data.status === 'emptySearch' && (
              <p className="info">Start searching... <span role="img" aria-label="point up emoji">☝️</span></p>
            )}

            {data.status === 'noResults' && (
              <p className="info">Ooops, no results <span role="img" aria-label="crying emoji">😢</span></p>
            )}

            {data.status === 'resultsFound' && (
              <div className="results">
                {data.items.map(item => (
                  <a href={item.url} target="_blank" rel="noreferrer" className="placeholder" key={item.id}>
