...
export function page(body: string, title: string = 'Bookstore'): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
export function layout(content: string, user: User | null = null): string {
  ...
  <header>
    <div class="container">
      <h1><a href="${routes.home.href()}">📚 Bookstore</a></h1>
      <nav>
        ...
        ${
          ...
          ${user.role === 'admin' ? `<a href="${routes.admin.index.href()}">Admin</a>` : ''}
        ...
        `
          ...
          <a href="${routes.auth.register.href()}">Register</a>
        `
        }
      </nav>
    </div>
  ...
  <footer>
    <div class="container">
      <p>&copy; 2024 Bookstore Demo. Built with fetch-router.</p>
    </div>
  </footer>
`,
  )
