...
export let StateSearchPage = clientEntry(moduleUrl, (handle: Handle, setup?: string) => {
  ...
  return () => (
    <section>
      ...
      >
        <label css={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 320px' }}>
          <span css={{ fontSize: 13, color: '#b9c6ff' }}>Search states</span>
          <input
            connect={(node) => (input = node)}
      ...
      {query.trim() ? (
        ...
        >
          <Frame
            ...
            fallback={<div css={{ color: '#9aa8e8' }}>Searching states…</div>}
          />
        </div>
      ) : (
        <p css={{ margin: 0, color: '#9aa8e8' }}>Enter a state name to run the frame search.</p>
      )}
    </section>
  )
})
