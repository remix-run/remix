...
- [`app/auth/controller.tsx`](app/auth/controller.tsx) uses `verifyCredentials()` and `completeAuth()` from `remix/auth` for the login POST action, so the route owns redirect behavior while Remix still owns credentials verification and session rotation.
