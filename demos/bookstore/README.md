...
- [`app/router.ts`](app/router.ts) installs the standard Remix UI render middleware with the app asset server, so actions use `context.render` and frames resolve through the router without app-owned integration code.
