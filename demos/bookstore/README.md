...
- [`app/middleware/models.ts`](app/middleware/models.ts) binds the model registry to the shared database for each request, populates `context.models`, and declares the `RequestContext` type augmentation in the same module.
- [`app/models.ts`](app/models.ts) defines a model registry (`Book`, `User`, etc.) plus a `BoundModels` interface for request context typing.
...
- [`app/models/book.ts`](app/models/book.ts), [`app/models/user.ts`](app/models/user.ts), and [`app/models/order.ts`](app/models/order.ts) show class-based models with ActiveRecord-style static methods built on `remix/data-model`.
