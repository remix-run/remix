...
- [`app/auth/controller.tsx`](app/auth/controller.tsx) uses `createCredentialsAuthLoginRequestHandler(passwordProvider, ...)` from `remix/auth` for the login POST action, so session rotation and session writes come from the framework abstraction instead of bespoke route code.
...
- [`app/admin/books/controller.tsx`](app/admin/books/controller.tsx) demonstrates complete CRUD operations with file uploads, using the RESTful routes generated by `resources('books')`.
