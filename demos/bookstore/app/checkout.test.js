...
import { assertContains, getSessionCookie, loginAsCustomer, requestWithSession, } from '../test/helpers.js';
describe('checkout handlers', () => {
    ...
    it('POST /checkout creates order when authenticated with items in cart', async () => {
        ...
        assert.ok(orderId);
        let orderDetailsRequest = requestWithSession(`https://remix.run/account/orders/${orderId}`, sessionCookie);
