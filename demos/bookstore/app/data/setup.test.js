...
import { db, initializeBookstoreDatabase } from './setup.js';
...
describe('bookstore database setup', () => {
    it('applies migrations and materializes expected schema artifacts', async () => {
        ...
        let ordersIndex = await db.exec(sql `select name from sqlite_master where type = 'index' and name = 'orders_user_id_idx'`);
        let orderItemsOrderIndex = await db.exec(sql `select name from sqlite_master where type = 'index' and name = 'order_items_order_id_idx'`);
