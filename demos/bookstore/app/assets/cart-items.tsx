...
export let CartItems = clientEntry(
  routes.scripts.href({ path: 'app/assets/cart-items.tsx' }) + '#CartItems',
  (handle: Handle) => {
    ...
    return ({ items, total, canCheckout }: CartItemsProps) => {
      ...
      return (
        <>
          ...
          <div
            mix={[css({ marginTop: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' })]}
          >
            <p
              mix={[
                css({ margin: 0, fontSize: '1.25rem', fontWeight: 'bold', marginRight: 'auto' }),
              ]}
            >
              Total: {totalLabel}
            </p>
