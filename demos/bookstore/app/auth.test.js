...
import { getSessionCookie, assertContains } from '../test/helpers.js';
describe('auth handlers', () => {
    ...
    it('flash error message is cleared after being displayed once', async () => {
        ...
        assert.ok(!secondHtml.includes('Invalid email or password'), 'Expected flash error to be cleared after first display');
    });
    ...
    it('accessing protected route redirects to login with returnTo parameter', async () => {
        ...
        assert.ok(location.includes(encodeURIComponent('/checkout')), 'Expected returnTo to contain /checkout');
    });
    it('successful login with returnTo redirects to original destination', async () => {
        let response = await router.fetch('https://remix.run/login?returnTo=' + encodeURIComponent('/checkout'), {
            method: 'POST',
            body: new URLSearchParams({
    ...
    it('failed login with returnTo preserves returnTo parameter', async () => {
        ...
        let location = response.headers.get('Location');
        assert.ok(location, 'Expected Location header');
        assert.ok(location.includes('returnTo=' + encodeURIComponent('/checkout')), 'Expected returnTo to be preserved in redirect');
