...
This demo renders a long page without fetching every section up front. Its first study sends one checked-in HTML/CSS motion artifact through a regular server-rendered `Frame` and the same route through `LazyFrame`. The remaining sections use the same lazy client component: it server-renders a placeholder, registers its stable host with a shared observer, then mounts a Remix `Frame` when the host enters a 320 px vertical viewport margin. Requested Frames stay mounted when they leave the viewport, so scrolling back never triggers another round trip.
...
- [`app/ui/public/lazy-frame.tsx`](app/ui/public/lazy-frame.tsx) owns Frame mounting and optional stage activity. It pools intersection observation by `rootMargin`, while its `children` and `fallback` props keep waiting and fetching presentation caller-owned. Its host-level `css()` mixin demonstrates styling Frame descendants from outside the response.
...
Run `pnpm test` and `pnpm typecheck` to validate the demo's route behavior and types.
