...
`Request` to `Response` contract. A `renderWith()` middleware exposes `context.render(node)` to
routes and converts each rendered node into a `nodeResponse`. Then `run(router, { fallback })` from
`remix/ui/spa` renders a live Remix fallback while the existing top-frame navigation runtime loads
the associated route node.
