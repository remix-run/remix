...
The end-to-end tests build and preview the production app by default, then pass Vite's URL and
cleanup method directly to `t.serve()`. Call `runTests('development')` in the test file to run the
same suite against Vite's development server instead.
