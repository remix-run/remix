...
Development mode transforms TypeScript/JSX files on-the-fly:
...
- [`server.ts`](server.ts) uses `@remix-run/dev-assets-middleware` in development for on-the-fly TypeScript/JSX transformation, and `@remix-run/assets-middleware` in production with pre-built assets and a manifest.
