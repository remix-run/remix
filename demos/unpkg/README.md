...
- [`app/middleware/render.ts`](app/middleware/render.ts) installs the request-scoped HTML renderer used by actions through `context.render`, while [`app/ui/document.ts`](app/ui/document.ts) keeps the shared document shell separate from feature controllers.
