...
- [`app/actions/controller.ts`](app/actions/controller.ts) owns the top-level home and package-browser actions, with package-browser response rendering colocated in [`app/actions/package-browser.ts`](app/actions/package-browser.ts) and lower-level formatting/MIME helpers in [`app/utils`](app/utils).
...
- [`app/middleware/render.ts`](app/middleware/render.ts) installs the request-scoped HTML renderer used by actions through `context.get(Renderer)`, while [`app/ui/document.ts`](app/ui/document.ts) keeps the shared document shell separate from feature controllers.
