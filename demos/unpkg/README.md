...
- [`app/middleware/render.ts`](app/middleware/render.ts) demonstrates the `renderWith()` escape hatch with a custom `HtmlDocument` renderer, while [`app/ui/document.ts`](app/ui/document.ts) keeps the shared document shell separate from feature controllers.
