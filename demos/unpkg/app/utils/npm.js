...
import { tarballCache, getTarballCacheKey } from './cache.js';
 ...
 */
export function isFullyResolvedVersion(metadata, specifier) {
 ...
 */
export function resolveVersion(metadata, specifier) {
 ...
 */
export async function fetchPackageContents(packageName, version) {
 ...
 */
async function fetchTarball(packageName, version, tarballUrl) {
    ...
    let file = new File([decompressed.buffer], `${packageName}@${version}.tar`, {
 ...
 */
async function parseTarball(data) {
    let files = new Map();
 ...
 */
export function getFilesAtPath(files, dirPath) {
    ...
    for (let [filePath, file] of files) {
        ...
        if (normalizedDir) {
            ...
            if (relativePath.includes('/') && file.type === 'file')
                continue;
 ...
 *   "@remix-run/cookie@1.0.0/src/index.js" -> { name: "@remix-run/cookie", version: "1.0.0", filePath: "src/index.js" }
 *   "react@^18.2" -> { name: "react", version: "^18.2", filePath: "" } (semver range)
 */
export function parsePackagePath(path) {
