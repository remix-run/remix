...
import { renderDirectoryListing } from './directory.js';
import { renderError } from './error.js';
import { parsePackagePath, fetchPackageMetadata, fetchPackageContents, getFilesAtPath, isFullyResolvedVersion, resolveVersion, PackageNotFoundError, VersionNotFoundError, InvalidPathError, } from './utils/npm.js';
import { html, render } from './utils/render.js';
export let router = createRouter();
router.map(routes, {
    actions: {
        home() {
            return render('UNPKG - npm package browser', html `
        ...
        async browse({ params }) {
            ...
            try {
                ...
                if (error instanceof PackageNotFoundError) {
                    return renderError('Package not found', `The package "${error.packageName}" was not found on npm.`);
                }
                if (error instanceof VersionNotFoundError) {
                    return renderError('Version not found', `Version "${error.version}" of package "${error.packageName}" was not found.`);
                }
