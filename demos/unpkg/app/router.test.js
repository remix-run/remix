...
import { installFetchMock, restoreFetchMock, addFetchHandler, createNpmRegistryMock, } from '../test/mock-fetch.js';
import { router } from './router.js';
before(() => {
    installFetchMock();
    addFetchHandler(createNpmRegistryMock({
        'is-number': {
...
describe('router', () => {
    ...
    describe('browse route', () => {
        ...
        it('returns file content for valid file path', async () => {
            let response = await router.fetch(new Request('http://localhost/is-number@7.0.0/package.json'));
            let text = await response.text();
            assert.equal(response.status, 200);
