...
function previewHorizontalDaySwap(
  ...
  if (movedBlock.dayOfWeek === originalBlock.dayOfWeek) return null
  if (movedBlock.startMinute !== originalBlock.startMinute) return null

  let displacedBlocks = originalBlocks.filter(
    (block) =>
      block.id !== movedBlock.id &&
      block.dayOfWeek === movedBlock.dayOfWeek &&
  ...
  let candidate = originalBlocks.map((block) => {
    ...
    swappedBlock.dayOfWeek = originalBlock.dayOfWeek
    return swappedBlock
  })
