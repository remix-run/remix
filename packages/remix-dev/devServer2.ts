...
export let serve = async (
  ...
  let appServerPort = flags.appServerPort ?? unstable_dev.appServerPort;
  if (appServerPort === undefined) {
    throw Error(
      `Must specify app server port via flag '--app-server-port' or future config 'unstable_dev.appServerPort'`
    );
  }
