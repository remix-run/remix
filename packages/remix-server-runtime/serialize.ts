...
import type { DeferredResponse, TypedResponse } from "./responses";
 ...
 */
export type SerializeFrom<T extends AppData | ArbitraryFunction> = T extends (
  ...args: any[]
) => infer Output
  ? Awaited<Output> extends DeferredResponse<infer U>
    ? SerializeDeferredObject<U>
...
// TODO: MOve to a better spot in the file:
// prettier-ignore
type SerializeDeferredObject<T extends object> = {
  [k in keyof T as T[k] extends NonJsonPrimitive
    ? never
    : k]: T[k] extends PromiseLike<infer U>
    ? Promise<Serialize<Awaited<U>>>
    : Serialize<T[k]>;
};
