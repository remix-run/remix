        ...
        for (let [suiteName, suiteTests] of suiteMap) {
            ...
            for (let test of suiteTests) {
                if (test.status === 'passed') {
                    ...
                    if (test.name)
                        console.log(`${testIndent}${colors.dim('↓')} ${colors.dim(`${test.name} ${formatPendingComment('skipped', test.reason)}`)}`);
                }
                else if (test.status === 'todo') {
                    if (test.name)
                        console.log(`${testIndent}${colors.yellow('…')} ${colors.yellow(`${test.name} ${formatPendingComment('todo', test.reason)}`)}`);
                }
            }
        }
    }
