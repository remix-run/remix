...
import { IS_BUN } from "./lib/utils.js";
...
async function executeRun() {
    ...
    try {
        ...
        if (config.coverage) {
            thresholdsPassed = await generateCombinedCoverageReport(allCoverageMaps, process.cwd(), config.coverage);
        }
...
async function findFiles(pattern, excludePattern) {
    let cwd = process.cwd();
    let files = [];
    if (IS_BUN) {
