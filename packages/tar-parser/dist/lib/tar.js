...
import { buffersEqual, concatChunks, computeChecksum, decodeLongPath, decodePax, getOctal, getString, overflow, } from './utils.js';
 ...
 */
export function parseTarHeader(block, options) {
...
function validatePaths(header, policy = 'relative') {
    if (policy === 'preserve')
        return;
...
function decodeTarHeader(block, options) {
    ...
    if (checksum !== computeChecksum(block)) {
        throw new TarParseError('Invalid tar header. Maybe the tar is corrupted or needs to be gunzipped?');
    }
...
function isLongHeader(type) {
    return (type === 'gnu-long-path' ||
        type === 'gnu-long-link-path' ||
        type === 'pax-global-header' ||
        type === 'pax-header');
}
export async function parseTar(archive, options, handler) {
    let opts;
...
export class TarParser {
     ...
     */
    constructor(options = {}) {
        let { maxEntrySize = 2 * 1024 * 1024, maxTotalSize = 20 * 1024 * 1024, maxEntries = 5000, } = options;
    ...
    #write(chunk, handler) {
        ...
        while (this.#buffer !== null && this.#buffer.length > 0) {
            ...
            if (this.#buffer.length < TarBlockSize)
                break;
    ...
    #parseHeader(handler) {
        ...
        if (pax) {
            if (pax.path)
                this.#header.name = pax.path;
            if (pax.linkpath)
                this.#header.linkname = pax.linkpath;
            this.#header.pax = pax;
            this.#pax = null;
        }
