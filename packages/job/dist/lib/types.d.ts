 ...
 */
export type EnqueueOptions<transaction = never> = EnqueueOptionsBase<transaction> & (EnqueueDelayOptions | EnqueueRunAtOptions);
/**
 ...
 */
export type JobReference<defs extends JobDefinitions, name extends JobName<defs> = JobName<defs>> = defs[name];
/**
 ...
 */
export type SchedulerEnqueueEvent<defs extends JobDefinitions, transaction = never> = SchedulerEnqueueInput<defs> & {
    /**
 ...
 */
export type WorkerHookName = 'onJobStart' | 'onJobComplete' | 'onJobRetry' | 'onJobFailed' | 'onPrune';
/**
...
export interface JobScheduler<defs extends JobDefinitions, transaction = never> {
     ...
     */
    enqueue<name extends JobName<defs>>(job: JobReference<defs, name>, payload: Infer<defs[name]['schema']>, options?: EnqueueOptions<transaction>): Promise<{
        jobId: string;
        deduped: boolean;
    }>;
    /**
     ...
     */
    retryFailedJob(jobId: string, options?: RetryFailedJobOptions<transaction>): Promise<RetryFailedJobResult>;
    /**
