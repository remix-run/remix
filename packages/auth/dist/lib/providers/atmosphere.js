...
export function createAtmosphereAuthProvider(options) {
    ...
    async function handleCallback(context, transaction) {
    ...
    async function refreshTokens(currentTokens) {
        ...
        let authorizationServer = validateAtmosphereTokenAuthorizationServer(currentTokens.authorizationServer);
        let tokenResponse = await sendDpopFormRequest({
            endpoint: authorizationServer.tokenEndpoint,
    ...
    let provider = createOAuthProvider(ATMOSPHERE_PROVIDER_NAME, {
        createAuthorizationURL() {
            throw new Error('Atmosphere auth must be prepared with provider.prepare(handleOrDid) before starting the flow.');
        },
        handleCallback,
        refreshTokens,
    });
...
function validateAtmosphereTokenAuthorizationServer(value) {
    if (typeof value !== 'object' || value == null || Array.isArray(value)) {
        throw new Error('Atmosphere tokens are missing authorization server metadata.');
    }
    let authorizationServer = value;
    if (typeof authorizationServer.issuer !== 'string' ||
    ...
    return {
        issuer,
        tokenEndpoint: ensureUrl(authorizationServer.tokenEndpoint, issuer, 'Atmosphere token endpoint'),
    };
}
...
async function resolveHandleToDid(handle) {
    ...
    let dnsAttemptPromise = resolveHandleViaDns(handle, dnsAbortController.signal).then((value) => ({
        source: 'dns',
    ...
    let httpsAttemptPromise = resolveHandleViaHttps(handle, httpsAbortController.signal).then((value) => ({
        source: 'https',
        result: { status: 'fulfilled', value },
    }), (reason) => ({
        source: 'https',
    ...
    let dnsPreferenceTimeout;
    let dnsPreferenceTimeoutPromise = new Promise((resolve) => {
    ...
    if (dnsPreferenceAttempt != null) {
        if (dnsPreferenceAttempt.result.status === 'fulfilled' &&
    ...
    let pendingAttempts = new Map([
        ['https', httpsAttemptPromise],
        ...(dnsPreferenceAttempt == null ? [['dns', dnsAttemptPromise]] : []),
    ]);
...
async function resolveHandleViaDns(handle, signal) {
...
async function resolveHandleViaHttps(handle, signal) {
...
function createDidWebDocumentUrl(did) {
    ...
    if (path.some((segment) => segment.length === 0 || segment.includes('/') || segment === '.' || segment === '..')) {
        throw new Error(`Unsupported did:web identifier "${did}".`);
    }
