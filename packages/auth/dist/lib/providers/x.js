import { createAuthorizationURL, createOAuthProvider, exchangeAuthorizationCode, exchangeRefreshToken, fetchJson, getAuthorizationCode, mergeRefreshedTokens, } from '../provider.js';
...
const X_PROFILE_ENDPOINT = 'https://api.x.com/2/users/me?user.fields=profile_image_url,verified,description,url';
 ...
 */
export function createXAuthProvider(options) {
    ...
    return createOAuthProvider('x', {
        ...
        async handleCallback(context, transaction) {
            ...
            let response = await fetchJson(X_PROFILE_ENDPOINT, {
                headers: {
...
function validateXProfile(response) {
