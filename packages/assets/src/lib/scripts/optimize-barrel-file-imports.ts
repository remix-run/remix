...
import type { ImportRewrite, ResolvedImport, ResolvedModule } from './resolve.ts'
...
type ImportDeclarationGroup = {
  declarations: [ImportDeclaration, ...ImportDeclaration[]]
  depPath: string
}
...
type SideEffects = boolean | readonly string[] | null

export function createBarrelFileImportOptimizer(): (
  graph: ReadonlyMap<string, ResolvedModule>,
) => Map<string, ResolvedModule> {
  let analysisCache = new WeakMap<ResolvedModule, ExportAnalysis>()

  return function optimizeBarrelFileImports(
    graph: ReadonlyMap<string, ResolvedModule>,
  ): Map<string, ResolvedModule> {
    ...
    for (let module of graph.values()) {
      ...
      let importRewrites: ImportRewrite[] = []
      let replacedImports = new Set<ResolvedImport>()
      ...
      for (let group of getImportDeclarationGroups(module, analysis.importDeclarations)) {
        ...
        if (!getExportAnalysis(importedModule, analysisCache).isBarrelFile) continue

        let resolvedBindingsByDeclaration = new Map<
          ImportDeclaration,
          Array<ImportBinding & ExportResolution>
        >()
        let unresolved = false
        for (let declaration of group.declarations) {
          let resolvedBindings: Array<ImportBinding & ExportResolution> = []
          for (let binding of declaration.bindings) {
            let resolution = resolveExport(
              importedModule.identityPath,
        ...
        if (unresolved || resolvedBindings.length === 0) continue
        ...
        if (
          !canOptimizeBarrelFileImport(
            module.identityPath,
            importedModule.identityPath,
            targetPaths,
            resolvedBindings.flatMap((binding) => binding.barrelFilePaths),
            graph,
            sideEffectsCache,
          )
        ) {
        ...
        let replacementImportPaths = getReplacementImportPaths(
          importedModule.identityPath,
          targetPaths,
          graph,
        )
      ...
      let remainingStaticDeps = new Set(
        runtimeDependencies
...
function getImportSpecifiers(
  bindings: readonly (ImportBinding & ExportResolution)[],
  depPath: string,
): ImportRewrite['imports'][number]['specifiers'] {
  return bindings
...
function getImportDeclarationGroups(
  module: ResolvedModule,
  declarations: readonly ImportDeclaration[],
): ImportDeclarationGroup[] {
  let groupsByDepPath = new Map<string, ImportDeclarationGroup>()
  let supportedImports = new Set<ResolvedImport>()
...
function getReplacementImportPaths(
  barrelFilePath: string,
  targetPaths: readonly string[],
  graph: ReadonlyMap<string, ResolvedModule>,
): string[] {
  let retained = getStaticallyReachableModules(targetPaths, graph)
  let replacementImports: string[] = []
  let requested = new Set<string>()
  let evaluated = new Set<string>()
  let visited = new Set<string>()

  function markEvaluated(identityPath: string): void {
    let queue = [identityPath]
    while (queue.length > 0) {
      let currentPath = queue.pop()
      if (!currentPath || evaluated.has(currentPath)) continue
  ...
  function visit(identityPath: string): void {
    ...
    for (let imported of module.imports) {
      if (!imported.dynamic) visit(imported.depPath)
    }
  }
  ...
  for (let targetPath of targetPaths) {
    if (!requested.has(targetPath)) replacementImports.push(targetPath)
  }

  return replacementImports
}

function canOptimizeBarrelFileImport(
  importerPath: string,
  barrelFilePath: string,
  targetPaths: readonly string[],
  barrelFilePaths: readonly string[],
  graph: ReadonlyMap<string, ResolvedModule>,
  sideEffectsCache: Map<string, SideEffects>,
): boolean {
  let retained = getStaticallyReachableModules(targetPaths, graph)
  ...
  if (barrelFileGraph.has(importerPath)) return false
  // Entering the same cycle through a target instead of the original import can change
  // initialization order.
  if (barrelFilePaths.some((modulePath) => retained.has(modulePath))) return false
  for (let module of barrelFileGraph.values()) {
    if (retained.has(module.identityPath)) continue
    if (!isSideEffectFree(module, sideEffectsCache)) return false
  }
  return true
}

function getStaticallyReachableModules(
...
function isSideEffectFree(module: ResolvedModule, cache: Map<string, SideEffects>): boolean {
...
function readSideEffects(packageJsonPath: string): SideEffects {
  try {
    ...
    if (
      ...
      sideEffects.every((pattern): pattern is string => typeof pattern === 'string')
    ) {
      return sideEffects
    }
  } catch {}
...
function getExportAnalysis(
  module: ResolvedModule,
  cache: WeakMap<ResolvedModule, ExportAnalysis>,
): ExportAnalysis {
  ...
  let explicit = new Map<string, Reexport>()
  let importDeclarations: ImportDeclaration[] = []
  let importedLocals = new Map<string, Reexport>()
  let reexportedLocals = new Set<string>()
  let stars: string[] = []
  ...
  for (let statement of parseResult.program.body) {
    if (statement.type === 'ImportDeclaration') {
      let imported = findResolvedImport(
        module.imports,
...
function findResolvedImport(
  ...
  return imports.find(
    (imported) => imported.start === sourceStart + 1 && imported.end === sourceEnd - 1,
  )
}
...
function resolveExport(
  ...
  for (let depPath of analysis.stars) {
    ...
    if (
      resolution.depPath !== candidate.depPath ||
