...
export function toAssetVarName(sourcePath: string): string {
  let slash = sourcePath.lastIndexOf('/')
  let filename = slash >= 0 ? sourcePath.slice(slash + 1) : sourcePath
  let parts = filename.split(/[^\p{L}\p{N}]+/u).filter(Boolean)
  let camel = parts
    .map((part, i) =>
      i === 0 ? part.toLowerCase() : part.charAt(0).toUpperCase() + part.slice(1).toLowerCase(),
    )
    .join('')
  if (/^\p{N}/u.test(camel)) camel = '_' + camel
  return camel + 'Asset'
}

export function generatePlaceholderScriptContent(sourcePath: string): string {
  let href = `/__@assets/${sourcePath}`
  let varName = toAssetVarName(sourcePath)
  return (
    [
      GENERATED_MARKER,
      `// source: ${sourcePath}`,
...
export function generatePlaceholderFileContent(sourcePath: string, rule: CompiledFileRule): string {
  ...
  let varName = toAssetVarName(sourcePath)
  let lines = [GENERATED_MARKER, `// source: ${sourcePath}`, `const ${varName} = {`]

  if (rule.variants) {
    let variantNames = Object.keys(rule.variants)
    if (rule.defaultVariant) {
      lines.push(`  href: '${baseUrl}?@${rule.defaultVariant}',`)
    }
    lines.push(`  variants: {`)
...
function formatPreloads(urls: string[], indent = ''): string {
...
function generateBuildScriptContent(
  ...
  return (
    [
      ...
      `const ${varName} = {`,
      `  href: '${href}',`,
      `  preloads: ${formatPreloads(preloadUrls, '  ')},`,
      `} as const`,
      `export default ${varName}`,
    ].join('\n') + '\n'
  )
...
function generateBuildFileContent(
  ...
  if ('variants' in output) {
    ...
    if (output.defaultVariant) {
      ...
      lines.push(`  href: '${toAssetUrl(defaultPath, baseUrl)}',`)
    }
    lines.push(`  variants: {`)
    ...
    lines.push(`  href: '${toAssetUrl(output.path, baseUrl)}',`)
  }
