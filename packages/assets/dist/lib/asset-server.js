...
export function createAssetServer(options) {
    ...
    async function handleBrowserHmrFileEvents(events) {
        ...
        for (let event of events) {
            ...
            scriptUpdates.push(...(await scriptCompiler.classifyHmrFileEvent(normalizedFilePath, event.event)));
        ...
        let scriptPayload = await createScriptHmrPayloadWithImportMaps(dedupeScriptHmrUpdates(scriptUpdates));
        if (scriptPayload) {
    ...
    let assetServer = {
        ...
        async fetch(request) {
            ...
            try {
                ...
                let scriptResult = await scriptCompiler.getScript(parsedRequestPathname.filePath, {
                    hmrTimestamp: resolvedOptions.hmr === null
                        ? null
        ...
        async getImportMap(filePath) {
            ...
            if (resolvedOptions.hmrModuleImporter) {
                let moduleImporter = await scriptCompiler.resolveSpecifierFromRoot(resolvedOptions.hmrModuleImporter);
...
function parseHmrTimestamp(value) {
...
function dedupeScriptHmrUpdates(updates) {
    let deduped = [];
    let seen = new Set();
...
function resolveAssetServerOptions(options) {
    ...
    let optimizeBarrelFileImports = normalizeOptimizeBarrelFileImportsOption(options.optimizeBarrelFileImports);
...
function normalizeImportMapsOption(importMaps) {
    if (importMaps !== undefined && typeof importMaps !== 'boolean') {
        throw new TypeError('importMaps must be a boolean');
    }
    return importMaps ?? true;
}
function normalizeOptimizeBarrelFileImportsOption(optimizeBarrelFileImports) {
