 ...
 */
export function createFsFileCache(options = {}) {
    ...
    async function initialize() {
        ...
        let pending = await fs.stat(pendingPath).catch((error) => {
        ...
        try {
            records = (await storage.list({ includeMetadata: true, limit: Number.MAX_SAFE_INTEGER }))
                .files;
            if (!records.every(isCacheRecordMetadata))
                throw new SyntaxError('Invalid cache metadata');
        }
        catch (error) {
            if (!(error instanceof SyntaxError) && !isFileError(error, 'ENOENT'))
                throw error;
    ...
    function withCacheMutation(action) {
    ...
    async function removeEntry(state, key) {
    ...
    async function trim(state, incomingSize = 0) {
        let entryLimit = maxEntries - (incomingSize > 0 ? 1 : 0);
        for (let key of state.entries.keys()) {
            if (state.entries.size <= entryLimit && state.size <= maxTotalSize - incomingSize)
                break;
    ...
    return {
        async get(key) {
            ...
            let file = await readRecord(state.storage, digest, maxFileSize).catch((error) => {
                if (isFileError(error, 'ENOENT'))
                    return null;
                throw error;
            });
            if (!file) {
                await withCacheMutation(async (current) => {
                    ...
                    if (current.entries.get(digest) === entry)
                        await removeEntry(current, digest);
                });
            }
            return file;
        },
        async put(key, file) {
            ...
            await withCacheMutation(async (state) => {
                ...
                if (state.entries.has(digest))
                    await removeEntry(state, digest);
...
export function createTransformCacheKey(namespace, identity) {
    return hash(new TextEncoder().encode(JSON.stringify(['transformed-file-v3', namespace, identity])));
}
async function readRecord(storage, digest, maxFileSize) {
    let file;
    try {
    ...
    if (bytes.length > maxFileSize || bytes[64] !== 10)
        return null;
    let decoder = new TextDecoder();
    let checksum = decoder.decode(bytes.subarray(0, 64));
    if ((await hash(bytes.subarray(65))) !== checksum)
        return null;
...
function isCacheRecordMetadata(value) {
