...
export function isTransformCacheable(output) {
...
export async function createTransformCacheKey(namespace, identity) {
...
export async function readCachedTransform(cache, key) {
    let file = await cache.get(key.slot);
    if (!file || file.size > maxCacheFileSize)
        return null;
    let bytes = new Uint8Array(await file.arrayBuffer());
    if (bytes.length > maxCacheFileSize || bytes[64] !== 10 || bytes[129] !== 10)
        return null;
    let decoder = new TextDecoder();
    if (decoder.decode(bytes.subarray(65, 129)) !== key.digest)
        return null;
...
export async function writeCachedTransform(cache, key, output) {
    if (!isTransformCacheable(output))
        return;
