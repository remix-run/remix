...
import { createTransformCacheKey, isTransformCacheable, readCachedTransform, writeCachedTransform, } from './cache.js';
...
export function createFileCompiler(options) {
    ...
    return {
        async getFile(filePath, getOptions) {
            ...
            if (shouldUseTransformPipeline(getOptions.transform)) {
                let parsedTransforms = parseRequestTransforms(getOptions.transform, resolvedOptions.transforms, resolvedOptions.maxRequestTransforms);
                let transformedCacheKey = await getTransformedRecordCacheKey(cacheKey, record, parsedTransforms);
                let notModified = getNotModifiedFile(getTransformedAssetMetadata(transformedCacheKey), getOptions);
                if (notModified)
                    return notModified;
                let transformedFile = await getOrCreateTransformedFile(record, parsedTransforms, transformedCacheKey);
        ...
        async getHref(filePath, hrefOptions) {
            ...
            if (resolvedOptions.fingerprintAssets) {
                ...
                if (shouldUseTransformPipeline(hrefOptions.transform)) {
                    let parsedTransforms = parseRequestTransforms(hrefOptions.transform, resolvedOptions.transforms, resolvedOptions.maxRequestTransforms);
                    let transformedCacheKey = await getTransformedRecordCacheKey(cacheKey, record, parsedTransforms);
                    emittedFile = await getOrCreateTransformedFile(record, parsedTransforms, transformedCacheKey);
                }
                else {
    ...
    async function getOrCreateTransformedFile(record, parsedTransforms, transformedCacheKey) {
    ...
    async function applyTransforms(filePath, source, transforms) {
        ...
        for (let requestTransform of transforms) {
            ...
            if (transform === undefined) {
                throw createAssetServerCompilationError(`Unknown file transform "${requestTransform.name}" requested for ${filePath}`, { code: 'FILE_TRANSFORM_QUERY_INVALID' });
            }
            if (!supportsTransformExtension(transform.extensions, currentExtension)) {
                throw createAssetServerCompilationError(`File transform "${requestTransform.name}" does not support ${currentExtension} inputs for ${filePath}`, { code: 'FILE_TRANSFORM_NOT_SUPPORTED' });
            }
            let result;
            let transformSubject = {
                kind: 'request',
    ...
    async function getCachedTransformedFile(cacheKey, identityPath) {
        ...
        if (!file)
            return null;
        let body = file.body;
        let metadata = getTransformedAssetMetadata(cacheKey) ??
    ...
    function clearTransformedCacheIndex(identityPath) {
        for (let [slot, entry] of transformedAssetMetadataBySlot) {
            if (entry.identityPath === identityPath)
                transformedAssetMetadataBySlot.delete(slot);
        }
    }
    function getTransformedAssetMetadata(key) {
        let entry = transformedAssetMetadataBySlot.get(key.slot);
        return entry?.digest === key.digest ? entry.metadata : undefined;
    }
    function rememberTransformedAssetMetadata(key, identityPath, metadata) {
...
function getTransformedRecordCacheKey(namespace, record, transforms) {
    return createTransformCacheKey(namespace, JSON.stringify([record.identityPath, record.invalidationVersion, transforms]));
}
...
function normalizeTransformResult(result, options) {
    ...
    if (result === null || typeof result !== 'object') {
        throw createAssetServerCompilationError(`${formatFileTransformSubject(options.transformSubject)} must return a string, Uint8Array, or object for ${options.filePath}`, { code: 'FILE_TRANSFORM_RESULT_INVALID' });
    }
    if (!('content' in result) ||
        (typeof result.content !== 'string' && !(result.content instanceof Uint8Array))) {
        throw createAssetServerCompilationError(`${formatFileTransformSubject(options.transformSubject)} must return a string or Uint8Array content value for ${options.filePath}`, { code: 'FILE_TRANSFORM_RESULT_INVALID' });
    }
    let extension = options.currentExtension;
    if ('extension' in result && result.extension !== undefined) {
        if (typeof result.extension !== 'string') {
            throw createAssetServerCompilationError(`${formatFileTransformSubject(options.transformSubject)} must return a string extension for ${options.filePath}`, { code: 'FILE_TRANSFORM_RESULT_INVALID' });
        }
        extension = normalizeTransformExtension(result.extension, options.filePath, options.transformSubject);
    }
    return {
...
function normalizeTransformExtension(extension, filePath, transformSubject) {
    let normalizedExtension = extension.trim().toLowerCase();
    if (!/^\.[A-Za-z0-9_-]+$/.test(normalizedExtension)) {
        throw createAssetServerCompilationError(`${formatFileTransformSubject(transformSubject)} returned an invalid extension "${extension}" for ${filePath}`, { code: 'FILE_TRANSFORM_RESULT_INVALID' });
    }
    return normalizedExtension;
}
function toFileTransformFailedError(error, filePath, transformSubject) {
    if (isAssetServerCompilationError(error))
        return error;
    return createAssetServerCompilationError(`${formatFileTransformSubject(transformSubject)} failed for ${filePath}. ${formatUnknownError(error)}`, {
        cause: error,
        code: 'FILE_TRANSFORM_FAILED',
    });
}
function getGlobalFileTransformSubject(globalTransform, index) {
...
function formatFileTransformSubject(transformSubject) {
