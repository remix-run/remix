...
import { createTransformCacheKey } from './cache.js';
...
export function createFileCompiler(options) {
    ...
    return {
        async getFile(filePath, getOptions) {
            ...
            if (shouldUseTransformPipeline(getOptions.transform)) {
                ...
                let notModified = getNotModifiedFile(toEmittedFileMetadata(transformedFile), getOptions);
                if (notModified)
                    return notModified;
                return {
    ...
    async function getOrCreateTransformedFile(record, parsedTransforms, transformedCacheKey) {
        ...
        let promise = (async () => {
            ...
            if (cache) {
                let name = path.basename(record.identityPath, path.extname(record.identityPath)) +
                    emittedFile.extension;
                await cache.put(transformedCacheKey, new File([toArrayBufferView(emittedFile.body)], name, { type: emittedFile.contentType }));
            }
            return emittedFile;
        })();
    ...
    async function applyTransforms(filePath, source, transforms) {
        ...
        let currentBody = new Uint8Array(source.body);
    ...
    async function getCachedTransformedFile(cacheKey, identityPath) {
        ...
        if (!file)
            return null;
        return createEmittedFile(new Uint8Array(await file.arrayBuffer()), {
