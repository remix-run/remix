...
export function createScriptCompiler(options) {
    ...
    async function getOrCreateEmittedScript(record) {
        ...
        let promise = (async () => {
            ...
            if (isFresh(record, startedVersion)) {
                scriptStore.setEmitted(record.identityPath, emitResolvedModuleResult.value, createModuleSnapshot(sourceRoot.trackedFiles));
            }
    ...
    async function resolveServedGraph(roots) {
        let sourceGraph = await resolveScriptGraph(roots);
        if (!barrelFileImportOptimizer)
            return sourceGraph;
        return getReachableGraph(roots.map((root) => root.identityPath), barrelFileImportOptimizer(sourceGraph));
    }
    function getReachableGraph(rootIdentityPaths, graph) {
