...
function createImportNarrowingConfig(specifier, exportSpecifiers) {
    let exportByName = new Map(Object.entries(exportSpecifiers).map(([name, narrowedSpecifier]) => [
        name,
        {
            name,
...
function narrowPackageImports(rawCode, resolvedPath, externalSet) {
    ...
    for (let node of parseResult.program.body) {
        ...
        if (!config || node.specifiers.length === 0)
            continue;
        if (externalSet.has(config.specifier))
            continue;
        if (node.specifiers.some((specifier) => specifier.type !== 'ImportSpecifier'))
            continue;
        let narrowedSpecifiers = new Map();
        let remainingSpecifiers = [];
        let changed = false;
...
function mayContainNarrowableSpecifier(rawCode) {
...
function formatNarrowedImport(imported, local) {
    return imported === local ? imported : `${imported} as ${local}`;
}
function formatImportSpecifiers(specifiers) {
