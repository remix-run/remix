...
async function analyzeModuleSource(sourceText, resolvedPath, transformOptions, options) {
    ...
    let componentHmrExportNames = null;
...
function runModuleLoadHooks(url, options) {
    ...
    for (let hook of loadHooks) {
        ...
        load = (nextUrl, nextContext) => {
            ...
            if (!nextLoadCalled && result.shortCircuit !== true) {
                throw createAssetServerCompilationError(`Module load hook for ${nextUrl} returned without calling nextLoad or setting shortCircuit: true.`, {
                    code: 'TRANSFORM_FAILED',
                });
            }
            return result;
...
function getModuleLoadSource(url, result) {
    if (result.format !== 'module') {
        throw createAssetServerCompilationError(`Module load hook for ${url} returned unsupported format ${JSON.stringify(result.format)}. Only "module" is supported.`, {
            code: 'TRANSFORM_FAILED',
        });
    }
...
function getComponentHmrExportNames(source) {
    let match = /__remixHmrComponentExportNames = (\[[^\n]*\]);/.exec(source);
    let namesSource = match?.[1];
    if (namesSource === undefined)
        return null;
    try {
        let names = JSON.parse(namesSource);
