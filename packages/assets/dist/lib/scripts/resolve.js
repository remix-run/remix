...
async function getBareImportScopePathname(args) {
    ...
    if (!routeMapping) {
        throw createAssetServerCompilationError(`Bare import "${args.specifier}" in ${args.importerPath} cannot be represented by an import map scope because its fileMap entry does not preserve directory hierarchy. ` +
            `Use a fileMap entry with the same trailing named wildcard in its URL and file patterns.`, { code: 'IMPORT_MAP_SCOPE_NOT_SUPPORTED' });
    }
    ...
    if (!isWithinDirectory(importerDirectory, minimumDirectory)) {
        throw createAssetServerCompilationError(`Bare import "${args.specifier}" in ${args.importerPath} cannot be represented by its fileMap entry.`, { code: 'IMPORT_MAP_SCOPE_NOT_SUPPORTED' });
    }
...
function getScopePathname(routeMapping, minimumDirectory, directory) {
    let relativeDirectory = path.relative(minimumDirectory, directory).replaceAll('\\', '/');
    return ensureTrailingSlash(relativeDirectory
...
function isWithinDirectory(directory, parentDirectory) {
