...
export async function resolveModule(record, transformed, args) {
    ...
    try {
        ...
        let failedRelativeResolutions = transformed.unresolvedImports
            .map((unresolved) => getTrackedRelativeImportResolution(transformed.importerDir, getDisplayImportSpecifier(unresolved.specifier), args.isWatchIgnored, true))
            .filter((resolution) => resolution !== null);
        return failResolve(error, trackedFiles, failedRelativeResolutions.map((resolution) => ({
            ...resolution,
            resolvedIdentityPath: null,
        })), transformed.resolvedPath, { isWatchIgnored: args.isWatchIgnored });
    }
    ...
    for (let unresolved of transformed.unresolvedImports) {
        ...
        if (!resolvedSpec?.absolutePath) {
            trackedResolution ??= getTrackedRelativeImportResolution(transformed.importerDir, displaySpecifier, args.isWatchIgnored, true);
    ...
    if (args.trackPackageSideEffects) {
        for (let candidatePath of getPackageJsonCandidatePaths(transformed.resolvedPath, packageJsonPath, args.packageJsonSearchRoot)) {
            if (!args.isWatchIgnored(candidatePath))
                resolveTrackingFiles.add(candidatePath);
        }
    }
    for (let trackedFile of trackedFiles)
        resolveTrackingFiles.add(trackedFile);
    return {
        ok: true,
...
function getPackageJsonCandidatePaths(filePath, nearestPackageJsonPath, searchRoot) {
    let candidates = [];
    let directory = path.dirname(filePath);
    ...
    let searchWithinRoot = directory === normalizedSearchRoot || directory.startsWith(`${normalizedSearchRoot}/`);
    while (true) {
        candidates.push(normalizeFilePath(path.join(directory, 'package.json')));
        if (directory === nearestPackageDirectory)
            break;
        if (nearestPackageDirectory === null &&
...
function getTrackedRelativeImportResolution(importerDir, specifier, isWatchIgnored, trackExactFile = false) {
