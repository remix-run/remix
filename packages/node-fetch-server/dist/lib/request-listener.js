...
async function sendResponseForCreationError(res, onError, error, isResponseClosed) {
...
function createClientAddress(req, headers, options) {
...
function getRequestAuthority(req) {
    ...
    if (authority != null && host != null) {
        ...
        let protocol = (Array.isArray(scheme) ? scheme[0] : scheme) ??
            ('encrypted' in req.socket && req.socket.encrypted ? 'https' : 'http');
