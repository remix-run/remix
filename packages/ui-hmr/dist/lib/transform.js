...
function createHmrImplementationBody(match, source, moduleUrl) {
    ...
    let renderReferencedDeclarationNames = getRenderReferencedSetupDeclarationNames(match.renderArgument, match.setupStatements);
    let renderStateNames = new Set(setupStateNames);
    for (let name of renderReferencedDeclarationNames)
        renderStateNames.add(name);
    let setupSource = createSetupSource(match.setupStatements, setupStateNames, renderReferencedDeclarationNames, source);
...
function createSetupSource(statements, setupStateNames, renderReferencedDeclarationNames, source) {
    let lines = [];
    let initializedStateNames = new Set();
    let hoistedFunctionStateNames = new Set();
    ...
    for (let statement of statements) {
        if (isSetupRuntimeDeclaration(statement)) {
            ...
            if (name && renderReferencedDeclarationNames.has(name)) {
                ...
                if (!hoistedFunctionStateNames.has(name))
                    lines.push(`__s__.${name} = ${name};`);
                continue;
            }
        }
...
function shouldRewriteIdentifier(node, parent) {
    ...
    if (node.type === 'JSXIdentifier') {
        if (parent.type === 'JSXAttribute' && getNode(parent, 'name') === node)
            return false;
        if (parent.type === 'JSXNamespacedName')
            return false;
        if (parent.type === 'JSXMemberExpression' && getNode(parent, 'property') === node)
            return false;
        return true;
    }
...
function getRenderReferencedSetupDeclarationNames(renderArgument, setupStatements) {
    let declarationNames = new Set();
...
function isSetupRuntimeDeclaration(node) {
    return (node.type === 'FunctionDeclaration' ||
        node.type === 'ClassDeclaration' ||
        node.type === 'TSEnumDeclaration');
}
function getReferencedNames(root, names) {
...
function getSetupHash(body, source) {
    ...
    let hoistedDeclarations = returnIndex === -1
        ? []
        : statements
            .slice(returnIndex + 1)
            .filter((statement) => statement.type === 'FunctionDeclaration')
...
function getIdentifierName(node) {
    if (!node || (node.type !== 'Identifier' && node.type !== 'JSXIdentifier'))
        return null;
