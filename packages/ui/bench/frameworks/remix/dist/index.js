...
var ComponentRuntime = class {
  ...
  #dequeueTasks(signal) {
    let needsSignal = signal === void 0 && this.#tasks.some((task) => task.length);
    if (needsSignal) {
...
addNamespacedSvgAliases("xmlns", void 0, "xlink");
function addNamespacedSvgAliases(prefix, ns, names) {
  for (let name2 of names.split(" ")) {
    let attr = `${prefix}:${name2}`;
    let alias = ns ? { ns, attr } : { attr };
    NAMESPACED_SVG_ALIASES.set(`${prefix}${name2.charAt(0).toUpperCase()}${name2.slice(1)}`, alias);
    NAMESPACED_SVG_ALIASES.set(attr, alias);
    NAMESPACED_SVG_ALIASES.set(`${prefix}-${name2}`, alias);
  }
}
...
function isIgnoredProp(name2) {
  return name2 === "children" || name2 === "mix" || name2 === "key" || name2 === "animate" || name2 === "innerHTML" || name2 === "on" || name2 === "class" || name2 === "className";
}
...
function patchHostProps(curr, next, dom) {
  ...
  for (let name2 in curr) {
    if (isIgnoredProp(name2)) continue;
...
function patchStyleObject(dom, curr, next) {
  ...
  if (curr) {
    for (let name2 in curr) {
      ...
      style.removeProperty(toCssPropertyName(name2));
    }
  }
  for (let name2 in next) {
    let nextCssValue = styleValueToCss(name2, next[name2]);
    ...
    style.setProperty(toCssPropertyName(name2), nextCssValue);
  }
}
...
// ../../../src/runtime/mixins/mixin-descriptor.ts
...
function syncControlledReflection(node, props) {
  ...
  state.hasValue = node.type !== "progress" && "value" in element && props.value !== void 0;
  state.value = props.value;
  state.hasChecked = "checked" in element && props.checked !== void 0;
  state.checked = props.checked;
  state.version++;
}
function shouldTrackControlledReflection(props) {
  return props.value !== void 0 || props.checked !== void 0;
}
...
function bindNodeMixRuntime(node, frame, scheduler, reclaimed = false, parent) {
  let state = node._mixState;
  bindMixinRuntime(
    state,
    {
...
function diffHost(curr, next, frame, scheduler, styles, vParent, rootTarget) {
  ...
  if (next._mixState) {
    bindNodeMixRuntime(next, frame, scheduler);
  }
  if (shouldDispatchMixinLifecycle) {
...
function adoptHostElement(node, dom, hostProps, frame, scheduler, styles, rootTarget, cursor, parent) {
...
function insert(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor) {
  ...
  if (isHostNode(node)) {
    ...
    adoptHostElement(node, dom, hostProps, frame, scheduler, styles, rootTarget, void 0, domParent);
    doInsert(dom);
    return cursor;
  }
...
function resolveClientFrame(node, runtime) {
  ...
  Promise.resolve(runtime.resolveFrame(frameSrc, signal)).then(async (content) => {
  ...
  if (matchAnalysis.hasRemovals) {
    ...
    for (let oldIndex = 0; oldIndex < curr.length; oldIndex++) {
      if (usedOldIndexes[oldIndex] === 0) {
        remove2(curr[oldIndex], domParent, scheduler, styles);
      }
    }
  }
  vParent._children = next;
...
function reclaimPersistedMixinNode(persistedNode, newNode, frame, scheduler, styles, vParent, rootTarget) {
  ...
  if (dispatchMixinLifecycle) {
    dispatchMixinUpdateEvent(newNode._mixState, "beforeUpdate");
  }
  ...
  if (newNode._mixState) {
    bindNodeMixRuntime(newNode, frame, scheduler, true);
  }
  if (dispatchMixinLifecycle) {
    scheduler.enqueueCommitPhase([
      () => dispatchMixinUpdateEvent(newNode._mixState, "commit")
    ]);
  }
}
...
function createRoot(container, options = {}) {
  return createVirtualRoot(
    ...
    void 0,
    container.innerHTML.trim() !== "" ? container.firstChild : void 0
  );
}
function createVirtualRoot(parent, options, createParentVNode, anchor, initialHydrationCursor) {
  let vroot = null;
  ...
  while (currentIndex < curr.length || nextIndex < next.length) {
    ...
    if (!c && n) {
      ...
      if (cursor) {
        ...
        let currentEndIndex = isFrameStartMarker(c) && isFrameEndMarker(cursor) ? findFrameEndIndex(curr, currentIndex) : isVirtualRootStartMarker(c) && isVirtualRootEndMarker(cursor) ? curr.indexOf(findHydrationEndMarker(c)) : currentIndex;
...
function diffNode(current, next, context) {
  ...
  if (isElement(current) && isElement(next)) {
    ...
    if (current instanceof HTMLTextAreaElement && next instanceof HTMLTextAreaElement && current.value !== next.value) {
      return;
    }
  ...
  for (let i = 0; i < nextChildren.length; i++) {
    ...
    if (isFrameEndMarker(nextChild)) {
      ...
      if (key != null) {
        ...
        if (idx !== void 0 && !used[idx]) matchIndex = idx;
      }
    }
    if (matchIndex === -1) {
      if (i < currentChildren.length && !used[i] && nodeTypesComparable(currentChildren[i], nextChild)) {
        matchIndex = i;
      }
    }
...
function replaceCommentMarkerRange(replacement, parent, context) {
  ...
  let node = replacement.nextStart;
  while (node) {
...
function disposeRemovedRuntimeNodes(node, context) {
...
function mergeRmxDataFromRoot(into, root2) {
  for (let script of root2.querySelectorAll("script#rmx-data")) {
...
function cleanupFrameRuntimeNodes(nodes, context) {
  for (let i = 0; i < nodes.length; i++) {
    let node = nodes[i];
