...
function adoptHostNode(node, dom, hostProps, frame, scheduler, styles, rootTarget, cursor, parent) {
  ...
  if (node._mixState) {
    bindNodeMixRuntime(node, frame, scheduler, false, parent);
  }
}
...
function insert(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor) {
  ...
  if (isHostNode(node)) {
    ...
    if (isHeadHostNode(node)) {
      let targetHead = domParent instanceof Document ? domParent.head : domParent instanceof Node ? domParent.ownerDocument?.head ?? null : null;
      if (targetHead) {
        ...
        if (node._mixState) {
          bindNodeMixRuntime(node, frame, scheduler);
        }
        return cursor;
      }
    }
    ...
    adoptHostNode(node, dom, hostProps, frame, scheduler, styles, rootTarget, void 0, domParent);
    doInsert(dom);
    return cursor;
  }
