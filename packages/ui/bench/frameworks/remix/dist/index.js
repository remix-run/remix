...
// ../../../src/runtime/core/markers.ts
var FRAME_START_PREFIX = "rmx:f:";
var FRAME_END_TEXT = "/rmx:f";
var HYDRATION_START_PREFIX = "rmx:h:";
var HYDRATION_END_TEXT = "/rmx:h";
var COMMENT_NODE = 8;
function frameStartMarkerData(frameId) {
  return ` ${FRAME_START_PREFIX}${frameId} `;
}
var FRAME_END_MARKER_DATA = ` ${FRAME_END_TEXT} `;
var FRAME_END_MARKER_HTML = `<!--${FRAME_END_MARKER_DATA}-->`;
var HYDRATION_END_MARKER_HTML = `<!-- ${HYDRATION_END_TEXT} -->`;
...
function getHydrationMarkerId(marker) {
  let trimmed = marker.data.trim();
...
function parseHydrationMarkerId(node) {
  if (!isCommentNode(node)) return void 0;
  let trimmed = node.data.trim();
  if (!trimmed.startsWith(HYDRATION_START_PREFIX)) return void 0;
  return trimmed.slice(HYDRATION_START_PREFIX.length);
}
...
function findEndMarkerIndex(nodes, startIndex, isStart, isEnd) {
  let depth = 1;
...
function insertFrame(node, domParent, vParent, context, svg, anchor, cursor) {
  ...
  if (isFrameStartMarker(cursor?.current)) {
    ...
    let end2 = findFrameEndMarker(start2);
    if (end2) {
...
function getCommentMarkerRangeReplacement(current, next, currentNodes, nextNodes, currentIndex, nextIndex, context) {
  ...
  if (isHydrationStartMarker(current) && isHydrationStartMarker(next) && !shouldPreserveHydrationStartMarker(current, next, context)) {
    return {
      currentStart: current,
      nextStart: next,
...
function createFrame(root2, init) {
  ...
  async function renderContent(content, options) {
    ...
    if (isFullDocumentReload && htmlContent !== void 0) {
      ...
      await createSubFrames(bodyContainer.childNodes, responseContext2, options);
      if (isRenderAborted(options.signal)) return;
      displayedContentStatus = options.contentStatus ?? "resolved";
      return;
    }
