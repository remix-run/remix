...
function diffFrame(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor) {
  ...
  if (currSrc !== nextSrc) {
    ...
    if (frameInstance) {
      void frameInstance.reloadClientFrameForAncestorReload(frameRuntime.serverFrameReloadSignal).catch(() => {
      });
    }
  }
...
function createFrame(root2, init) {
  ...
  async function reloadClientFrameForAncestorReload(signal) {
  ...
  function startClientFrameReloadForAncestorReload(signal) {
    reloadController?.abort();
    let controller = new AbortController();
    reloadController = controller;
    if (signal) {
...
function createFrameRuntime(init) {
  return {
    ...
    serverFrameReloadSignal: void 0
  };
}
