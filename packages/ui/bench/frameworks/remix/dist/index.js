...
function readInstalledImportMap(doc, transportScripts) {
...
function getImportMapDelta(installedImportMap, importMap, baseUrl) {
  ...
  if (!imports && !scopes) return void 0;
  return {
...
function getImportMapImportsDelta(installedImports, imports, baseUrl, scope) {
  ...
  for (let [specifier, href] of Object.entries(imports)) {
    ...
    if (installedEntry) {
      ...
      console.warn(
        `[remix] Ignoring conflicting import map entry for "${specifier}"${scopeDescription}: ${formatImportMapAddress(installedEntry.href)} is already installed, but the new map points to ${formatImportMapAddress(href)}`
      );
      continue;
    }
...
function appendImportMapScript(doc, importMap) {
...
function createFrame(root2, init) {
  ...
  function consumeClientEntryResources(source) {
