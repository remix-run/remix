...
var REMIX_PRESERVE_DOM_ATTRIBUTE = "rmx-preserve-dom";
...
function diffElementChildren(current, next, context) {
  ...
  for (let i = 0; i < nextChildren.length; i++) {
    ...
    if (matchIndex === -1) {
      ...
      if (candidateIndex < currentChildren.length && !ownedByHydrationRange[candidateIndex] && !used[candidateIndex] && nodeTypesComparable(currentChildren[candidateIndex], nextChild)) {
  ...
  for (let i = 0; i < nextChildren.length; i++) {
    ...
    if (mi !== -1) {
      ...
      if (replacement) {
        ...
        for (let j = i + 1; j < replacement.nextEndIndex; j++) committed[j] = void 0;
...
function findNodesOwnedByHydrationRanges(nodes) {
  let owned = new Uint8Array(nodes.length);
  for (let startIndex = 0; startIndex < nodes.length; startIndex++) {
    let start = nodes[startIndex];
    let endIndex = isVirtualRootStartMarker(start) ? findHydrationEndIndex(nodes, startIndex) : startIndex;
...
function shouldPreserveHydrationStartMarker(current, next, context) {
  ...
  return currentData !== void 0 && nextData !== void 0 && currentData.moduleUrl === nextData.moduleUrl && currentData.exportName === nextData.exportName;
}
...
function hydrateRegion(vElement, start, end, context, signal) {
  if (signal?.aborted) return;
...
function isHydratedVirtualRootMarker(node) {
  return isCommentNode3(node) && "$rmx" in node;
}
