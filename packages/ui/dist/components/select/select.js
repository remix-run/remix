...
import { hiddenTypeahead } from "../../interactions/typeahead/typeahead-mixin.js";
import { onKeyDown } from "../../interactions/keydown/keydown.js";
...
function SelectProvider(handle) {
    ...
    function open() {
        if (state !== State.Closed || handle.props.disabled)
            return;
        state = State.Open;
        activeValue = value;
        handle.update();
    ...
    return () => {
        ...
        return (_jsx(listbox.Context, { flashSelection: true, ref: (nextListboxRef) => {
            ...
            }, value: value, activeValue: activeValue, onSelectSettled: settleSelectedOption, onSelect: selectOption, onHighlight: highlightOption, selectionFlashAttribute: "data-select-flash", children: handle.props.children }));
    };
}
...
export function Option(handle) {
    return () => {
        ...
        return (_jsxs("div", { ...divProps, mix: [listbox.optionStyle, select.option({ value, label, disabled, textValue }), mix], children: [_jsx(Glyph, { mix: listbox.glyphStyle, name: "check" }), _jsx("span", { mix: listbox.labelStyle, children: children ?? handle.props.label })] }));
    };
}
