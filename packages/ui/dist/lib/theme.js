...
let controlQuietToneUtility = css({
    ...
    backgroundImage: 'linear-gradient(to bottom, rgb(255 255 255 / 0.96) 0%, rgb(247 247 247 / 0.98) 100%)',
...
let buttonDefaultsMixin = createMixin((handle, hostType) => (props) => {
...
export function createTheme(values, options = {}) {
    ...
    function Theme() {
        return (props = {}) => createElement('style', {
            nonce: props.nonce,
            'data-rmx-theme': '',
...
function collectThemeVars(tree, values, path = []) {
    let vars = {};
    for (let [key, value] of Object.entries(tree)) {
        let nextPath = [...path, key];
        let themeValue = values[key];
        if (typeof value === 'string') {
        ...
        Object.assign(vars, collectThemeVars(value, themeValue, nextPath));
    }
    return vars;
}
function serializeThemeCss(selector, vars, options) {
...
function serializeThemeResetCss(selector) {
...
function createSinglePropertyUtilities(property, scale) {
    let utilities = {};
