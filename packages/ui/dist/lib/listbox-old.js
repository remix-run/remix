...
/* eslint-disable */
// @ts-nocheck
import { ref } from '@remix-run/component';
import { Glyph } from "./glyph.js";
import { anchor } from "./anchor.js";
import { hidePopover, isPopoverOpen, showPopover } from "./popover.js";
import { ui } from "./theme.js";
export let listboxChangeEventType = 'rmx:listbox-change';
export let listboxOpenChangeEventType = 'rmx:listbox-open-change';
...
function getMixedAttr(node, name) {
...
function isListboxOptionElement(node) {
    return isRemixElement(node) && node.type === ListboxOptionImpl;
}
function getListboxOptionElements(node, out = []) {
    ...
    getListboxOptionElements(node.props.children, out);
    return out;
}
function hasCustomListboxStructure(node) {
    if (Array.isArray(node)) {
    ...
    return hasCustomListboxStructure(node.props.children);
}
function getListboxOptionLabel(option) {
...
function replaceValueChildren(node, label) {
    if (Array.isArray(node)) {
...
function getParts(root) {
    ...
    return {
        ...
        valueNodes: valueNodes.filter((node) => node instanceof HTMLElement),
        items: items.filter((node) => node instanceof HTMLElement),
    };
}
function ListboxOptionImpl() {
    return (props) => {
        ...
        return (_jsxs("div", { ...domProps, mix: optionMix, children: [_jsx(Glyph, { mix: ui.listbox.itemIndicator, name: "check" }), _jsx("span", { mix: ui.listbox.itemLabel, children: children ?? textValue ?? value })] }));
    };
}
function ListboxImpl(handle) {
    let currentProps = null;
    let highlightedValue = null;
    let rootNode = null;
    let popupNode = null;
    let cleanupAnchor = () => { };
    let popupListenersAbort = null;
    ...
    function dispatchChange(value, itemValue) {
        rootNode?.dispatchEvent(new ListboxChangeEvent(value, itemValue));
    }
    function dispatchOpenChange(open) {
    ...
    function finishSelection(value) {
        let { popup } = getParts(rootNode);
        if (!popup) {
            if (currentProps?.value === undefined) {
    ...
    function flashSelection(node, callback) {
    ...
    function openPopup({ highlightLast = false } = {}) {
        let { popup } = getParts(rootNode);
        if (!popup) {
            return;
        }
    ...
    function syncValueNodes() {
        ...
        let firstItem = getItemRecords()[0] ?? null;
        for (let node of valueNodes) {
            ...
            if (firstItem) {
                node.textContent = firstItem.textValue;
            }
        }
    }
    function syncPopupNode() {
        ...
        let handleToggle = (event) => {
            ...
            if (!isNowOpen) {
                ...
                let activeInsideTrigger = activeElement
                    ? (trigger?.contains(activeElement) ?? false)
                    : false;
                let activeInsidePopup = activeElement ? (popup?.contains(activeElement) ?? false) : false;
                let shouldRestoreFocus = restoreTriggerFocusOnClose || !activeElement || activeInsideTrigger || activeInsidePopup;
    ...
    function attachRootListeners(node, signal) {
        let onDocumentPointerDown = (event) => {
            if (selectionActive) {
                return;
            }
        ...
        let onFocusOut = (event) => {
            ...
            let relatedTarget = event instanceof FocusEvent && event.relatedTarget instanceof HTMLElement
                ? event.relatedTarget
                : null;
            let { trigger, popup } = getParts(rootNode);
        ...
        let onPointerUp = (event) => {
            ...
            let shouldSelect = startedItemPointerDown || (pointerDownAge !== null && pointerDownAge >= 200);
            pointerDownTime = null;
            startedItemPointerDown = false;
            if (!shouldSelect) {
                return;
    ...
    return (props) => {
        ...
        let firstOption = optionElements[0];
        let usesCustomStructure = hasCustomListboxStructure(props.children);
        let currentItem = rootNode ? getCurrentItem() : null;
        let renderedLabel = currentValue !== null
            ? (currentItem?.textValue ??
                selectedOption?.props.textValue ??
                (selectedOption ? getListboxOptionLabel(selectedOption) : null) ??
                null)
            : (firstOption?.props.textValue ??
                (firstOption ? getListboxOptionLabel(firstOption) : null) ??
                null);
        let renderedChildren = usesCustomStructure
            ? replaceValueChildren(props.children, renderedLabel)
            : [
                _jsx("div", { mix: ui.listbox.anchor, children: _jsxs("button", { mix: ui.listbox.trigger, children: [_jsx("span", { mix: ui.listbox.value, children: renderedLabel }), _jsx(Glyph, { mix: ui.listbox.indicator, name: "chevronDown" })] }) }),
                _jsx("div", { mix: ui.listbox.popup, children: _jsx("div", { mix: ui.listbox.list, children: props.children }) }),
            ];
        handle.queueTask(() => {
            syncDom();
        });
        ...
        return (_jsxs("div", { ...domProps, mix: mix, children: [renderedChildren, props.name ? (_jsx("input", { disabled: props.disabled, name: props.name, type: "hidden", value: getValue() ?? '' })) : null] }));
    };
}
...
export let ListboxOption = ListboxOptionImpl;
//# sourceMappingURL=listbox-old.js.map