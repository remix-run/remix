...
export function lockScroll(targetDocument = globalThis.document) {
    ...
    if (!state) {
        ...
        let scrollbarWidth = documentElement.clientWidth > 0
...
export const lockScrollOnToggle = createMixin((handle) => {
    let unlockScroll = () => { };
    ...
    return () => [
        on('beforetoggle', (event) => {
            ...
            unlockScroll = lockScroll(event.currentTarget.ownerDocument);
        }),
    ];
});
