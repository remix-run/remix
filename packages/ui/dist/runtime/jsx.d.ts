...
type NestedMixValue<mix, depth extends number = 4> = depth extends 0 ? NullableMixValue<mix> | ReadonlyArray<NullableMixValue<mix>> : NullableMixValue<mix> | ReadonlyArray<NestedMixValue<mix, PreviousMixDepth[depth]>>;
type NormalizeMixProp<props> = props extends {
    mix?: infer mix;
} ? Omit<props, 'mix'> & {
    mix?: Array<NormalizeMixLeaf<mix>>;
} : props;
type ExpandMixProp<props> = props extends {
    mix?: infer mix;
} ? Omit<props, 'mix'> & {
    mix?: NestedMixValue<NormalizeMixLeaf<mix>>;
} : props;
type MixinElementType = ((handle: {
    update(): Promise<AbortSignal>;
}, setup: unknown) => (props: any) => RemixElement) & {
    __rmxMixinElementType: string;
};
type MergeComponentProps<handleProps, renderProps> = [handleProps] extends [Record<string, never>] ? renderProps : handleProps & renderProps;
/**
 ...
 */
export declare function jsx(type: ElementType, props: ElementProps, key?: string): RemixElement;
...
declare global {
    namespace JSX {
        ...
        type ElementType = keyof IntrinsicElements | ((handle: Handle<any, any>) => RenderFn<any>) | MixinElementType;
        ...
        type LibraryManagedAttributes<component, props> = component extends MixinElementType ? ExpandMixProp<Parameters<ReturnType<component>>[0]> : component extends (handle: Handle<infer P, any>) => RenderFn<infer R> ? ExpandMixProp<MergeComponentProps<P, R>> : component extends () => RenderFn<infer P> ? ExpandMixProp<P> : ExpandMixProp<props>;
        ...
        interface IntrinsicElements extends IntrinsicSVGElements, IntrinsicMathMLElements, IntrinsicHTMLElements {
        }
    }
}
