 ...
 */
type SerializableProperty<value> = value extends SerializablePrimitive | RemixNode ? value : value extends (...args: never[]) => unknown ? never : value extends abstract new (...args: never[]) => object ? never : value extends readonly unknown[] ? {
    [index in keyof value]: SerializableProperty<value[index]>;
} : value extends object ? keyof value extends never ? never : {
    [key in keyof value]: SerializableProperty<value[key]>;
} : never;
export type SerializableProps<props extends object = SerializableObject> = props extends SerializableObject ? props : {
...
] ? ((handle: Handle<props, context>) => RenderFn) & EntryMetadata : never;
/**
 ...
 * ```
 */
export declare function clientEntry<props extends object = {}, context = NoContext>(entryId: string, component: ((handle: Handle<props, context>) => RenderFn) & ([props] extends [SerializableProps<props>] ? unknown : never)): EntryComponent<props, context>;
/**
