...
import { setNavigationDriver } from './navigation-driver.js';
import { reloadNavigationFrame, } from './navigation-frame.js';
import { getNamedFrame, getTopFrame } from './run.js';
const historyStateKey = '__remixNavigation';
let nextEntryId = 0;
let supportsFormDataSubmitter;
export function startHistoryNavigationListener(signal) {
...
export function startHistoryNavigationListenerImpl(signal, options) {
    ...
    let imageSubmitCoordinates = new WeakMap();
    ...
    window.addEventListener('click', (event) => {
        ...
        if (!destination || !shouldHandleLink(event, link, destination))
            return;
    ...
    window.addEventListener('submit', (event) => {
        ...
        if (!shouldHandleForm(form, submitter))
            return;
        let method = getFormMethod(form, submitter);
        let destination = new URL(getFormAction(form, submitter), document.baseURI);
        if (!canRewriteHistory(destination))
            return;
        let imageCoordinates = submitter instanceof HTMLInputElement ? imageSubmitCoordinates.get(submitter) : undefined;
        if (submitter instanceof HTMLInputElement)
            imageSubmitCoordinates.delete(submitter);
        ...
        let defaultHistory = submission && destination.href === window.location.href ? 'replace' : 'push';
        let history = getHistoryAction(getNavigationAttribute(form, submitter, 'data-rmx-history'), defaultHistory);
        event.preventDefault();
    ...
    window.addEventListener('popstate', (event) => {
        ...
        currentEntry = entry;
        void startTransition(new URL(window.location.href), entry.navigation, 'traverse', undefined, entry).catch(() => { });
    }, { signal });
    ...
    async function startTransition(destination, state, action, submission, traversedEntry) {
        activeController?.abort();
        ...
        let pendingUserScrollStart;
        let pendingUserScrollFrame;
        ...
        let onScroll = () => {
            ...
            if (start && !scrollPositionsEqual(start, getScrollPosition()))
                userScrolled = true;
        };
        let onUserScroll = () => {
            pendingUserScrollStart = getScrollPosition();
            if (pendingUserScrollFrame !== undefined)
                cancelAnimationFrame(pendingUserScrollFrame);
        ...
        if (action === 'traverse') {
            if (!traversedEntry)
                throw new Error('Expected a traversed history entry');
            ...
            if (action === 'replace') {
                ...
                scrollPositions.set(currentEntry.id, currentEntry.scroll);
                window.history.pushState(createHistoryState(window.history.state, currentEntry), '', destination.href);
            }
        }
        try {
            let { frame, topFrame, redirectedTo } = await reloadNavigationFrame(destination.href, state, controller.signal, submission, options);
            ...
            if (!focusChanged)
                resetFocus(startingFocus);
            throwIfStale(controller, transition);
            if (!userScrolled)
                restoreScroll(destination, state, action, savedScroll);
            throwIfStale(controller, transition);
    ...
    function throwIfStale(controller, transition) {
        if (!controller.signal.aborted && transition === activeTransition)
            return;
    ...
    function cancelScheduledScrollSave() {
        if (scrollSaveTimer === undefined)
            return;
        clearTimeout(scrollSaveTimer);
        scrollSaveTimer = undefined;
    ...
    function persistCurrentScroll() {
        ...
        if (activeEntry?.id !== currentEntry.id)
            return;
        currentEntry = { ...currentEntry, scroll: getScrollPosition() };
        scrollPositions.set(currentEntry.id, currentEntry.scroll);
        replaceHistoryEntry(currentEntry);
    }
    function adoptActiveHistoryEntry() {
        let activeEntry = getHistoryNavigationEntry(window.history.state);
        if (activeEntry) {
            currentEntry = {
                ...activeEntry,
...
function createNavigationState(href, source) {
    return {
...
function createFormNavigationState(href, form, submitter) {
    return {
...
function shouldHandleLink(event, link, destination) {
    ...
    if (!link.hasAttribute('href'))
        return false;
    if (link.hasAttribute('data-rmx-document') || link.hasAttribute('download'))
        return false;
...
function shouldHandleForm(form, submitter) {
    if (hasNavigationAttribute(form, submitter, 'data-rmx-document'))
        return false;
    if (getFormMethod(form, submitter) === 'dialog')
        return false;
...
function getNavigationAttribute(form, submitter, name) {
    if (submitter?.hasAttribute(name))
        return submitter.getAttribute(name);
    return form.getAttribute(name);
}
function hasNavigationAttribute(form, submitter, name) {
    return submitter?.hasAttribute(name) === true || form.hasAttribute(name);
}
function createFormData(form, submitter, imageCoordinates) {
    if (!submitter)
...
function detectFormDataSubmitterSupport() {
...
function getFormDestination(action, formData) {
    let destination = new URL(action);
    let search = new URLSearchParams();
    for (let [name, value] of formData) {
        search.append(normalizeLineBreaks(name), normalizeLineBreaks(typeof value === 'string' ? value : value.name));
    }
...
function getHistoryAction(value, defaultAction) {
    if (value === 'replace')
        return 'replace';
    if (value === 'push')
        return 'push';
...
function getDefaultHistoryAction(destination) {
    return destination.href === window.location.href ? 'replace' : 'push';
}
function canRewriteHistory(destination) {
    let current = new URL(window.location.href);
    return (destination.protocol === current.protocol &&
...
function getHistoryNavigationEntry(value) {
    if (!isRecord(value))
        return;
...
function createHistoryState(value, entry) {
    return { ...(isRecord(value) ? value : {}), [historyStateKey]: entry };
}
function replaceHistoryEntry(entry, href) {
    if (href === undefined) {
        let activeEntry = getHistoryNavigationEntry(window.history.state);
        if (activeEntry && historyEntriesEqual(activeEntry, entry))
            return;
    }
...
function scrollPositionsEqual(left, right) {
    return left.x === right.x && left.y === right.y;
}
function restoreScroll(destination, state, action, savedPosition) {
...
function getFragmentTarget(hash) {
    ...
    return (document.getElementById(name) ??
        Array.from(document.getElementsByTagName('a')).find((anchor) => anchor.getAttribute('name') === name));
}
function resetFocus(startingFocus) {
