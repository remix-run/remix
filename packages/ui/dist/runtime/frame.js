...
import { findStreamRegion, installStreamTemplateMover } from "./stream-directives.js";
...
export function createFrame(root, init) {
    ...
    async function hydrateInitial() {
        ...
        if (currentMarker?.status === 'pending') {
            await watchPendingFrameTemplate(currentMarker, initialHydrationTracker, undefined, undefined, true);
        }
    ...
    async function watchPendingFrameTemplate(marker, initialHydrationTracker, signal, onResolved, fromInitialHydration = false) {
        if (signal?.aborted)
            return;
        ...
        let regionParent = container.regionParent ?? container.root;
        let observer = new MutationObserver(() => {
            if (signal?.aborted || pendingTemplateMarkerId !== marker.id)
                return;
            if (regionHasStreamDirectives(marker.id))
                return;
    ...
    function regionHasStreamDirectives(id) {
        return findStreamRegion(id, container.doc) != null;
    }
    async function resolveStreamedRegion(initialHydrationTracker, signal, onResolved) {
        clearPendingFrameTemplateWatch();
        if (signal?.aborted)
            return;
 ...
 */
export function installStreamResolution(doc = document) {
...
const COMPLETE_TEMPLATE_WITH_ID_PATTERN = /<template\b[^>]*\bfor=(?:"([^"]+)"|'([^']+)')[^>]*>[\s\S]*?<\/template>/gi;
