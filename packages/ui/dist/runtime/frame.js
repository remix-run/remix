...
export function createFrame(root, init) {
    ...
    async function render(content, options) {
        ...
        if (isFullDocumentReload && htmlContent !== undefined) {
            ...
            if (options?.signal?.aborted)
                return;
            scheduleHydrationInContainer(bodyContainer, context, options?.initialHydrationTracker, options?.signal);
        ...
        if (options?.signal?.aborted)
            return;
        scheduleHydrationInContainer(container, context, options?.initialHydrationTracker, options?.signal);
