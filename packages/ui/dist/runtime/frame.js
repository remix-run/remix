...
import { getFrameEndMarker, getFrameMarkerId, getHydrationEndMarker, isCommentNode, isFrameStartMarker, isHydrationEndMarker, isHydrationStartMarker, parseHydrationMarkerId, } from './core/markers.js';
...
function isHydrationMarkerLive(marker, context) {
    ...
    if (marker.start.parentNode !== marker.end.parentNode)
        return false;
    if (parseHydrationMarkerId(marker.start) !== marker.id)
        return false;
    if (!isHydrationEndMarker(marker.end))
        return false;
