...
import { bindMixinRuntime, cancelPendingMixinRemoval, dispatchMixinUpdateEvent, getMixinRuntimeSignal, prepareMixinRemoval, resolveMixedProps, teardownMixins, } from "./mixins/mixin.js";
...
function ensureControlledReflection(node, scheduler) {
    ...
    let state = {
        ...
        onInputOrChange: (event) => {
            ...
            if (event.type === 'input' && (state.hasChecked || node.type === 'select'))
                return;
    ...
    if (isHostNode(node)) {
        ...
        if (isHeadHostNode(node)) {
            ...
            if (targetHead) {
                ...
                adoptHostElement(node, targetHead, hostProps, frame, scheduler, styles, rootTarget, childCursor);
                return cursor;
            }
        }
        ...
        if (cursor instanceof Element) {
            ...
            if (cursorTag === node.type) {
                let nextCursor = cursor.nextSibling;
                adoptHostElement(node, cursor, hostProps, frame, scheduler, styles, rootTarget, cursor.firstChild);
                return nextCursor;
            }
            else {
                ...
                if (nextSibling instanceof Element) {
                    ...
                    if (nextTag === node.type) {
                        ...
                        adoptHostElement(node, nextSibling, hostProps, frame, scheduler, styles, rootTarget, nextSibling.firstChild);
                        return nextCursor;
                    }
                }
...
function resolveClientFrame(node, runtime) {
    ...
    Promise.resolve(runtime.resolveFrame(frameSrc, signal))
        .then(async (content) => {
        if (node._frameResolveToken !== token || signal.aborted)
            return;
        node._frameFallbackRoot?.dispose();
        node._frameFallbackRoot = undefined;
        await instance.render(content instanceof ReadableStream ? createAbortableReadableStream(content, signal) : content, { signal });
        if (node._frameResolveToken !== token || signal.aborted)
            return;
        node._frameResolved = true;
    })
