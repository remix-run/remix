import { createComponent, Fragment, Frame } from "./component.js";
import { createFrame } from "./frame.js";
import { createRangeRoot } from "./vdom.js";
import { isCommittedComponentNode, isComponentNode, isCommittedHostNode, isCommittedTextNode, isFragmentNode, isHostNode, isNonRenderNode, isTextNode, findContextFromAncestry, TEXT_NODE, NON_RENDER_NODE, } from "./vnode.js";
...
export function diffVNodes(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, rootCursor) {
    ...
    if (typeof type === 'function') {
        diffComponent(curr, next, frame, scheduler, styles, domParent, vParent, rootTarget, anchor, rootCursor);
        return rootCursor;
    }
...
function insert(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor) {
    ...
    if (isFragmentNode(node)) {
        ...
        for (let i = 0; i < children.length; i++) {
            cursor = insert(children[i], domParent, frame, scheduler, styles, node, rootTarget, anchor, cursor);
        }
        return cursor;
    }
...
function diffChildren(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, cursor, anchor) {
    ...
    if (curr === null) {
        ...
        for (let i = 0; i < next.length; i++) {
            cursor = insert(next[i], domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
        }
...
export function findNextSiblingDomAnchor(curr) {
    ...
    if (!vParent)
        return null;
    if (!Array.isArray(vParent._children))
        return vParent._rangeEnd ?? null;
    let children = vParent._children;
    if (children.length === 0)
        return vParent._rangeEnd ?? findNextSiblingDomAnchor(vParent);
