...
import { createNavigationSourceResolver } from './form-navigation.js';
const formSubmissionNavigationInfoType = 'frame-form-submission';
const frameRedirectNavigationInfoType = 'frame-redirect';
function resyncWebKitScrollAfterNavigation(event, resetScroll) {
...
function preserveStartingDocumentScrollState(navigation, event) {
...
function getRuntimeNavigation(navigation, event, resolveNavigationSource) {
    if (event.navigationType === 'traverse') {
...
function getSourceElementNavigation(navigation, event, resolveNavigationSource) {
