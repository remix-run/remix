...
import { getLinkNavigationElement, getReplaceHistory, interceptNavigation, } from './navigation-event.js';
    ...
    navigation.addEventListener('navigate', (event) => {
        ...
        let handler = async () => {
            ...
            if (frame.src !== requestedFrameSrc) {
                ...
                // Start the successor navigation without awaiting it: this handler must settle before the
                // replacement navigation can finish.
                navigation.navigate(redirectedSrc, {
                    history: 'replace',
                    state: redirectedState,
                    info: { type: frameRedirectNavigationInfoType },
                });
            }
...
function getSourceElementNavigation(event, resolveFormNavigation) {
    ...
    if (!formNavigation || formNavigation.hasAttribute('rmx-document'))
        return;
    ...
    return {
        ...
        replaceHistory: getReplaceHistory(formNavigation.getAttribute('rmx-history'), replaceHistoryByDefault),
