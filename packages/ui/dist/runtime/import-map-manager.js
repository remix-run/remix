...
function readInstalledImportMap(doc, transportScripts) {
    ...
    for (let script of doc.querySelectorAll(IMPORT_MAP_SELECTOR)) {
        if (transportScripts.has(script))
            continue;
        let importMap = parseImportMap(script.textContent ?? '');
        if (importMap)
            mergeInstalledImportMap(installedImportMap, importMap, baseUrl);
    }
...
function mergeInstalledImportMap(installedImportMap, importMap, baseUrl) {
    if (importMap.imports) {
        for (let [specifier, href] of Object.entries(importMap.imports)) {
            ...
            if (normalizedSpecifier === null)
                continue;
            if (installedImportMap.imports.has(normalizedSpecifier))
                continue;
    ...
    if (importMap.scopes) {
        for (let [scope, imports] of Object.entries(importMap.scopes)) {
            ...
            for (let [specifier, href] of Object.entries(imports)) {
                ...
                if (normalizedSpecifier === null)
                    continue;
                if (installedScopeImports.has(normalizedSpecifier))
                    continue;
