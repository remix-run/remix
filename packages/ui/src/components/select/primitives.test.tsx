...
const defaultLabelSwapDelayMs = 75
...
function TestSelect(
  ...
  return () => {
    let { children, defaultLabel, defaultValue, id, labelSwapDelayMs, mix, name, type } =
      handle.props
    ...
    return (
      <select.Context
        ...
        labelSwapDelayMs={labelSwapDelayMs}
        name={name}
      >
        <button id={id} type={type ?? 'button'} mix={[select.trigger(), mix]}>
          <SelectLabel />
        </button>
        <popover.Context>
          <div mix={select.popover()}>
            <div mix={select.list()}>{children}</div>
          </div>
        </popover.Context>
...
function TestOption(
  handle: Handle<{
    ...
    disabled?: boolean
    label: string
    textValue?: select.SelectOptionProps['textValue']
    value: string
  }>,
): () => RemixNode {
  return () => {
...
function renderSelect(props: RenderSelectOptions = {}) {
  ...
  return (
    <TestSelect defaultLabel={defaultLabel} defaultValue={defaultValue} name={name}>
      <TestOption label="Remix framework" value="remix">
        Remix
      </TestOption>
      <TestOption disabled label="React Router framework" value="react-router">
        React Router
      </TestOption>
      <TestOption label="React framework" value="react">
        React
      </TestOption>
    </TestSelect>
  )
...
async function finishSelectUpdate(
  surface: HTMLElement,
  root: ReturnType<typeof createRoot>,
  labelSwapDelayMs = defaultLabelSwapDelayMs,
) {
  await timers.advanceAsync(flashDurationMs)
  await finishCloseTransition(surface)
  await settle(root)
  await timers.advanceAsync(labelSwapDelayMs)
  await settle(root)
}
...
describe('Select', () => {
  ...
  it('supports a custom label swap delay on the primitive context', async (t) => {
    let { container, root } = renderApp(
      <TestSelect
        defaultLabel="Select a framework"
        defaultValue={unsetDefaultValue}
    ...
    await timers.advanceAsync(99)
    await settle(root)

    expect(trigger.textContent).toContain('Select a framework')

    await timers.advanceAsync(1)
    await settle(root)
      ...
      return () => (
        <popover.Context>
          ...
          >
            ...
            >
              <TestOption label="No grouping" value="none" />
              <TestOption label="Status" value="status" />
              <TestOption label="Priority" value="priority" />
            </TestSelect>
          </div>
        </popover.Context>
      )
  ...
  it('typeahead on the closed trigger matches option textValue values', async () => {
    let { container, root } = renderApp(
      <TestSelect defaultLabel="Select an environment" defaultValue={unsetDefaultValue}>
        <TestOption label="Production environment" value="production">
          Production
        </TestOption>
        <TestOption label="Staging environment" textValue="beta" value="staging">
          Staging
        </TestOption>
        <TestOption label="Local environment" textValue="dev" value="local">
          Local
        </TestOption>
      </TestSelect>,
    )
  ...
  it('ArrowDown and ArrowUp reopen from the selected value', async () => {
    let { container, root } = renderApp(
      <TestSelect defaultLabel="Select an environment" defaultValue="staging">
        <TestOption label="Local" value="local" />
        <TestOption label="Staging" value="staging" />
        <TestOption label="Production" value="production" />
      </TestSelect>,
    )
    let trigger = getTrigger(container)
  ...
  it('participates in formdata with hidden input', async (t) => {
    let { container, root } = renderApp(
      <form>
        <TestSelect defaultLabel="Select a framework" name="framework" type="button">
          <TestOption label="Remix framework" value="remix">
            Remix
          </TestOption>
          <TestOption disabled label="React Router framework" value="react-router">
