...
const baseStyle: CSSMixinDescriptor = css({
  ...
  '&:focus-visible': {
    outline: '2px solid rgba(53, 115, 246, 0.35)',
    outlineOffset: '2px',
  },
...
export const hiddenInputStyle: CSSMixinDescriptor = css({
  position: 'absolute',
  width: '1px',
  height: '1px',
  margin: '-1px',
  ...
  return () => {
    ...
    return (
      <checkboxPrimitive.Context
        ...
        value={value}
      >
        <span {...spanProps} mix={[checkbox({ size }), checkboxPrimitive.control(), mix]} />
        <checkboxPrimitive.UncheckedInput />
        <input mix={[checkbox({ size }), hiddenInputStyle, checkboxPrimitive.hiddenInput(), inputMix]} />
      </checkboxPrimitive.Context>
    )
  }
}
...
export function CheckboxGroup(handle: Handle<CheckboxGroupProps>): () => RemixNode {
  return () => {
    let { children, defaultValue, disabled = false, mix, name, onValueChange, value, ...divProps } =
      handle.props
...
export function CheckboxGroupParent(handle: Handle<CheckboxGroupParentProps>): () => RemixNode {
  return () => {
    ...
    return (
      <checkboxPrimitive.ParentContext
        disabled={disabled}
        form={form}
        inputRef={inputRef}
        readOnly={readOnly}
