...
Pass `nonce` when rendering per-request HTML under a [Content Security Policy](#content-security-policy). It applies to import maps and CSS mixin styles. Render errors reject the returned promise.
...
- **`onError(error)`** - Receives rendering errors and defaults to logging them. Fatal errors reject the stream even when a handler is supplied.
