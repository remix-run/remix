...
export function createTransitionManager(init: TransitionManagerInit) {
  ...
  async function handleActionFetchSubmission(
    ...
    let yeetedKeys = yeetStaleFetchers(loadId);
    if (yeetedKeys) {
  ...
  function yeetStaleFetchers(landedId: number): false | string[] {
    let yeetedKeys = [];
  ...
  async function loadPageData(
    ...
    let yeets = yeetStaleFetchers(navigationLoadId);
    if (yeets) {
      markFetchersDone(yeets);
    }
    ...
    update({
      ...
      transition: IDLE_TRANSITION,
      fetchers: yeets ? new Map(state.fetchers) : state.fetchers
    });
  }
