    ...
    if (context.hydrationData.size > 0) {
        for (let [, { css, js }] of context.hydrationData) {
            ...
            for (let props of css) {
                if (context.seenLinks.has(props.href))
                    continue;
                context.seenLinks.add(props.href);
                context.headElements.push(`<link ${renderAttributes({ rel: 'stylesheet', ...props }, false)} />`);
            }
            for (let { src, ...props } of js) {
                if (context.seenLinks.has(src))
                    continue;
                context.seenLinks.add(src);
                context.headElements.push(`<link ${renderAttributes({ rel: 'modulepreload', href: src, ...props }, false)} />`);
            }
        }
