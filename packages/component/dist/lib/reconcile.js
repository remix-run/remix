...
import { diffHostProps, cleanupCssProps } from "./diff-props.js";
import { skipComments, logHydrationMismatch } from "./hydration.js";
import { toVNode } from "./to-vnode.js";
import { findMatchingExitingNode, getPresenceConfig, markNodeExiting, playEnterAnimation, playExitAnimation, shouldPlayEnterAnimation, unmarkNodeExiting, } from "./presence.js";
import { registerLayoutElement, unregisterLayoutElement, updateLayoutElement, } from "./layout-animation.js";
...
export function diffVNodes(curr, next, domParent, frame, scheduler, vParent, rootTarget, anchor, rootCursor) {
    ...
    if (isFragmentNode(curr) && isFragmentNode(next)) {
        diffChildren(curr._children, next._children, domParent, frame, scheduler, vParent, rootTarget, undefined, anchor);
        return rootCursor;
    }
...
function insert(node, domParent, frame, scheduler, vParent, rootTarget, anchor, cursor) {
        ...
        if (cursor instanceof Element) {
            ...
            if (cursorTag === node.type) {
                ...
                if (node.props.innerHTML != null) {
                    ...
                    diffChildren(null, node._children, cursor, frame, scheduler, node, rootTarget, childCursor);
                }
                ...
                if (nextSibling instanceof Element) {
                    ...
                    if (nextTag === node.type) {
                        ...
                        if (node.props.innerHTML != null) {
                            ...
                            diffChildren(null, node._children, nextSibling, frame, scheduler, node, rootTarget, childCursor);
                        }
    ...
    if (isComponentNode(node)) {
        return diffComponent(null, node, frame, scheduler, domParent, vParent, rootTarget, anchor, cursor);
    }
    if (node.type === Frame) {
...
export function renderComponent(handle, currContent, next, domParent, frame, scheduler, rootTarget, vParent, anchor, cursor) {
    let [element, tasks] = handle.render(next.props);
    let content = toVNode(element);
    let newCursor = diffVNodes(currContent, content, domParent, frame, scheduler, next, rootTarget, anchor, cursor);
...
function diffComponent(curr, next, frame, scheduler, domParent, vParent, rootTarget, anchor, cursor) {
    if (curr === null) {
        ...
        return renderComponent(next._handle, null, next, domParent, frame, scheduler, rootTarget, vParent, anchor, cursor);
    }
    next._handle = curr._handle;
    let { _content, _handle } = curr;
    return renderComponent(_handle, _content, next, domParent, frame, scheduler, rootTarget, vParent, anchor, cursor);
}
...
export function remove(node, domParent, scheduler) {
    if (isCommittedTextNode(node)) {
