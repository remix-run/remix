...
let animatingNodes = new WeakMap();
let initialEntranceSeenByParent = new WeakMap();
...
let animateEntranceMixin = createMixin((handle) => {
...
let animateExitMixin = createMixin((handle) => {
    let currentConfig = true;
    let node = null;
    handle.addEventListener('insert', (event) => {
        node = event.node;
