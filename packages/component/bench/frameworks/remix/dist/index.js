...
async function hoistHeadElements(doc, fragment) {
  let target = doc.head;
  if (!target) return;
...
function loadStyle(link) {
  let el2 = document.querySelector(`link[href="${link.href}"][rel="stylesheet"]`);
  let entry = performance.getEntriesByType("resource").find(({ name: name2 }) => name2 === link.href);
  if (entry && el2?.sheet?.href === link.href) return;
  return new Promise((resolve) => {
    let done = () => resolve();
    console.log({
      entry,
      sheetHref: el2?.sheet?.href,
      el: el2
    });
    if (!el2) {
      el2 = document.createElement("link");
      el2.rel = "stylesheet";
      for (let [name2, value] of Object.entries(el2)) {
      ...
      el2.addEventListener("load", done);
