...
describe('run', () => {
  ...
  it('updates hoisted modulepreloads after hydrating a client entry', async () => {
    let TogglePreload = clientEntry(
      '/js/toggle-preload.js#TogglePreload',
      function TogglePreload(handle: Handle) {
        let phase = 0
        return () => (
          <>
            <link rel="modulepreload" href={phase === 0 ? '/scripts/a.js' : '/scripts/b.js'} />
            <button
              id="toggle-preload"
              mix={[
                on('click', () => {
  ...
  it('replaces stale hoisted modulepreloads when a frame reloads', async () => {
    let renderCount = 0
    let reload: undefined | (() => Promise<AbortSignal>)

    let ReloadButton = clientEntry(
      '/assets/reload-modulepreloads.js#ReloadModulepreloads',
      function ReloadModulepreloads(handle: Handle) {
        reload = () => handle.frame.reload()
        return () => <button id="reload-modulepreloads">Reload modulepreloads</button>
      },
    )

    async function renderModulepreloadFragment() {
      renderCount++
      ...
      return await drain(
        renderToStream(
          <>
            <link
              rel="modulepreload"
              href={renderCount === 1 ? '/scripts/shared-a.js' : '/scripts/shared-b.js'}
            />
            <ReloadButton />
          </>,
          {
            async resolveClientEntryResourceHints() {
              return [
                {
                  rel: 'modulepreload',
                  href: renderCount === 1 ? '/scripts/client-a.js' : '/scripts/client-b.js',
                },
                {
                  rel: 'modulepreload',
                  href: renderCount === 1 ? '/scripts/shared-a.js' : '/scripts/shared-b.js',
    ...
    let html = await drain(
      renderToStream(
        <main>
          <Frame src="/reload-modulepreloads" fallback={<div>Loading…</div>} />
        </main>,
        { resolveFrame: renderModulepreloadFragment },
      ),
    )
    document.body.innerHTML = html
  ...
  it('hoists and dedupes modulepreloads from streamed frame content into head', async () => {
    let Counter = clientEntry('/js/counter.js#Counter', function Counter() {
      return () => <button id="counter">Counter</button>
    })
    ...
    invariant(!first.done)
    document.body.innerHTML = first.value
    document.head.innerHTML = '<link rel="modulepreload" href="/scripts/shared.js">'
