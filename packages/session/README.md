...
- **Web Standards:** Built directly on top of the [web `fetch` API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) primitives for maximum portability and easy testing
...
// Note: A session cookie *must* be signed for security
...
Note: The `createRequest` helper above is useful in testing and docs to simulate how a browser would normally persist the session cookie across requests, but the requests in a normal flow would be generated by a user making requests to your server. You can see how we wrote this helper [in our tests](./test/helpers.ts).
