...
describe('queries', () => {
    it('supports direct construction and createDatabase wrapper', async () => {
        ...
        let directRows = await direct.exec(query(accounts).all());
        let wrappedRows = await wrapped.exec(query(accounts).all());
    ...
    it('executes unbound Query objects through db.exec() in every execution mode', async () => {
        let db = createTestDatabase(createAdapter({
            accounts: [
        ...
        let insertResult = await db.exec(query(accounts).insert({ id: 3, email: 'casey@studio.test', status: 'inactive' }));
        ...
        let updateResult = await db.exec(query(accounts).where({ status: 'inactive' }).update({ status: 'active' }));
        let deleteResult = await db.exec(query(accounts).where({ id: 4 }).delete());
        let upsertResult = await db.exec(query(accounts).upsert({ id: 6, email: 'fran@studio.test', status: 'active' }, { conflictTarget: ['id'] }));
        ...
        let reloaded = await db.exec(query(accounts).orderBy('id', 'asc').all());
    ...
    it('is immutable and supports eager hasMany loading', async () => {
        ...
        let allAccountsQuery = query(accounts);
        let activeAccountsQuery = allAccountsQuery.where({ status: 'active' });
        let allAccounts = await db.exec(allAccountsQuery.all());
        let activeAccounts = await db.exec(activeAccountsQuery
    ...
    it('supports hasManyThrough loading', async () => {
        ...
        let accountRows = await db.exec(query(accounts).orderBy('id', 'asc').with({ tasks: openTasks }).all());
    ...
    it('applies hasMany relation limit/offset per parent row', async () => {
        ...
        let db = createTestDatabase(adapter);
        let firstProjectPerAccount = await db.exec(query(accounts).orderBy('id', 'asc')
            .with({
        ...
        let secondProjectPerAccount = await db.exec(query(accounts).orderBy('id', 'asc')
            .with({
    ...
    it('applies hasManyThrough relation pagination per parent row', async () => {
        ...
        let db = createTestDatabase(adapter);
        let firstTaskPerAccount = await db.exec(query(accounts).orderBy('id', 'asc')
            .with({
        ...
        let secondTaskPerAccount = await db.exec(query(accounts).orderBy('id', 'asc')
            .with({
    ...
    it('applies hasManyThrough through-relation pagination per parent row', async () => {
        ...
        let accountRows = await db.exec(query(accounts).orderBy('id', 'asc')
    ...
    it('supports composite primary keys in find()', async () => {
        ...
        let db = createTestDatabase(adapter);
        let membership = await db.exec(query(memberships).find({ organization_id: 9, account_id: 2 }));
        assert.ok(membership);
        assert.equal(membership.role, 'member');
    });
    it('supports query-based reads with relations and counts', async () => {
        let adapter = createAdapter({
            accounts: [
                { id: 1, email: 'amy@studio.test', status: 'active' },
                { id: 2, email: 'brad@studio.test', status: 'inactive' },
        ...
        let account = await db.exec(query(accounts).find(1));
        let activeAccount = await db.exec(query(accounts).where({ status: 'active' }).orderBy('id', 'asc').first());
        let activeAccounts = await db.exec(query(accounts).where({ status: 'active' })
            .orderBy('status', 'asc')
            .orderBy('id', 'asc')
            .limit(1)
            .all());
        let activeCount = await db.exec(query(accounts).where({ status: 'active' }).count());
        let accountsWithProjects = await db.exec(query(accounts).orderBy('id', 'asc').with({ projects: openProjects }).all());
    ...
    it('supports query-based updates and loading related rows', async () => {
        let adapter = createAdapter({
            accounts: [
                { id: 1, email: 'amy@studio.test', status: 'active' },
                { id: 2, email: 'brad@studio.test', status: 'inactive' },
        ...
        let updated = await db.exec(query(accounts).with({ projects: openProjects }).find(1));
        assert.equal(updated?.status, 'inactive');
        assert.equal(updated?.projects.length, 1);
        assert.equal(updated?.projects[0].id, 100);
    });
    it('returns update rows directly when adapters support RETURNING', async () => {
        ...
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(accounts).where({ id: 1 }).update({ status: 'inactive' }, { returning: '*' }));
        let updated = 'rows' in result ? result.rows[0] : null;
        assert.equal(updated?.id, 1);
        assert.deepEqual(operationKinds, ['update']);
    });
    it('supports scoped update and delete queries', async () => {
        let adapter = createAdapter({
            accounts: [
        ...
        let db = createTestDatabase(adapter);
        let updateResult = await db.exec(query(accounts).where({ status: 'inactive' })
            .orderBy('id', 'asc')
            .limit(1)
            .update({ status: 'archived' }));
        let deleteResult = await db.exec(query(accounts).where({ status: 'inactive' })
            .orderBy('id', 'asc')
            .limit(1)
            .delete());
        ...
        assert.equal(deleteResult.affectedRows, 1);
        assert.deepEqual(rows.map((row) => ({ id: row.id, status: row.status })), [
            { id: 1, status: 'archived' },
            { id: 3, status: 'active' },
        ]);
    });
    it('supports scoped delete queries with composite primary keys', async () => {
        let adapter = createAdapter({
            accounts: [],
            projects: [],
            tasks: [],
        ...
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(memberships).where({ organization_id: 9 }).orderBy('account_id', 'asc').limit(1).delete());
        let rows = await db.exec(query(memberships).orderBy('organization_id', 'asc').all());
    ...
    it('supports insert and insertMany metadata and returning rows', async () => {
        let adapter = createAdapter({
            accounts: [{ id: 1, email: 'existing@studio.test', status: 'active' }],
            projects: [{ id: 100, account_id: 3, name: 'Onboarding', archived: false }],
            tasks: [],
            memberships: [],
        });
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(accounts).insert({
        ...
        let created = await db.exec(query(accounts).with({ projects: accountProjects.orderBy('id', 'asc') }).find(3));
        let bulkResult = await db.exec(query(accounts).insertMany([
            { id: 4, email: 'a@studio.test', status: 'active' },
            { id: 5, email: 'b@studio.test', status: 'inactive' },
        ]));
        let rowsResult = await db.exec(query(accounts).insertMany([{ id: 6, email: 'c@studio.test', status: 'active' }], { returning: '*' }));
        let rows = 'rows' in rowsResult ? rowsResult.rows : [];
        assert.equal(result.affectedRows, 1);
    ...
    it('supports insertMany() batches that include at least one explicit value', async () => {
        ...
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(tasks).insertMany([{}, { title: 'hello world' }]));
    ...
    it('throws for createMany({ returnRows: true }) when adapter has no RETURNING support', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async function () {
            await db.exec(query(accounts).insertMany([{ id: 1, email: 'a@studio.test', status: 'active' }], { returning: '*' }));
        }, function (error: unknown) {
            return (error instanceof DataTableQueryError &&
    ...
    it('supports join/groupBy/having with count()', async () => {
        ...
        let db = createTestDatabase(adapter);
        let count = await db.exec(query(accounts).join(projects, eq('archived', false))
            .groupBy('status')
    ...
    it('supports count() and exists()', async () => {
        ...
        let db = createTestDatabase(adapter);
        let activeCount = await db.exec(query(accounts).where({ status: 'active' }).count());
        let hasInactive = await db.exec(query(accounts).where({ status: 'inactive' }).exists());
        let hasArchived = await db.exec(query(accounts).where({ status: 'archived' }).exists());
        assert.equal(activeCount, 1);
    ...
    it('supports eager loading for hasOne and belongsTo', async () => {
        ...
        let db = createTestDatabase(adapter);
        let accountRows = await db.exec(query(accounts).with({ profile: accountProfile }).all());
        let projectRows = await db.exec(query(projects).with({ account: projectAccount }).all());
    ...
    it('supports cross schema query', async () => {
        ...
        let db = createTestDatabase(adapter);
        let invoiceRows = await db.exec(query(invoices).join(accounts, eq(accounts.id, invoices.account_id))
            .select({
...
describe('writes and validation', () => {
    it('validates values and applies timestamps', async () => {
        ...
        let insertResult = await db.exec(query(accounts).insert({
            id: 10,
        ...
        let savedAccount = await db.exec(query(accounts).find(10));
    ...
    it('passes create/update operation context to table validators', async () => {
        ...
        let db = createTestDatabase(adapter);
        await db.exec(query(validatedAccounts).insert({ id: 2, email: 'b@studio.test', status: 'active' }));
        await db.exec(query(validatedAccounts).insertMany([{ id: 3, email: 'c@studio.test', status: 'active' }]));
        await db.exec(query(validatedAccounts).where({ id: 1 }).update({ status: 'inactive' }));
        await db.exec(query(validatedAccounts).where({ id: 2 }).update({ status: 'active' }));
    ...
    it('uses create operation for both upsert payloads', async () => {
        ...
        let db = createDatabase(adapter);
        await db.exec(query(validatedAccounts).upsert({ id: 1, email: 'a@studio.test', status: 'inactive' }, { conflictTarget: ['id'], update: { status: 'active' } }));
    ...
    it('rejects unknown columns before and after table validation', async () => {
        ...
        await assert.rejects(async () => {
            await db.exec(query(strictAccounts).insert({ id: 1, email: 'a@studio.test', status: 'active' }));
        }, (error: unknown) => error instanceof DataTableValidationError &&
            error.message === 'Unknown column "ghost" for table "accounts"');
        await assert.rejects(async () => {
            await db.exec(query(strictAccounts).insert({
                id: 1,
                email: 'a@studio.test',
                status: 'active',
    ...
    it('runs beforeWrite -> validate -> touch -> afterWrite for create', async () => {
        ...
        let db = createTestDatabase(adapter);
        await db.exec(query(writeTrackedAccounts).insert({
            id: 1,
            email: 'ops@studio.test',
            status: 'active',
        }));
        let saved = await db.exec(query(writeTrackedAccounts).find(1));
        assert.ok(saved);
    ...
    it('passes scoped delete context to callbacks and reports affected rows', async () => {
        ...
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(deleteTrackedAccounts).where({ status: 'active' })
            .orderBy('id', 'asc')
            .limit(1)
            .delete());
        assert.equal(result.affectedRows, 1);
    ...
    it('allows beforeDelete to veto deletes with issues', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async () => {
            await db.exec(query(vetoedAccounts).where({ id: 1 }).delete());
        }, (error: unknown) => error instanceof DataTableValidationError &&
    ...
    it('includes callback source metadata for afterRead issues', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async () => {
            await db.exec(query(issueAfterReadAccounts).find(1));
        }, (error: unknown) => error instanceof DataTableValidationError &&
            error.message === 'Invalid value for table "accounts"' &&
    ...
    it('passes projected row shapes to afterRead callbacks', async () => {
        ...
        let db = createTestDatabase(adapter);
        let rows = await db.exec(query(projectedAccounts).select({ id: projectedAccounts.id }).all());
        assert.equal(rows.length, 1);
    ...
    it('applies afterRead to root rows, related rows, and write-returning rows', async () => {
        ...
        let db = createTestDatabase(adapter);
        let rows = await db.exec(query(readableAccounts).with({ projects: readableAccountProjects }).all());
        assert.equal(rows[0].email, 'AMY@STUDIO.TEST');
        assert.equal(rows[0].projects[0].name, 'Spring Campaign!');
        let insertResult = await db.exec(query(readableAccounts).insert({ id: 2, email: 'new@studio.test', status: 'active' }, { returning: '*' }));
    ...
    it('enforces synchronous lifecycle callbacks', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async () => {
            await db.exec(query(asyncBeforeWriteAccounts).insert({ id: 2, email: 'new@studio.test' }));
        }, (error: unknown) => error instanceof DataTableValidationError &&
            error.message === 'Invalid beforeWrite callback result for table "accounts"' &&
            error.metadata?.source === 'beforeWrite');
        await assert.rejects(async () => {
            await db.exec(query(asyncAfterReadAccounts).find(1));
        }, (error: unknown) => error instanceof DataTableValidationError &&
    ...
    it('throws for invalid beforeDelete callback return values', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async () => {
            await db.exec(query(invalidBeforeDeleteAccounts).where({ id: 1 }).delete());
        }, (error: unknown) => error instanceof DataTableValidationError &&
    ...
    it('throws for update returning when adapter has no RETURNING support', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async () => {
            await db.exec(query(accounts).where({ status: 'active' })
                .orderBy('id', 'asc')
                .limit(1)
                .update({ status: 'inactive' }, { returning: ['id', 'status'] }));
        }, (error: unknown) => error instanceof DataTableQueryError &&
    ...
    it('supports insertMany() and delete() returning with RETURNING adapters', async () => {
        ...
        let db = createTestDatabase(adapter);
        let inserted = await db.exec(query(accounts).insertMany([
            { id: 1, email: 'a@studio.test', status: 'active' },
            { id: 2, email: 'b@studio.test', status: 'active' },
        ], { returning: ['id', 'email'] }));
        assert.ok('rows' in inserted);
        let deleted = await db.exec(query(accounts).where({ id: 2 })
            .delete({ returning: ['id'] }));
    ...
    it('supports upsert() and conflictTarget', async () => {
        ...
        let db = createTestDatabase(adapter);
        let result = await db.exec(query(accounts).upsert({ id: 1, email: 'a@studio.test', status: 'inactive' }, { conflictTarget: ['id'], returning: ['id', 'status'] }));
    ...
    it('throws for upsert() when adapter does not support it', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async function () {
            await db.exec(query(accounts).upsert({ id: 1, email: 'a@studio.test', status: 'active' }, { conflictTarget: ['id'] }));
        }, function (error: unknown) {
            return error instanceof DataTableQueryError;
        });
    });
    it('throws when read-only query modifiers are used with write terminals', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async function () {
            await db.exec(query(accounts).join(projects, eq('accounts.id', 'projects.account_id'))
        ...
        await assert.rejects(async function () {
            await db.exec(query(accounts).groupBy('status').delete());
        }, function (error: unknown) {
            return (error instanceof DataTableQueryError &&
                error.message.includes('delete() does not support these query modifiers: groupBy()'));
        });
        await assert.rejects(async function () {
            await db.exec(query(accounts).with({ projects: accountProjects })
    ...
    it('throws when scoped query modifiers are used with insert-like terminals', async () => {
        ...
        let db = createTestDatabase(adapter);
        await assert.rejects(async function () {
            await db.exec(query(accounts).where({ id: 1 }).insert({
                id: 2,
                email: 'b@studio.test',
                status: 'active',
            }));
        ...
        await assert.rejects(async function () {
            await db.exec(query(accounts).orderBy('id', 'asc')
        ...
        await assert.rejects(async function () {
            await db.exec(query(accounts).limit(1).upsert({
                id: 1,
                email: 'a@studio.test',
                status: 'inactive',
            }));
    ...
    it('does not validate filter values at runtime', async () => {
        ...
        await db.exec(query(accounts).where(invalidWhere)
            .all());
        await db.exec(query(accounts).join(projects, eq('projects.archived', invalidArchived))
            .all());
        await db.exec(query(accounts).groupBy('status')
...
describe('transactions and raw sql', () => {
    it('supports nested transactions using savepoints', async () => {
        ...
        await db.transaction(async (outerTransaction) => {
            await outerTransaction.exec(query(accounts).insert({ id: 2, email: 'pm@studio.test', status: 'active' }));
            await outerTransaction
                .transaction(async (innerTransaction) => {
                await innerTransaction.exec(query(accounts).insert({ id: 3, email: 'design@studio.test', status: 'active' }));
    ...
    it('treats transaction options as best-effort adapter hints', async () => {
        ...
        await db.transaction(async (transactionDatabase) => {
            await transactionDatabase.exec(query(accounts).insert({ id: 2, email: 'pm@studio.test', status: 'active' }));
        }, {
    ...
    it('rolls back outer transactions on errors', async () => {
        ...
        await db
            ...
            .catch(() => undefined);
        let rows = await db.exec(query(accounts).orderBy('id', 'asc').all());
        assert.deepEqual(rows.map((row) => ({ id: row.id, email: row.email, status: row.status })), [{ id: 1, email: 'founder@studio.test', status: 'active' }]);
    });
...
describe('adapter errors', () => {
    it('wraps adapter failures in DataTableAdapterError', async () => {
        ...
        await assert.rejects(async function () {
            ...
            return (error.metadata?.dialect === 'failing' &&
