...
import { executeOperation, runInTransaction, } from './execution-context.js';
...
function assertWhereIsNotStructurallyUnconditional(where, operation) {
    if (where.length > 0 && where.every(isStructurallyUnconditional)) {
        throw new DataTableQueryError(operation + '() does not allow a structurally unconditional where clause');
    }
}
function isStructurallyUnconditional(predicate) {
