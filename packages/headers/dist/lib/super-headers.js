...
const ApplyAppendHeaderNames = new Set([
    ...ObjectHeaderDescriptors.filter((descriptor) => ApplyAppendObjectHeaderProperties.has(descriptor.property)),
...
export class SuperHeaders extends Headers {
    ...
    #shouldSetHeaderDescriptorValue(descriptor) {
        let key = descriptor.name.toLowerCase();
        return (key !== SetCookieKey && key !== 'cookie' && key !== 'vary' && !ApplyAppendHeaderNames.has(key));
    }
