...
const TEST_REPEATABLE_FLAGS = new Set(testCommandFlags.filter((flag) => flag.multiple).map((flag) => flag.name));
const TEST_FILE_VALUE_FLAGS = new Set(testCommandFlags.filter((flag) => flag.files).map((flag) => flag.name));
const TEST_FLAG_ALIASES = new Map(testCommandFlags.flatMap((flag) => flag.alias === undefined ? [] : [[flag.alias, flag.name]]));
...
function completeTest(tokens, currentWord, usedGlobalFlags) {
    ...
    for (let token of filteredTokens) {
        ...
        if (token.startsWith('-') && token !== '-') {
            ...
            if (group.every((flag) => flag !== undefined && TEST_BOOLEAN_FLAG_SET.has(flag))) {
    ...
    let longFlags = [...TEST_BOOLEAN_FLAGS, ...TEST_VALUE_FLAGS].filter((flag) => !usedFlags.has(flag) || TEST_REPEATABLE_FLAGS.has(flag));
    let shortFlags = [...TEST_FLAG_ALIASES].flatMap(([short, long]) => !usedFlags.has(long) || TEST_REPEATABLE_FLAGS.has(long) ? [short] : []);
...
function completeDb(tokens, currentWord, usedGlobalFlags) {
    let filteredTokens = filterGlobalCommandTokens(tokens, usedGlobalFlags);
    if (filteredTokens == null) {
        return completeValues([], currentWord);
    }
    ...
    if (subcommand === 'migrate') {
        return completeDbOptions(rest, currentWord, usedGlobalFlags, [], ['--connection-env', '--journal-table', '--migrations', '--to']);
    }
    if (subcommand === 'reset') {
    ...
    if (subcommand === 'status') {
        return completeDbOptions(rest, currentWord, usedGlobalFlags, [], ['--connection-env', '--journal-table', '--migrations']);
    }
    return completeValues([], currentWord);
}
function completeDbOptions(tokens, currentWord, usedGlobalFlags, booleanFlags, valueFlags) {
    let usedFlags = new Set();
    let expectedValueFor;
...
function expectsGlobalConfigValue(tokens) {
