 ...
 */
export function decodePathnameWithParams(source: string): {
  pathname: string
  param(begin: number, end: number): string
} {
  let pathname = decodePathname(source)
  let rawIndexByDecodedIndex: Array<number> | null | undefined
...
function decodePathnameParam(source: string): string {
  if (!source.includes('%')) return source
...
function createRawIndexByDecodedIndex(source: string, decoded: string): Array<number> | null {
  let result = [0]
  let rawIndex = 0
  let decodedIndex = 0
...
function appendLiteralDecodedCharIndexes(
  result: Array<number>,
  rawIndex: number,
  decodedIndex: number,
  length: number,
...
function readPercentEncodedByte(source: string, index: number): number | undefined {
  let hex = source.slice(index + 1, index + 3)
  if (hex.length !== 2 || !/^[\dA-Fa-f]{2}$/.test(hex)) return undefined
  return Number.parseInt(hex, 16)
}

function utf8ByteCount(byte: number): number {
