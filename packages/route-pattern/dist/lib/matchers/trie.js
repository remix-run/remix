...
import { stringifyTokens } from "../stringify.js";
/**
...
export class TrieMatcher {
    ...
    add(pattern, node) {
        ...
        if (parsed.protocol)
            maxDepthInPattern = Math.max(maxDepthInPattern, this.#maxOptionalDepthInTokens(parsed.protocol));
        if (parsed.hostname)
            maxDepthInPattern = Math.max(maxDepthInPattern, this.#maxOptionalDepthInTokens(parsed.hostname));
        if (parsed.pathname)
            maxDepthInPattern = Math.max(maxDepthInPattern, this.#maxOptionalDepthInTokens(parsed.pathname));
        if (maxDepthInPattern > this.#maxOptionalDepth) {
            throw new Error(`Pattern exceeds maxOptionalDepth (${this.#maxOptionalDepth}): ${routePattern.source}`);
        }
        ...
        if (isOrigin) {
            ...
            if (parsed.protocol) {
                if (parsed.protocol.length === 1 && parsed.protocol[0].type === 'variable') {
                    ...
                    this.#addHostnamePattern(this.#originRoot.protocolVariableChild.node.hostnameRoot, parsed.hostname || [], parsed.port, parsed.pathname, routePattern, node, parsed.searchConstraints, parsed);
                }
                else {
                    ...
                    for (let protocolKey of protocolVariants) {
                        ...
                        this.#addHostnamePattern(existing.hostnameRoot, parsed.hostname || [], parsed.port, parsed.pathname, routePattern, node, parsed.searchConstraints, parsed);
                    }
                }
            }
            else {
                ...
                this.#addHostnamePattern(this.#originRoot.anyProtocolChild.hostnameRoot, parsed.hostname || [], parsed.port, parsed.pathname, routePattern, node, parsed.searchConstraints, parsed);
            }
        }
        else {
            ...
            this.#buildPathTrie(root, parsed.pathname, routePattern, node, parsed.searchConstraints, parsed);
        }
        this.#patternCount++;
    }
    /**
    ...
    *matchAll(url) {
        ...
        allMatches.sort((a, b) => this.#finalScore(b.match, b.state) - this.#finalScore(a.match, a.state));
    ...
    #updateDepthUp(node) {
        ...
        for (let child of Object.values(node.staticChildren))
            updateDepth(child, 1, 1);
        for (let entry of node.shapeChildren.values())
            updateDepth(entry.node, 1, 1);
        if (node.variableChild)
            updateDepth(node.variableChild, 1, 1);
        // Wildcard and optional edges don't increment min depth
        if (node.wildcardEdge)
            updateDepth(node.wildcardEdge.continuation, 0, 100);
        for (let opt of node.optionalEdges)
            updateDepth(opt.continuation, 0, 0);
    ...
    #addHostnamePattern(hostnameNode, hostnameTokens, port, pathnameTokens, pattern, userNode, searchConstraints, parsed) {
        ...
        if (pathnameTokens) {
            this.#buildPathTrie(pathnameTrie, pathnameTokens, pattern, userNode, searchConstraints, parsed);
    ...
    #findInterSegmentOptional(tokens) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (token.type === 'optional') {
                ...
                let hasPathSeparator = token.tokens.some((t) => t.type === 'separator' && (tokens[i - 1]?.type !== 'separator' || i === 0));
                if (hasPathSeparator) {
                    return i;
                }
            }
    ...
    #getSimpleShapeKey(tokens, ignoreCase) {
        return tokens
            .map((t) => {
            if (t.type === 'text')
                return `L:${ignoreCase ? t.value.toLowerCase() : t.value}`;
            if (t.type === 'variable')
                return `V:${t.name}`;
            if (t.type === 'wildcard')
                return `W:${t.name || ''}`;
    ...
    #matchShape(shapeEntry, segment) {
        ...
        for (let i = 0; i < shapeEntry.tokens.length; i++) {
            ...
            if (token.type === 'text') {
                ...
                if (!normalizedSegment.startsWith(lit, pos))
                    return null;
                pos += lit.length;
                spec += 100;
            }
        ...
        if (pos !== normalizedSegment.length)
            return null;
    ...
    #findOriginMatches(url, segments, urlSearch, earlyExit) {
        ...
        if (this.#originRoot.protocolVariableChild) {
            ...
            this.#matchHostnameAndPathname(this.#originRoot.protocolVariableChild.node, url, segments, results, protocolParams, urlSearch, earlyExit);
            if (earlyExit && results.length > 0)
                return results;
        }
        // Try any-protocol patterns
        if (this.#originRoot.anyProtocolChild) {
            this.#matchHostnameAndPathname(this.#originRoot.anyProtocolChild, url, segments, results, {}, urlSearch, earlyExit);
            if (earlyExit && results.length > 0)
                return results;
        }
    ...
    #matchHostnameAndPathname(protocolNode, url, segments, results, protocolParams, urlSearch, earlyExit) {
        ...
        for (let { node: hostnameNode, params: hostnameParams } of hostnameMatches) {
            ...
            if (url.port) {
                ...
                if (portTrie) {
                    this.#matchPathnameInTrie(portTrie, segments, hostnameParams, results, urlSearch, earlyExit);
                }
            }
            // Try default pathname trie (matches any port or no port)
            if (hostnameNode.defaultPathnameTrie) {
                this.#matchPathnameInTrie(hostnameNode.defaultPathnameTrie, segments, hostnameParams, results, urlSearch, earlyExit);
            }
        }
    }
    #matchHostnameLabels(node, labels, labelIndex, params) {
        ...
        if (node.variableChild) {
            ...
            results.push(...this.#matchHostnameLabels(node.variableChild.node, labels, labelIndex + 1, newParams));
        }
    ...
    #matchPathnameInTrie(pathnameTrie, segments, hostnameParams, results, urlSearch, earlyExit) {
        ...
        let traversalResults = this.#bestFirstTraversal(pathnameTrie, initialState, earlyExit, urlSearch);
    ...
    #bestFirstTraversal(startNode, startState, earlyExit, urlSearch) {
        ...
        while (stack.length > 0 && statesExplored < this.#maxTraversalStates) {
            ...
            if (earlyExit && state.specificity < bestSpec - 100)
                continue; // Threshold for low spec
                ...
                if (node.patterns.length === 1 && !node.patterns[0].searchConstraints) {
                    ...
                    for (let pattern of node.patterns) {
                        if (pattern.searchConstraints) {
                            ...
                            if (!this.#matchSearch(searchToMatch, pattern.searchConstraints))
                                continue;
                        }
                        let score = this.#finalScore(pattern, state);
                        results.push({ match: pattern, state: { ...state } });
     ...
     */
    #createMatchState(baseState, segmentIndex, params, specificity, nodeId, wildcardSpan) {
        return {
     ...
     */
    #compilePrefixMatcher(prefix, ignoreCase) {
        if (prefix.length === 1) {
            // Single character - ultra fast path
            let charCode = ignoreCase ? prefix.toLowerCase().charCodeAt(0) : prefix.charCodeAt(0);
            if (ignoreCase) {
                return new Function('segment', `return segment.length === 1 && (segment.charCodeAt(0) === ${charCode} || segment.charCodeAt(0) === ${prefix.toUpperCase().charCodeAt(0)})`);
            }
            return new Function('segment', `return segment.length === 1 && segment.charCodeAt(0) === ${charCode}`);
        }
        ...
        for (let i = 0; i < targetPrefix.length; i++) {
            ...
            if (ignoreCase && prefix.charCodeAt(i) >= 65 && prefix.charCodeAt(i) <= 90) {
                ...
                checks.push(`(segment.charCodeAt(${i}) === ${upperCode} || segment.charCodeAt(${i}) === ${lowerCode})`);
            }
            else {
        ...
        return new Function('segment', fnBody);
    }
    ...
    #expandTraversalState(node, state, states, // Collect
    earlyExit, bestSpec) {
        ...
        if (staticChild) {
            let newState = this.#createMatchState(state, state.segmentIndex + 1, state.params, state.specificity + 100, staticChild.id);
            let ts = {
                node: staticChild,
                state: newState,
        ...
        for (let [shapeKey, shapeEntry] of node.shapeChildren) {
            ...
            if (matchResult) {
                let newState = this.#createMatchState(state, state.segmentIndex + 1, { ...state.params, ...matchResult.params }, state.specificity + matchResult.specificity, shapeEntry.node.id);
                let ts = {
        ...
        if (node.variableChild) {
            ...
            let newState = this.#createMatchState(state, state.segmentIndex + 1, newParams, state.specificity + 10, node.variableChild.id);
            let ts = {
    ...
    #expandOptionalStates(optionalEdge, currentNode, state, states, // Now collects
    earlyExit, bestSpec) {
    ...
    #expandWildcardStates(wildcardEdge, state, states, earlyExit, bestSpec) {
        ...
        if (minConsume > maxConsume)
            return;
        // Greedy-first: try larger consumption counts first
        for (let consumeCount = maxConsume; consumeCount >= minConsume; consumeCount--) {
            let consumedSegments = state.segments.slice(state.segmentIndex, state.segmentIndex + consumeCount);
            let newParams = { ...state.params };
            if (wildcardEdge.paramName) {
                newParams[wildcardEdge.paramName] = consumedSegments.join('/');
            }
            let newState = this.#createMatchState(state, state.segmentIndex + consumeCount, newParams, state.specificity + 1, continuation.id, `${state.segmentIndex}-${state.segmentIndex + consumeCount}`);
            let ts = {
                node: continuation,
                state: newState,
