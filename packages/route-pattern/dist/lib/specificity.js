 ...
 */
export let ascending = (a, b) => compare(a, b);
/**
 ...
 */
export let descending = (a, b) => compare(a, b) * -1;
/**
...
function compareHostname(hostname, a, b) {
...
function comparePathname(a, b) {
    ...
    while (aIndex < a.length || bIndex < b.length) {
        ...
        let bRange = b[bIndex];
        if (aRange === undefined)
            return 1; // a is fully static from here
        if (bRange === undefined)
            return -1; // b is fully static from here
        // Skip to the minimum begin of the two current ranges
        i = Math.min(aRange.begin, bRange.begin);
        if (i < aRange.begin)
            return 1; // a has static content at i
        if (i < bRange.begin)
            return -1; // b has static content at i
...
function compareSearch(a, b) {
    ...
    if (aSpecificity.keyAndExactValue > bSpecificity.keyAndExactValue)
        return 1;
    if (aSpecificity.keyAndExactValue < bSpecificity.keyAndExactValue)
        return -1;
    if (aSpecificity.keyAndAnyValue > bSpecificity.keyAndAnyValue)
        return 1;
    if (aSpecificity.keyAndAnyValue < bSpecificity.keyAndAnyValue)
        return -1;
