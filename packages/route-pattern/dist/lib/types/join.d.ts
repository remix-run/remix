...
export type JoinPatterns<A extends string, B extends string> = string extends A | B ? string : _JoinPatterns<Parse<A>, Parse<B>>;
type _JoinPatterns<A, B> = A extends never ? never : B extends never ? never : A extends ParsedPattern ? B extends ParsedPattern ? Stringify<{
...
type JoinOriginField<A extends ParsedPattern, B extends ParsedPattern, Field extends 'protocol' | 'hostname' | 'port'> = B[Field] extends undefined ? A[Field] : B[Field];
...
type JoinPathnameTokens<A extends Token[], B extends Token[]> = B extends [Separator] ? A : StartsWithSeparator<B> extends true ? [...A, ...B] : [
    ...A,
    Separator,
    ...B
];
