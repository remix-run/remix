...
export type Split<T extends string> = _Split<T> extends infer S ? [
    S
] extends [never] ? never : S extends Partial<SplitPattern> ? {
...
type _Split<T extends string> = T extends '' ? {} : T extends `${infer L}?${infer R}` ? _Split<L> & {
    search: R;
} : T extends `${infer Protocol}://${infer R}` ? Protocol extends '' ? (R extends `${infer Host}/${infer Pathname}` ? SplitHost<Host> & ToPathname<Pathname> : SplitHost<R>) : Protocol extends `${string}/${string}` ? {
    pathname: T;
} : R extends `${infer Host}/${infer Pathname}` ? SplitHost<Host> & {
    protocol: Protocol;
} & ToPathname<Pathname> : SplitHost<R> & {
    protocol: Protocol;
...
type SplitHost<T extends string> = T extends `${infer L}:${infer R}` ? IsDigits<R> extends true ? (L extends '' ? never : {
    hostname: L;
    port: R;
}) : SplitHost<R> extends {
...
type ToPathname<T extends string> = T extends '' ? {} : {
    pathname: T;
};
