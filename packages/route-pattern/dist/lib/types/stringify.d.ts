...
export type Stringify<T extends ParsedPattern> = HasOrigin<T> extends true ? `${StringifyTokens<T['protocol'], ''>}://${StringifyTokens<T['hostname'], '.'>}${StringifyPort<T['port']>}${StringifyPathname<T['pathname']>}${StringifySearch<T['search']>}` : `${StringifyPathname<T['pathname']>}${StringifySearch<T['search']>}`;
type HasOrigin<T extends ParsedPattern> = T['protocol'] extends Token[] ? true : T['hostname'] extends Token[] ? true : T['port'] extends string ? true : false;
