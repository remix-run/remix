...
## Not yet in this spec

These are features on our radar, but aren't ready to be included in this spec just yet:

- Matching URL query parameters (`?foo=bar`)
- `**`-style globs
...
**Note:** Route patterns always match against complete URLs, but in our examples we may show only the relevant part of the URL for brevity.
...
Globs capture "everything beyond this point" using `*` followed by a name:
...
const { pattern, params } = matcher.bestMatch('https://example.com/products/123/files/docs/readme.md')
