...
function generatePatterns(): string[] {
  ...
  for (let i = 0; i < 200; i++) {
    patterns.push(`/static/path/${i}/resource`)
  }
  ...
  for (let i = 0; i < 300; i++) {
    ...
    patterns.push(`/dynamic/${i}/${segments.join('/')}`)
  }
  ...
  for (let i = 0; i < 100; i++) {
    patterns.push(`/wildcard/${i}/*path`)
    patterns.push(`/*prefix/middle/${i}/suffix`)
  }

  for (let i = 0; i < 150; i++) {
    patterns.push(`https://:tenant${i}.myapp.com/admin`)
    patterns.push(`http(s)://:subdomain.example${i}.com/api`)
    patterns.push(`://:store.shop${i}.com/products/:id`)
  }

  for (let i = 0; i < 100; i++) {
    patterns.push(`/search/${i}?q=&filter=`)
    patterns.push(`/products/${i}?category=&price_min=&price_max=`)
  }
  ...
  patterns.push('/prefix:param/middle:param2/suffix')
