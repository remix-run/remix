...
export class RequestContext {
    ...
    #installContextProperty(key, property) {
        ...
        if (existingProperty != null && existingProperty !== property) {
            throw new Error(`Cannot install context property ${formattedProperty} because this context key already uses ${JSON.stringify(existingProperty)}.`);
        }
        let existingKey = this.#contextProperties.get(property);
        if (existingKey != null) {
            if (existingKey !== key) {
                throw new Error(`Cannot install context property ${formattedProperty} because another context key already uses it.`);
            }
            return;
        }
        if (property in this) {
            throw new Error(`Cannot install context property ${formattedProperty} because it already exists on RequestContext.`);
        }
