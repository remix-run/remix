 ...
 */
export type ContextParams<context> = context extends RequestContext<infer params extends Record<string, any>, any> ? params : {};
type DuplicateParamNames<left extends Record<string, any>, right extends Record<string, any>> = Extract<keyof left, keyof right>;
/**
 * Merges two params objects and fails with `never` when they define the same param name.
 */
export type MergeContextParams<left extends Record<string, any>, right extends Record<string, any>> = [DuplicateParamNames<left, right>] extends [never] ? Simplify<left & right> : never;
/**
 * Replaces the params type of a {@link RequestContext} while preserving its existing context entries.
 */
export type WithParams<context, params extends Record<string, any>> = context extends RequestContext<any, infer entries extends ContextEntries> ? MergeContextParams<ContextParams<context>, params> extends infer merged ? [merged] extends [never] ? never : RequestContext<Extract<merged, Record<string, any>>, entries> : never : RequestContext<params>;
type ResolveContextEntryValue<entries extends ContextEntries, key extends object, fallback> = entries extends readonly [...infer rest extends ContextEntries, infer last extends ContextEntry] ? [key] extends [last[0]] ? [last[0]] extends [key] ? last[1] : ResolveContextEntryValue<rest, key, fallback> : ResolveContextEntryValue<rest, key, fallback> : fallback;
/**
 * Resolves the value type returned by `context.get(key)` for the given context and key.
 */
export type GetContextValue<context, key extends object> = context extends RequestContext<any, infer entries extends ContextEntries> ? ResolveContextEntryValue<entries, key, ContextFallbackValue<key>> : ContextFallbackValue<key>;
/**
 * Appends context entries to an existing {@link RequestContext}.
 */
export type MergeContext<context, additions extends ContextEntries> = context extends RequestContext<infer params extends Record<string, any>, infer entries extends ContextEntries> ? RequestContext<params, [...entries, ...additions]> : never;
/**
 * Replaces or adds the value type for a single context key in a {@link RequestContext}.
 */
export type SetContextValue<context, key extends object, value> = MergeContext<context, [
    readonly [key, value]
]>;
/**
...
export declare class RequestContext<params extends Record<string, any> = {}, entries extends ContextEntries = []> {
     ...
     */
    get: <key extends object>(key: key) => ResolveContextEntryValue<entries, key, ContextFallbackValue<key>>;
    /**
