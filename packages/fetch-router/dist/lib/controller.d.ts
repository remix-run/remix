 ...
 */
export interface RequestHandler<context extends RequestContext<any, any, any> = RequestContext, output = DefaultOutput> {
    /**
     * Handles a matched request and returns the router output.
     */
    (context: context): Defined<output> | Promise<Defined<output>>;
}
export declare function isRequestHandler(object: unknown): object is RequestHandler<any, any>;
...
type ActionContext<route extends ActionRoute, context extends RequestContext<any, any, any>, output> = ContextWithParams<ContextWithOutput<context, output>, MatchParams<ActionPattern<route>>>;
export type ActionObject<route extends ActionRoute, context extends RequestContext<any, any, any> = DefaultContext, middleware extends readonly AnyMiddleware<any>[] = readonly AnyMiddleware<any>[], output = DefaultOutput> = {
    /**
     ...
     */
    handler: RequestHandler<MiddlewareContext<middleware, ActionContext<route, context, output>>, output>;
};
/**
 ...
 */
export type Action<route extends ActionRoute, context extends RequestContext<any, any, any> = DefaultContext, middleware extends readonly AnyMiddleware<any>[] = readonly AnyMiddleware<any>[], output = DefaultOutput> = RequestHandler<ActionContext<route, context, output>, output> | ActionObject<route, context, middleware, output>;
/**
 ...
 */
export declare function createAction<route extends ActionRoute, context extends RequestContext<any, any, any> = DefaultContext, const middleware extends readonly AnyMiddleware<any>[] = readonly AnyMiddleware<any>[], output = DefaultOutput>(route: route, action: Action<route, context, middleware, output>): Action<route, context, middleware, output>;
export declare function isAction(obj: unknown): obj is Action<any, any, any, any>;
export declare function isActionObject(obj: unknown): obj is ActionObject<any, any, any, any>;
type ControllerActions<routes extends RouteMap, context extends RequestContext<any, any, any>, output> = routes extends any ? {
    [name in keyof routes as routes[name] extends Route<any, any> ? name : never]: routes[name] extends Route<any, any> ? Action<routes[name], context, readonly AnyMiddleware<output>[], output> : never;
} & {
 ...
 */
export type Controller<routes extends RouteMap, context extends RequestContext<any, any, any> = DefaultContext, middleware extends readonly AnyMiddleware<any>[] = readonly AnyMiddleware<any>[], output = DefaultOutput> = {
    middleware?: readonly [...middleware] & readonly AnyMiddleware<output>[];
    actions: ControllerActions<routes, MiddlewareContext<middleware, ContextWithOutput<context, output>>, output>;
};
/**
 ...
 */
export declare function createController<routes extends RouteMap, context extends RequestContext<any, any, any> = DefaultContext, const middleware extends readonly AnyMiddleware<any>[] = readonly AnyMiddleware<any>[], output = DefaultOutput>(routes: routes, controller: Controller<routes, context, middleware, output>): Controller<routes, context, middleware, output>;
