...
export type ActionObjectWithoutMiddleware<params extends Record<string, any>, context extends RequestContext<any, any>> = {
    middleware?: undefined;
    handler: RequestHandler<params, context>;
};
export type ActionObjectWithMiddleware<params extends Record<string, any>, context extends RequestContext<any, any>, middleware extends readonly AnyMiddleware[]> = {
    middleware: readonly [...middleware];
    handler: RequestHandler<params, ApplyMiddlewareTuple<context, middleware>>;
};
export type ActionInput<params extends Record<string, any>, context extends RequestContext<any, any>, middleware extends readonly AnyMiddleware[] = readonly AnyMiddleware[]> = RequestHandler<params, context> | ActionObjectWithoutMiddleware<params, context> | ActionObjectWithMiddleware<params, context, middleware>;
...
type ControllerActions<routes extends RouteMap, context extends RequestContext<any, any>> = routes extends any ? {
    [name in keyof routes]: (routes[name] extends Route<infer method extends RequestMethod | 'ANY', infer pattern extends string> ? Action<method, pattern, context> : routes[name] extends RouteMap ? Controller<routes[name], context> : never);
} : never;
 ...
 */
export type Action<_method extends RequestMethod | 'ANY', pattern extends string, context extends RequestContext<any, any> = RequestContext> = ActionInput<Params<pattern>, WithParams<context, Params<pattern>>, readonly AnyMiddleware[]>;
/**
 * Builds an {@link Action} type from a string pattern, {@link RoutePattern}, or {@link Route}.
 */
export type BuildAction<method extends RequestMethod | 'ANY', route extends string | RoutePattern | Route, context extends RequestContext<any, any> = RequestContext> = route extends string ? Action<method, route, context> : route extends RoutePattern<infer pattern> ? Action<method, pattern, context> : route extends Route<infer _, infer pattern> ? Action<method, pattern, context> : never;
/**
