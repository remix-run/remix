...
export type BuildRouteMap<base extends string, defs extends RouteDefs> = Simplify<{
    -readonly [name in keyof defs]: (defs[name] extends Route<infer method extends RequestMethod | 'ANY', infer pattern extends string> ? Route<method, JoinPatterns<base, pattern>> : defs[name] extends RouteDef ? BuildRouteWithBase<base, defs[name]> : defs[name] extends RouteDefs ? BuildRouteMap<base, defs[name]> : never);
}>;
type BuildRouteWithBase<base extends string, def extends RouteDef> = def extends string ? Route<'ANY', JoinPatterns<base, def>> : def extends RoutePattern<infer pattern extends string> ? Route<'ANY', JoinPatterns<base, pattern>> : def extends {
    method: infer method extends RequestMethod | 'ANY';
    pattern: infer pattern;
} ? (pattern extends string ? Route<method, JoinPatterns<base, pattern>> : pattern extends RoutePattern<infer source extends string> ? Route<method, JoinPatterns<base, source>> : never) : never;
/**
