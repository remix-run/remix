...
async function rewriteDependencies(resolvedStyle, options) {
    ...
    for (let dependency of resolvedStyle.dependencies) {
        ...
        if (start < 0) {
            throw createStyleServerCompilationError(`Missing dependency placeholder "${dependency.placeholder}" while emitting CSS.`, {
...
function toEmitError(error, identityPath) {
    if (isStyleServerCompilationError(error))
        return error;
    return createStyleServerCompilationError(`Failed to emit CSS in ${identityPath}. ${error instanceof Error ? error.message : String(error)}`, {
        cause: error,
        code: 'STYLE_EMIT_FAILED',
    });
}
