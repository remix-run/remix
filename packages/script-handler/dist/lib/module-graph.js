...
import { resolveAbsolutePathFromResolvedRoots } from "./path-utils.js";
let lexerReady = lexerInit;
export function createModuleGraphStore() {
    let raw = new Map();
    let rawInFlight = new Map();
...
function joinBasePath(base, relativePath) {
...
async function compileRaw(absolutePath, store, opts) {
...
async function _compileRaw(absolutePath, store, opts) {
    ...
    for (let imp of imports) {
        if (imp.n == null)
            continue;
        let specifier = imp.n;
        if (opts.external.includes(specifier))
            continue;
    ...
    let resolvedPaths = toResolve.length > 0
        ? await batchResolveSpecifiers(toResolve.map((s) => s.specifier), importerDir)
        : new Map();
    let rawImports = [];
    let depsSet = new Set();
...
function getSourceMapSourcePath(absolutePath, opts) {
    if (opts.sourceMapSourcePaths === 'absolute')
        return absolutePath;
...
function getSourceMapModeSentinel(opts) {
...
async function buildRawGraph(absolutePath, store, opts, visited = new Set()) {
...
function collectRawTransitiveDeps(absolutePath, store) {
    let visited = new Set();
    let result = [];
...
function getPublicPath(absolutePath, store, opts) {
    if (store.publicPaths.has(absolutePath)) {
        return store.publicPaths.get(absolutePath) ?? null;
    }
    let publicPath = resolveAbsolutePathFromResolvedRoots(absolutePath, opts.roots)?.publicPath ?? null;
...
async function finalizeModule(absolutePath, store, opts) {
...
async function finalizeCycle(scc, store, opts) {
    ...
    let sortedSources = scc
        ...
        .join('\0');
    let externalDepHashes = new Set();
    for (let p of scc) {
        for (let dep of store.raw.get(p).deps) {
            if (!sccSet.has(dep)) {
                let depResult = store.compiled.get(dep);
                if (depResult)
                    externalDepHashes.add(depResult.compiledHash);
            }
        }
    }
    let sortedSourceMapHashes = scc
        ...
        .join('\0');
    let smSentinel = getSourceMapModeSentinel(opts);
    let cycleHash = await hashContent(sortedSources +
        '\0\0' +
...
export async function buildGraph(absolutePath, store, opts) {
...
export function collectTransitiveDeps(absolutePath, store) {
    let visited = new Set();
    let result = [];
    let queue = [absolutePath];
    while (queue.length > 0) {
...
export async function isCompiledGraphFresh(absolutePath, store) {
    let visited = new Set();
    let queue = [absolutePath];
    while (queue.length > 0) {
        let current = queue.shift();
...
function getUnconfiguredRootErrorMessage(importerPath, specifier, absolutePath) {
    return (`Resolved import "${specifier}" in ${importerPath} points to ${absolutePath}, which is outside all configured roots.\n\n` +
                ...
                setup(build) {
                    build.onStart(async () => {
                        ...
                        for (let i = 0; i < specifiers.length; i++) {
                            ...
                            let absolutePath = r && !r.external && r.path && path.isAbsolute(r.path) ? r.path : null;
