...
import { isDeepEqual as isDeepStrictEqual } from "./deep-equal.js";
...
function isPartialMatcher(value) {
    return (typeof value === 'object' &&
        value !== null &&
        value[PARTIAL_MATCHER] === true);
}
...
function isDeepEqual(actual, expected) {
    if (isPartialMatcher(expected))
        return matchesPartial(actual, expected.expected);
...
function matchesPartial(actual, expected) {
    ...
    return Object.keys(expected).every((key) => hasOwn(actual, key) &&
        matchesPartial(actual[key], expected[key]));
}
...
function getMockCalls(received, matcherName) {
...
function checkErrorMatch(error, expected) {
...
function describeExpectedError(expected) {
    if (expected === undefined)
        return 'an error';
    if (typeof expected === 'function') {
        if (expected.prototype != null && expected.prototype instanceof Error) {
            return `an instance of ${expected.name}`;
        }
        return 'an error matching the validator';
    }
    if (expected instanceof Error)
        return `an error with message ${JSON.stringify(expected.message)}`;
    if (expected instanceof RegExp)
        return `an error matching ${expected}`;
    if (typeof expected === 'string')
        return `an error containing ${JSON.stringify(expected)}`;
    return String(expected);
}
function fail(operator, message, actual, expected) {
    throw new AssertionError({ message, actual, expected, operator });
}
function createMatchers(received, negated) {
    function check(condition, makeMessage, expected, operator) {
    ...
    return {
        toBe(expected) {
            check(Object.is(received, expected), () => `${stringify(received)} to be ${stringify(expected)}`, expected, 'toBe');
        },
        toEqual(expected) {
            check(isDeepEqual(received, expected), () => `${stringify(received)} to deeply equal ${stringify(expected)}`, expected, 'toEqual');
        },
        toBeNull() {
            check(received === null, () => `${stringify(received)} to be null`, null, 'toBeNull');
        },
        toBeUndefined() {
            check(received === undefined, () => `${stringify(received)} to be undefined`, undefined, 'toBeUndefined');
        },
        toBeDefined() {
            check(received !== undefined, () => `${stringify(received)} to be defined`, undefined, 'toBeDefined');
        },
        toBeTruthy() {
            check(Boolean(received), () => `${stringify(received)} to be truthy`, true, 'toBeTruthy');
        },
        toBeInstanceOf(ctor) {
            check(received instanceof ctor, () => `${stringify(received)} to be instance of ${ctor.name || ctor}`, ctor, 'toBeInstanceOf');
        },
        toBeGreaterThan(n) {
            check(typeof received === 'number' && received > n, () => `${stringify(received)} to be greater than ${n}`, n, 'toBeGreaterThan');
        },
        toBeGreaterThanOrEqual(n) {
            check(typeof received === 'number' && received >= n, () => `${stringify(received)} to be greater than or equal to ${n}`, n, 'toBeGreaterThanOrEqual');
        },
        toBeLessThan(n) {
            check(typeof received === 'number' && received < n, () => `${stringify(received)} to be less than ${n}`, n, 'toBeLessThan');
        },
        toBeLessThanOrEqual(n) {
            check(typeof received === 'number' && received <= n, () => `${stringify(received)} to be less than or equal to ${n}`, n, 'toBeLessThanOrEqual');
        },
        toBeCloseTo(n, precision = 2) {
            let diff = Math.abs(received - n);
            let tolerance = Math.pow(10, -precision) / 2;
            check(typeof received === 'number' && diff < tolerance, () => `${stringify(received)} to be close to ${n} (precision ${precision})`, n, 'toBeCloseTo');
        },
        toContain(item) {
            let contained;
            ...
            else if (received != null && typeof received[Symbol.iterator] === 'function') {
                contained = Array.from(received).some((v) => Object.is(v, item) || v === item);
            }
            else {
                contained = false;
            }
            check(contained, () => `${stringify(received)} to contain ${stringify(item)}`, item, 'toContain');
        },
        toMatch(re) {
            let pattern = typeof re === 'string' ? new RegExp(escapeRegex(re)) : re;
            check(typeof received === 'string' && pattern.test(received), () => `${stringify(received)} to match ${pattern}`, re, 'toMatch');
        },
        toHaveLength(n) {
            let len = received?.length;
            check(len === n, () => `${stringify(received)} to have length ${n} (got ${len})`, n, 'toHaveLength');
        },
        toHaveProperty(key, value) {
            ...
            check(pass, () => arguments.length < 2
                ? `${stringify(received)} to have property "${key}"`
                : `${stringify(received)} to have property "${key}" with value ${stringify(value)}`, value, 'toHaveProperty');
        },
        toMatchObject(expected) {
            check(matchesPartial(received, expected), () => `${stringify(received)} to recursively match ${stringify(expected)}`, expected, 'toMatchObject');
        },
        toThrow(expected) {
            ...
            check(pass, () => thrown
                ...
                : `function to throw ${describeExpectedError(expected)}`, expected, 'toThrow');
        },
        toHaveBeenCalled() {
            let calls = getMockCalls(received, 'toHaveBeenCalled');
            check(calls.length > 0, () => `mock to have been called (got ${calls.length} calls)`, undefined, 'toHaveBeenCalled');
        },
        toHaveBeenCalledTimes(n) {
            let calls = getMockCalls(received, 'toHaveBeenCalledTimes');
            check(calls.length === n, () => `mock to have been called ${n} times (got ${calls.length})`, n, 'toHaveBeenCalledTimes');
        },
        toHaveBeenCalledWith(...args) {
            let calls = getMockCalls(received, 'toHaveBeenCalledWith');
            let matched = calls.some((call) => isDeepEqual(call.arguments, args));
            check(matched, () => `mock to have been called with ${stringify(args)} (calls: ${stringify(calls.map((c) => c.arguments))})`, args, 'toHaveBeenCalledWith');
        },
        toHaveBeenNthCalledWith(nth, ...args) {
            ...
            check(matched, () => `mock call #${nth} to be ${stringify(args)} (got ${call ? stringify(call.arguments) : '<no call>'})`, args, 'toHaveBeenNthCalledWith');
        },
    };
}
function createAsyncMatchers(promise, mode, negated) {
    ...
    function buildMatcher(matcher) {
        return async (...args) => {
            ...
            if (mode === 'resolves') {
                ...
                let m = createMatchers(result.value, negated);
                m[matcher](...args);
            }
            else {
                if (result.resolved) {
                ...
                if (matcher === 'toThrow') {
                    ...
                    if (negated ? pass : !pass) {
                        throw new AssertionError({
                            message: (negated ? 'expected not ' : 'expected ') +
                ...
                let m = createMatchers(result.error, negated);
                m[matcher](...args);
            }
        };
    }
...
function expectImpl(received) {
    return {
        ...createMatchers(received, false),
        not: createMatchers(received, true),
        rejects: createAsyncMatchers(received, 'rejects', false),
        resolves: createAsyncMatchers(received, 'resolves', false),
    };
}
/**
...
function objectContaining(expected) {
    return { [PARTIAL_MATCHER]: true, expected };
}
/**
