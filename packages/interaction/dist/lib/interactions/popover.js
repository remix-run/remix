import { defineInteraction } from "../interaction.js";
/**
 * ### Description
 *
...
function Popover(handle) {
    if (!(handle.target instanceof HTMLElement))
        return;
    ...
    handle.on(popover, {
        toggle(event) {
            target.dispatchEvent(new ToggleEvent(popoverToggle, {
        ...
        beforetoggle(event) {
            target.dispatchEvent(new ToggleEvent(beforePopoverToggle, {
                bubbles: event.bubbles,
                cancelable: event.cancelable,
                composed: event.composed,
                newState: event.newState,
