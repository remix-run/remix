 ...
 * Tracks import relationships and caches transform results.
 */
...
export function devAssets(options: DevAssetsOptions): Middleware {
  ...
  async function interceptHtmlResponse(response: Response): Promise<Response> {
    ...
    // HMR runtime script - auto-connects when loaded
...
async function transformSource(
  ...
  if (
    ...
    if (DEBUG) {
      ...
      console.log(
        `[dev-assets-middleware] cache hit ${path.basename(filePath)} in ${elapsed}ms`,
      )
    }
