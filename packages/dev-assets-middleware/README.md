...
This middleware transforms TypeScript and JSX files on-demand, rewrites imports for browser compatibility, and serves files from `node_modules`. **This is for development only** - for production, use `remix/assets-middleware` with a build manifest generated by `remix/assets`.
...
```tsx
import { component } from 'remix/component'
...
```js
import { component } from '/__@workspace/node_modules/remix/component/src/index.ts'
import { greet } from './utils.ts'
