        ...
        return function render(node, init) {
            let stream = renderToStream(node, {
                ...
                ...(assets == null
                    ? {}
                    : {
                        resolveClientEntry: (entryId, component) => resolveClientEntry(assets, entryId, component),
                    }),
            });
...
async function resolveClientEntry(assets, entryId, component) {
    ...
    if (!exportName) {
        throw new Error(`clientEntry() requires either an export name in the entry ID (e.g., import.meta.url + "#ExportName") or a named component function. Received "${entryId}".`);
    }
    return {
