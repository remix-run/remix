import { getByteLength, getIndexes } from "./byte-range.js";
/**
...
export class LazyBlob {
     ...
     */
    toString() {
        throw new TypeError('Cannot convert LazyBlob to string. Use .stream() to get a ReadableStream for Response and other streaming APIs, or .toBlob() for non-streaming APIs that require a complete Blob (e.g. FormData). Always prefer .stream() when possible.');
    }
}
/**
...
export class LazyFile {
     ...
     */
    toString() {
        throw new TypeError('Cannot convert LazyFile to string. Use .stream() to get a ReadableStream for Response and other streaming APIs, or .toFile()/.toBlob() for non-streaming APIs that require a complete File/Blob (e.g. FormData). Always prefer .stream() when possible.');
    }
}
