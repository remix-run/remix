...
export function createMemcacheClient(server) {
...
async function getMemcacheValue(address, key) {
...
async function setMemcacheValue(address, key, value, ttlSeconds) {
    let valueBuffer = Buffer.from(value, 'utf8');
    let expires = getMemcacheExpiration(ttlSeconds);
    let commandBuffer = Buffer.from(`set ${key} 0 ${expires} ${valueBuffer.byteLength}${CRLF}`, 'utf8');
...
async function deleteMemcacheValue(address, key) {
...
async function sendMemcacheCommand(address, command, isComplete) {
...
async function connectToMemcache(address) {
    return await new Promise((resolve, reject) => {
        ...
        function onError(error) {
            finalize(new Error(`Failed to connect to Memcache ${address.host}:${address.port}: ${error.message}`));
        }
        function onTimeout() {
...
async function writeToSocket(socket, command) {
...
async function readFromSocket(socket, isComplete) {
    return await new Promise((resolve, reject) => {
        let chunks = [];
        let totalLength = 0;
        let settled = false;
...
function isLineResponseComplete(response) {
    return response.indexOf(CRLF) !== -1;
}
function parseSingleLineResponse(response, command) {
    let lineEnd = response.indexOf(CRLF);
    if (lineEnd === -1) {
...
function getMemcacheExpiration(ttlSeconds) {
    if (ttlSeconds <= MAX_RELATIVE_EXPIRATION_SECONDS) {
        return ttlSeconds;
    }
    return Math.floor(Date.now() / 1000) + ttlSeconds;
}
function isGetResponseComplete(response) {
...
function parseMemcacheServer(server) {
    let url;
    try {
