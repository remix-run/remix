 ...
 */
export function createDataTableJobStorage(db, options = {}) {
    let baseDb = db;
    ...
    return {
        enqueue(input, writeOptions) {
            let db = writeOptions?.transaction ?? baseDb;
            return runOperation(() => db.transaction(async (database) => {
                await cleanupExpiredDedupe(database, tables, input.createdAt);
                if (input.dedupeKey != null) {
                    let dedupeRows = await database.exec(rawSql(`select job_id from ${tables.dedupe} where dedupe_key = ? and expires_at > ? limit 1`, [input.dedupeKey, input.createdAt]));
                ...
                if (input.dedupeKey != null && input.dedupeTtlMs != null && input.dedupeTtlMs > 0) {
                    await database.exec(rawSql(`insert into ${tables.dedupe} (dedupe_key, job_id, expires_at) values (?, ?, ?)`, [input.dedupeKey, jobId, input.createdAt + input.dedupeTtlMs]));
                }
                return {
                    jobId,
                    deduped: false,
        ...
        get(jobId) {
            return runOperation(async () => {
                let rows = await baseDb.exec(rawSql(`select * from ${tables.jobs} where id = ? limit 1`, [jobId]));
                let row = rows.rows?.[0];
                if (row == null) {
                    return null;
                }
        ...
        listFailedJobs(input) {
            ...
            return runOperation(async () => {
                let rows = await baseDb.exec(rawSql([
                    `select * from ${tables.jobs} `,
                    'where status = ? ',
                    input.queue == null ? '' : 'and queue = ? ',
                    'order by failed_at desc, updated_at desc limit ?',
                ].join(''), input.queue == null ? ['failed', input.limit ?? 50] : ['failed', input.queue, input.limit ?? 50]));
                return (rows.rows ?? []).map(toJobRecord);
            });
        },
        retryFailedJob(input, writeOptions) {
            let db = writeOptions?.transaction ?? baseDb;
            return runOperation(() => db.transaction(async (database) => {
                let rows = await database.exec(rawSql(`select * from ${tables.jobs} where id = ? and status = ? limit 1`, [input.jobId, 'failed']));
        ...
        prune(input, writeOptions) {
            ...
            return runOperation(() => db.transaction(async (database) => {
                ...
                await database.exec(rawSql(`delete from ${tables.jobs} where id in ${inClause}`, candidateIds));
                return {
        ...
        claimDueJobs(input) {
            ...
            return runOperation(() => baseDb.transaction(async (database) => {
                ...
                ].join(''), ['queued', 'running', input.now, ...input.queues, input.now, input.limit]));
                let claimed = [];
                for (let row of dueRows.rows ?? []) {
                    if (typeof row.id !== 'string') {
                        continue;
                    }
                    let lockResult = await database.exec(rawSql([
                    ...
                    let claimedRows = await database.exec(rawSql(`select * from ${tables.jobs} where id = ? limit 1`, [row.id]));
        ...
        heartbeat(input) {
            return runOperation(async () => {
                let result = await baseDb.exec(rawSql([
                    `update ${tables.jobs} set locked_until = ?, updated_at = ? `,
                    'where id = ? and status = ? and locked_by = ? and (locked_until is null or locked_until > ?)',
                ].join(''), [input.now + input.leaseMs, input.now, input.jobId, 'running', input.workerId, input.now]));
                return (result.affectedRows ?? 0) > 0;
            });
        },
        complete(input) {
            return runOperation(async () => {
                await baseDb.exec(rawSql([
                    `update ${tables.jobs} set status = ?, locked_by = null, locked_until = null, `,
                    'completed_at = ?, failed_at = null, canceled_at = null, updated_at = ? ',
                    'where id = ? and status = ? and locked_by = ?',
                ].join(''), ['completed', input.now, input.now, input.jobId, 'running', input.workerId]));
            });
        },
        fail(input) {
            return runOperation(async () => {
                if (input.terminal) {
                    await baseDb.exec(rawSql([
                        ...
                        'where id = ? and status = ? and locked_by = ?',
                    ].join(''), ['failed', input.now, input.error, input.now, input.jobId, 'running', input.workerId]));
                    return;
                }
                await baseDb.exec(rawSql([
...
function createJobStorageSchema(tablePrefix) {
 ...
 */
export function createDataTableJobStorageMigration(options = {}) {
    let storageSchema = createJobStorageSchema(options.tablePrefix);
...
function createOperationRunner(dialect) {
    if (dialect !== 'sqlite') {
...
async function cleanupExpiredDedupe(db, tables, now) {
