...
describe('analyzeNodeHmrSource', () => {
  ...
  it('detects accepted dependencies from real accept calls', () => {
    assert.deepEqual(
      ...
      {
        acceptedDeps: [
          'file:///app/routes/message.ts',
          'file:///app/routes/one.ts',
          'file:///app/two.ts',
        ],
        selfAccepting: false,
        usesImportMetaHot: true,
  ...
  it('detects self-accepting modules from optional chaining accept calls', () => {
    assert.deepEqual(analyzeNodeHmrSource('file:///app/message.ts', `import.meta.hot?.accept()`), {
      acceptedDeps: [],
      selfAccepting: true,
      usesImportMetaHot: true,
    })
  ...
  it('skips parsing source that does not mention the hot API', () => {
    assert.deepEqual(analyzeNodeHmrSource('file:///app/message.ts', `export function broken( {`), {
      acceptedDeps: [],
      selfAccepting: false,
      usesImportMetaHot: false,
    })
