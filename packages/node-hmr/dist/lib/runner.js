...
export function createFileChangeEventDeduper() {
    let previousStatsByFilePath = new Map();
    return (filePath, stats) => {
        ...
        return (previousStats !== undefined &&
...
const chokidarWatcherBySupervisor = new WeakMap();
export function getSupervisorChokidarWatcher(supervisor) {
...
export function createHmrSupervisor(options) {
    ...
    function handleChildMessage(message, childRestartGeneration) {
        ...
        if (message.type === 'node-hmr:child:server-ready') {
            if (childRestartGeneration !== pendingRestartGeneration)
                return;
            serverReadyCount += 1;
            waitingForEntryServerReady = false;
        ...
        try {
            ...
            for (let changedPath of changedPaths) {
                ...
                for (let moduleInfo of modules) {
                    ...
                    hotUpdates.push(...hotUpdateBoundaries.map((boundary) => ({
...
function isBrowserHmrEvent(event) {
    if (typeof event !== 'object' || event === null || !('type' in event))
        return false;
    if (event.type === 'update') {
        return ('data' in event && isJsonObject(event.data) && isOptionalStringArrayProperty(event, 'files'));
    }
