...
- **Automated releases**: When changes are pushed to `main`, the [changes-version-pr workflow](/.github/workflows/changes-version-pr.yaml) automatically opens/updates a "Version Packages" PR. Merging this PR creates tags which then triggers the publish workflow.
- **Manual releases**: `pnpm changes:version` updates package.json, CHANGELOG.md, creates a git commit and tags. Use this for hotfixes or if the automated flow fails.
- **Test change/release code with preview scripts**: When modifying any change/release code, run `pnpm changes:preview` and `pnpm create-github-release:preview` to test locally. For the version PR scripts, run `node ./scripts/changes-version-pr-open.ts --preview`.
