...
- **Automated releases**: When changes are pushed to `main`, the [changes-version-pr workflow](/.github/workflows/changes-version-pr.yaml) automatically opens/updates a "Version Packages" PR. Merging this PR triggers the [publish workflow](/.github/workflows/publish.yaml) which publishes packages to npm.
- **Manual releases**: `pnpm changes:version` updates package.json, CHANGELOG.md, and creates a git commit. Push to `main` and the publish workflow will handle the rest (including tags and GitHub releases). Use `--tags` if you need to create tags locally.
- **Test change/release code with preview scripts**: When modifying any change/release code, run `pnpm changes:preview` and `pnpm create-github-release:preview` to test locally. For the version PR script, run `node ./scripts/changes-version-pr.ts --preview`. For the publish script, run `node ./scripts/publish.ts --preview`.
