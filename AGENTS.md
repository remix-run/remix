...
- **Versioning**: Follow semver, but ensure you follow 0.x conventions where breaking changes can happen in minor releases:
  ...
  - For **prerelease packages** (has `.changes/prerelease.json`): Use normal `major/minor/patch` change files. These will bump the prerelease counter instead of semver (e.g. `1.0.0-alpha.1` → `1.0.0-alpha.2`). Changelog entries still get proper Major/Minor/Patch sections.
  - For **entering prerelease**: Create `prerelease.json` with `{ "tag": "alpha" }` and add a **major** change file. Prereleases are currently for major versions only. Version becomes next major + suffix (e.g. `1.5.0` → `2.0.0-alpha.0`).
  - For **transitioning prerelease tags** (e.g. alpha → beta): Update `prerelease.json` tag and add a change file. Version resets to new tag (e.g. `1.0.0-alpha.5` → `1.0.0-beta.0`). The bump type is for changelog categorization only—by convention, use `patch`.
  - For **graduating from prerelease**: Delete `prerelease.json` and add a change file. The prerelease suffix will be stripped (e.g. `1.0.0-alpha.5` → `1.0.0`). The bump type is for changelog categorization only—by convention, use `major` for a major release announcement.
...
- **Test change/release code with preview scripts**: When modifying any change/release code, run `pnpm changes:preview` to test locally. For the version PR script, run `node ./scripts/changes-version-pr.ts --preview`.
