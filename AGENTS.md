...
- **PR branch auto-updates**: The `Update Remix package` workflow can push commits to feature branches/PRs when package surface changes need to be reflected in the generated `remix` package exports. If a push is rejected with a non-fast-forward error, fetch/rebase onto the latest remote branch and push again. Treat these bot-generated branch updates as expected when working on package-level PRs.
