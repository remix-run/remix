...
- **Generics**: Use descriptive lowercase names for type parameters (e.g., `source`, `method`, `pattern`) instead of single uppercase letters like `T`, `P`, or `K`. The type parameter and variable should ideally share the same name (e.g., `<file extends FileLike>(file: file)`)
