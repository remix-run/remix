...
describe('parse()', () => {
  it('parses all elements from a realistic HTML document', () => {
    let elements = parse(fixtureHtml).elements
  ...
  it('serializes the parsed tree instead of the flat element list', () => {
    ...
    assert.equal(dom.toString(), '<div class="test"><a href="/home">Home</a></div>')
  })
  ...
  it('serializes attribute and innerHTML mutations in place', () => {
    ...
    let links = dom.elements.filter((el) => el.name === 'link' && el.getAttribute('href') !== null)
    let anchors = dom.elements.filter((el) => el.name === 'a' && el.getAttribute('href') !== null)
  ...
  it('preserves comments in the serialized tree when enabled', () => {
    ...
    assert.equal(dom.toString(), '<div><!-- rmx:h:0 --><span>content</span><!-- /rmx:h --></div>')
  })
})
