...
const TYPE_ORDER: ApiTypeKind[] = ['type', 'interface', 'class', 'function', 'mixin', 'variable']
  ...
  for (let pkg of sortedPackages) {
    ...
    if (overview) {
      let page: PageDefinition = {
        ...
        eyebrow: 'Package',
        navLabel: 'Overview',
        path: routes.api.href({ version, slug: overview.urlPath }),
        sectionId: pkg,
        title: pkg,
        docFile: overview,
      }
    ...
    if (demos && demos.length > 0) {
      ...
      for (let file of sortedDemos) {
        let page: PageDefinition = {
          ...
          eyebrow: `${pkg} · Demo`,
          navLabel: file.name,
          path: routes.api.href({ version, slug: file.urlPath }),
          sectionId: pkg,
          title: file.name,
          docFile: file,
        }
...
export function buildNotFoundPage(slug: string, version?: string): PageDefinition {
  return {
    ...
    eyebrow: 'Not Found',
    navLabel: 'Not Found',
    path: routes.api.href({ version, slug }),
    sectionId: 'start',
